    ...
    <div class="space-y-8 pb-12">
        <div class="relative overflow-hidden bg-slate-900 rounded-3xl shadow-2xl border border-slate-800">
            ...
            <div class="relative px-8 py-10 z-10">
                ...
                <p class="text-slate-300 font-medium max-w-xl mt-2">
                    Escolha por onde e quando deseja receber cada tipo de aviso. O horário "Não perturbe" evita envio de
                    e-mail e push no período definido.
                </p>
            </div>
        </div>

        {{-- Infobox educativa: privacidade e controle --}}
        <div class="rounded-2xl border border-indigo-200 dark:border-indigo-800 bg-indigo-50/50 dark:bg-indigo-950/30 p-6"
            role="region" aria-label="Sobre suas preferências">
            <div class="flex gap-4">
                ...
                <div>
                    <h2 class="text-lg font-bold text-gray-900 dark:text-white" style="font-family: 'Poppins', sans-serif;">
                        Sua privacidade é nossa prioridade</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-300 mt-1" style="font-family: 'Inter', sans-serif;">
                        Escolha aqui quais avisos deseja receber por E-mail ou Push para manter seu foco no que importa.
                        Nada é enviado sem sua preferência por canal.
                    </p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-2" style="font-family: 'Inter', sans-serif;">
                        Notificações marcadas pela administração como <strong>importantes</strong> (prioridade alta/urgente)
                        são sempre entregues, mesmo no horário de silêncio ou com canal desativado.
                    </p>
                </div>
            </div>
        </div>
        ...
        @if (auth()->user() &&
                (auth()->user()->hasRole('admin') ||
                    auth()->user()->hasRole('lideranca') ||
                    (method_exists(auth()->user(), 'isAdmin') && auth()->user()->isAdmin())) &&
                Route::has('admin.notifications.control.dashboard'))
            <div
                class="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/30 px-4 py-3 flex items-center gap-3">
                <x-icon name="circle-info" class="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0" />
                <p class="text-sm text-amber-800 dark:text-amber-200">
                    Você está no <strong>painel do membro</strong>, configurando as preferências do seu perfil. Para
                    gerenciar notificações do sistema (enviar, templates, Control Room), use o
                    <a href="{{ route('admin.notifications.control.dashboard') }}"
                        class="font-semibold underline hover:no-underline">painel administrativo → Notificações</a>.
                </p>
            </div>
        @endif
        ...
        @if (session('success'))
            <div
                class="rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 p-4 flex items-center gap-3">
        ...
        <form action="{{ route('memberpanel.preferences.notifications.update') }}" method="POST" class="space-y-6">
            ...
            <div
                ...
                <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/30">
                    ...
                    <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Marque os canais em que deseja receber cada
                        aviso.</p>
                </div>
                    ...
                    @foreach ($notificationTypes as $typeKey => $typeLabel)
                        ...
                        <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-4">
                            <div class="sm:w-64 shrink-0">
                                <label
                                    class="block text-sm font-bold text-gray-700 dark:text-slate-300">{{ $typeLabel }}</label>
                                <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Não perturbe (opcional)</p>
                                <div class="flex items-center gap-2 mt-2">
                                    <input type="time" name="preferences[{{ $typeKey }}][dnd_from]"
                                        value="{{ $dndFrom }}"
                                    ...
                                    <input type="time" name="preferences[{{ $typeKey }}][dnd_to]"
                                        value="{{ $dndTo }}"
                                        class="rounded-lg border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white text-sm w-28"
                            ...
                            <div class="flex flex-wrap gap-4">
                                @foreach ($channels as $channelKey => $channelLabel)
                                    <label class="inline-flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="preferences[{{ $typeKey }}][channels][]"
                                            value="{{ $channelKey }}"
                                        ...
                                        <span
                                            class="text-sm font-medium text-gray-700 dark:text-slate-300">{{ $channelLabel }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
            ...
            <div class="flex flex-wrap items-center justify-end gap-4">
                <button type="submit"
                    class="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all">
