...
- **Pastores/Ministros**: Pregação, ensino e cuidado pastoral
...
- **Fallback**: Se a igreja não tiver conselho formal, permitir que admins com role `pastor` ou `admin` aprovem mesmo sem `CouncilMember` (configurável em Settings: `church_council_allow_admin_approval`).
