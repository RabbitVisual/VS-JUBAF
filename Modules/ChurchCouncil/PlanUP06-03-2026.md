...
"The Final Bridge" (Conselho + Tesouraria + Ministérios)
Contexto: Já implementamos o versionamento, disciplina, transferências e auditoria no Modules\ChurchCouncil.

Missão Final: Agora, preciso fechar as integrações pendentes com outros módulos para que o sistema seja o hub de governança completo.

1. Integração com Tesouraria (Treasury):

Crie uma funcionalidade de "Parecer Fiscal". O Conselho deve poder visualizar os fechamentos mensais vindos do módulo Treasury.

Adicione um botão "Aprovar para Assembleia" nos relatórios financeiros. Somente após esse "ok" do conselho, o relatório deve ser marcado como ready_for_assembly.

2. Integração com Ministérios e Eventos (Ministries/Events):

Implemente um painel de "Homologação de Planejamento".

O Conselho deve listar eventos criados no módulo Events que estão com status waiting_approval.

Ao aprovar no Conselho, o evento muda automaticamente para active ou published.

3. Visto Digital (Ata Digital):

No histórico de versões das Atas, adicione uma tabela minutes_signatures (user_id, minutes_version_id, signed_at).

Crie um botão simples "Dar Visto na Ata" para os membros do conselho. O PDF final da ata deve listar no rodapé os nomes de quem deu o visto digital.

4. Refinamento de Segurança de Arquivos:

Garanta que arquivos de Casos Disciplinares sejam salvos em um disco/pasta protected (não acessível por URL pública), enquanto Editais de Convocação podem ser públicos.

Requisito: Siga o padrão de Services e Notifications já estabelecido nas fases anteriores. Após isso, revise se todos os módulos (19 ao todo) possuem algum ponto de contato lógico com o Conselho que ainda não foi mapeado.
