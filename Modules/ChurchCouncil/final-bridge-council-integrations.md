...
- **Objetivo**: completar as integrações entre `ChurchCouncil` e os demais módulos para que o conselho seja o hub de governança: parecer fiscal, homologação de planejamento (eventos/ministérios), visto digital em atas e endurecimento da segurança de arquivos.
- **Abordagem**: aproveitar padrões já existentes (API v1 da Tesouraria, `CouncilApproval`, `CouncilAuditService`, `InAppNotificationService`) e acrescentar apenas o mínimo de novas tabelas/campos necessários, mantendo o desenho modular.
...
flowchart LR
  ...
  council -->|Parecer Fiscal| treasury
  council -->|Homologação Eventos| events
  events -->|requires_council_approval + CouncilApproval| council
  council -->|Diretrizes & Decisões| ministries
```

- **ChurchCouncil** continua orquestrando: decisões, atas, disciplina, transferências e agora pareceres fiscais + homologação de planejamento.
...
- Já existe integração para `requires_council_approval` em `EventController@store` e `@update` criando `CouncilApproval::TYPE_EVENT_CREATION` e regredindo status para `draft`.
...
- Manter `CouncilApproval::executeApproval` como hoje, garantindo que, ao aprovar, o status do evento seja atualizado para `STATUS_PUBLISHED` (ativo) – já alinhado ao requisito "active ou published".
...
- Na tela de detalhes do evento (`events::admin.events.show`), destacar quando `requires_council_approval` estiver ativo e o evento estiver `waiting_approval`, mostrando um selo "Aguardando homologação do conselho" ao lado do ministério associado.
...
- Em `CouncilApprovalObserver` já há notificações para novas solicitações; o painel só organiza a fila.
- Garantir auditoria via `CouncilAuditService` quando um `CouncilApproval` de evento for aprovado/rejeitado (já em parte feito na fase anterior, apenas reforçar para o tipo `TYPE_EVENT_CREATION`).
...
- Em quaisquer novos uploads sensíveis (disciplina), registrar via `CouncilAuditService` (`action = 'discipline_file_uploaded'`) com tamanho/nome originais no payload.
...
- **Admin**: define usuários/roles e configurações globais que impactam o conselho (ex.: `church_council_allow_admin_approval`), além de expor o menu de acesso ao módulo ChurchCouncil.
- **Assets**: hoje sem fluxo direto de aprovação; futuras compras acima de limite podem usar `CouncilApproval::TYPE_FINANCIAL_REQUEST` ou um futuro `TYPE_ASSET_ACQUISITION`.
- **Bible**: sem decisões de governança; apenas insumo espiritual (leituras/planos) usado em outros módulos.
- **ChurchCouncil**: hub de governança (reuniões, pautas, atas, disciplina, cartas de transferência, recomendações à assembleia, parecer fiscal e homologação de eventos).
