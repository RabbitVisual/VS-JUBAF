...
<div class="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-200 font-sans transition-colors duration-200">
    ...
    <div class="max-w-7xl mx-auto p-6">
        @if(session('success'))
            <div class="mb-4 rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 px-4 py-3 text-sm font-medium text-green-800 dark:text-green-200 flex items-center gap-2">
                <x-icon name="check-circle" class="w-5 h-5 flex-shrink-0" />
                {{ session('success') }}
            </div>
        @endif
        @if(session('error'))
            <div class="mb-4 rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-4 py-3 text-sm font-medium text-red-800 dark:text-red-200 flex items-center gap-2">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                        <thead class="bg-gray-50 dark:bg-slate-900/50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Data</th>
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Local</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-slate-800 bg-white dark:bg-slate-900">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-700 dark:text-slate-300">
                                    {{ $meeting->scheduled_date->format('d/m/Y') }}
                                    <span class="block text-xs font-normal text-gray-500 dark:text-slate-500">{{ $meeting->scheduled_date->format('H:i') }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    @if ($meeting->description)
                                        <div class="text-xs font-medium text-gray-500 dark:text-slate-500 mt-1 max-w-xs truncate">
                                            {{ $meeting->description }}
                                        </div>
                                    @endif
                                </td>
