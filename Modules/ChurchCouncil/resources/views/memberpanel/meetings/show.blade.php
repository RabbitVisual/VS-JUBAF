...
<div class="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-200 font-sans transition-colors duration-200">
    <div class="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <!-- Meeting Details -->
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm dark:shadow-lg border border-gray-200 dark:border-slate-800 p-6 relative overflow-hidden transition-colors duration-200">
            <div class="absolute top-0 right-0 p-6 opacity-10">
                <x-icon name="users" class="w-32 h-32 text-blue-600 dark:text-blue-500" />
            </div>

            <h3 class="text-xs font-black text-blue-600 dark:text-blue-500 uppercase tracking-widest mb-6">Informações da Reunião</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                <div>
                    <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-1">Tipo de Reunião</p>
                    <p class="text-lg font-bold text-gray-900 dark:text-white">{{ ucfirst(str_replace('_', ' ', $meeting->meeting_type)) }}</p>
                </div>
                <div>
                    <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-1">Presidente</p>
                    <div class="flex items-center gap-3">
                        @if($meeting->president && $meeting->president->user->photo)
                            <img src="{{ asset('storage/' . $meeting->president->user->photo) }}" class="w-8 h-8 rounded-full border border-gray-200 dark:border-slate-700" alt="">
                        @else
                            <div class="w-8 h-8 rounded-full bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-slate-400">
                                {{ $meeting->president ? substr($meeting->president->user->name, 0, 1) : '?' }}
                            </div>
                        @endif
                        <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $meeting->president->user->name ?? 'Não definido' }}</p>
                    </div>
                </div>
                @if($meeting->description)
                <div class="md:col-span-2">
                    <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-2">Descrição</p>
                    <div class="bg-gray-50 dark:bg-slate-950/50 rounded-lg p-4 border border-gray-100 dark:border-slate-800">
                        <p class="text-base text-gray-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">{{ $meeting->description }}</p>
                    </div>
                </div>
                @endif
            </div>
        ...
        <!-- Agendas -->
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm dark:shadow-lg border border-gray-200 dark:border-slate-800 p-6 transition-colors duration-200">
            <div class="flex items-center justify-between mb-6">
                ...
                @if($meeting->status === 'in_progress')
                    <span class="flex items-center gap-2 text-xs font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-500/10 px-3 py-1.5 rounded-full border border-emerald-200 dark:border-emerald-500/20 animate-pulse">
            ...
            @if($meeting->agendas->count() > 0)
                <div class="space-y-4">
                    ...
                    <div class="border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950/30 rounded-xl p-5 hover:border-blue-300 dark:hover:border-slate-700 transition-colors">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-600 dark:text-slate-400 mb-4 leading-relaxed">{{ $agenda->description }}</p>

                                @if($agenda->member_vote)
                                    <div class="mt-4 p-4 bg-gray-100 dark:bg-slate-800/50 rounded-lg border border-gray-200 dark:border-slate-700/50 flex items-start gap-4">
                                        <div class="flex-shrink-0 mt-0.5">
                                            @if($agenda->member_vote->vote == 'yes')
                                                <x-icon name="circle-check" class="w-5 h-5 text-emerald-500 dark:text-emerald-400" />
                                            @elseif($agenda->member_vote->vote == 'no')
                                                <x-icon name="circle-xmark" class="w-5 h-5 text-red-500 dark:text-red-400" />
                                            @else
                                                <x-icon name="circle-minus" class="w-5 h-5 text-gray-400 dark:text-slate-400" />
                                            @endif
                                        </div>
                                        <div>
                                            <p class="text-sm font-bold text-gray-900 dark:text-white mb-1">Seu voto: <span class="uppercase">{{ ucfirst($agenda->member_vote->vote) }}</span></p>
                                            @if($agenda->member_vote->comments)
                                                <p class="text-xs text-gray-500 dark:text-slate-400 italic">"{{ $agenda->member_vote->comments }}"</p>
                                            @endif
                                        </div>
                                    </div>
                                @elseif($meeting->status === 'in_progress' && $agenda->status === 'pending')
                                    <form method="POST" action="{{ route('memberpanel.churchcouncil.meetings.vote', $agenda) }}" class="mt-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-500/20 p-4 rounded-xl">
                                        @csrf
                                        <p class="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-2">Registrar Voto</p>
                                        <div class="flex flex-col sm:flex-row items-center gap-3">
                                            ...
                                            <input type="text" name="comments" placeholder="Comentários (opcional)"
            ...
            @else
                <div class="text-center py-10 border-2 border-dashed border-gray-200 dark:border-slate-800 rounded-xl">
                    ...
                    <p class="text-gray-500 dark:text-slate-500 font-medium">Nenhuma pauta adicionada a esta reunião ainda.</p>
                </div>
            @endif
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm dark:shadow-lg border border-gray-200 dark:border-slate-800 p-6 transition-colors duration-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="flex items-center space-x-3 p-3 rounded-lg bg-gray-50 dark:bg-slate-950/50 border border-gray-200 dark:border-slate-800">
                    <div class="flex-shrink-0">
                        @if($participant->user->photo)
                            <img class="h-10 w-10 rounded-full object-cover border border-gray-200 dark:border-slate-700" src="{{ asset('storage/' . $participant->user->photo) }}" alt="{{ $participant->user->name }}">
                        @else
                            <div class="h-10 w-10 rounded-full bg-gray-200 dark:bg-slate-800 flex items-center justify-center border border-gray-200 dark:border-slate-700">
                                <span class="text-sm font-bold text-gray-500 dark:text-slate-400">{{ substr($participant->user->name, 0, 1) }}</span>
                            </div>
                        @endif
                    </div>
                    <div>
                        <p class="text-sm font-bold text-gray-900 dark:text-white">{{ $participant->user->name }}</p>
                        <p class="text-xs text-gray-500 dark:text-slate-500 font-medium uppercase tracking-wide">{{ $participant->role_display }}</p>
                    </div>
                </div>
                @endforeach
            </div>
