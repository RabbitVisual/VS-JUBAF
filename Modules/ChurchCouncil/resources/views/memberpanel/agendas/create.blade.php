...
@section('title', 'Criar Nova Pauta - Conselho da Igreja')
...
<div class="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-200 font-sans transition-colors duration-200">
    <div class="max-w-3xl mx-auto p-6 space-y-6">
        ...
        <!-- Form -->
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm dark:shadow-lg border border-gray-200 dark:border-slate-800 p-6 md:p-8 relative overflow-hidden transition-colors duration-200">
            ...
            <form id="createAgendaForm" class="space-y-6 relative z-10">
                ...
                <!-- Meeting Selection -->
                <div>
                     <label for="meeting_id" class="block text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">
                ...
                <!-- Priority -->
                <div>
                    <label for="priority" class="block text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">
                ...
                <!-- Form Actions -->
                <div class="flex flex-col md:flex-row justify-end gap-3 pt-6 border-t border-gray-100 dark:border-slate-800 mt-8">
                    <a href="{{ route('memberpanel.churchcouncil.agendas.index') }}"
...
document.getElementById('createAgendaForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('{{ route("memberpanel.churchcouncil.agendas.store") }}', {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            window.location.href = result.redirect || '{{ route("memberpanel.churchcouncil.agendas.index") }}';
        } else {
