...
@section('title', 'Minhas Pautas - Conselho da Igreja')
...
<div class="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-200 font-sans transition-colors duration-200">
    <!-- Hero Section -->
    <div class="relative bg-linear-to-r from-emerald-900 to-emerald-800 dark:from-slate-900 dark:to-slate-950 border-b border-emerald-200 dark:border-emerald-900/30 p-6 md:p-10 transition-colors duration-200">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="flex-1 text-center md:text-left space-y-2">
                <p class="text-emerald-200 dark:text-emerald-500 font-bold uppercase tracking-widest text-xs">Minhas Propostas</p>
            ...
            <a href="{{ route('memberpanel.churchcouncil.agendas.create') }}"
               class="group relative inline-flex items-center justify-center px-6 py-3 bg-white dark:bg-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-500 text-emerald-700 dark:text-white rounded-lg font-bold text-sm transition-all shadow-lg hover:shadow-xl dark:hover:shadow-emerald-500/20 hover:-translate-y-0.5 border border-emerald-200 dark:border-emerald-500/50">
                <x-icon name="plus" class="w-5 h-5 mr-2 text-emerald-600 dark:text-white group-hover:scale-110 transition-transform" />
                Nova Pauta
            </a>
        </div>
    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                        <thead class="bg-gray-50 dark:bg-slate-900/50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Pauta</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Reunião</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Prioridade</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-slate-800 bg-white dark:bg-slate-900">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-gray-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                                        {{ $agenda->title }}
                                    </div>
                                    <div class="text-sm text-gray-500 dark:text-slate-500 mt-1 leading-snug">{{ Str::limit($agenda->description, 60) }}</div>
                                    <div class="flex items-center gap-2 text-[10px] text-gray-400 dark:text-slate-600 mt-1 uppercase tracking-wider">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-slate-300">
                                    @if ($agenda->meeting)
                                        ...
                                        <span class="block text-xs font-normal text-gray-500 dark:text-slate-500">{{ $agenda->meeting->scheduled_date->format('d/m/Y') }}</span>
                                    @else
