...
@section('title', 'Aprovações - Conselho da Igreja')
...
<div class="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-200 font-sans transition-colors duration-200">
    <!-- Hero Section -->
    <div class="relative bg-linear-to-r from-amber-600 to-amber-500 dark:from-slate-900 dark:to-slate-950 border-b border-amber-200 dark:border-amber-900/30 p-6 md:p-10 transition-colors duration-200">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="flex-1 text-center md:text-left space-y-2">
                <p class="text-amber-100 dark:text-amber-500 font-bold uppercase tracking-widest text-xs">Solicitações</p>
            ...
            <button onclick="showApprovalRequestModal()"
               class="group relative inline-flex items-center justify-center px-6 py-3 bg-white dark:bg-amber-600 hover:bg-amber-50 dark:hover:bg-amber-500 text-amber-600 dark:text-white rounded-lg font-bold text-sm transition-all shadow-lg hover:shadow-xl dark:hover:shadow-amber-500/20 hover:-translate-y-0.5 border border-amber-200 dark:border-amber-500/50">
                <x-icon name="plus" class="w-5 h-5 mr-2 text-amber-600 dark:text-white group-hover:scale-110 transition-transform" />
                Solicitar Aprovação
            </button>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-lg overflow-hidden transition-colors duration-200">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                        <thead class="bg-gray-50 dark:bg-slate-900/50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Tipo</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest">Solicitação</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-slate-800 bg-white dark:bg-slate-900">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-600 border border-gray-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="flex flex-col gap-0.5 mt-1">
                                        <div class="text-xs font-medium text-gray-500 dark:text-slate-500 mt-0.5 max-w-xs truncate">
                                            {{ Str::limit($approval->description, 60) }}
                                        </div>
                                        <span class="text-[10px] uppercase tracking-wider text-gray-400 dark:text-slate-600">Solicitado em {{ $approval->submitted_at->format('d/m/Y H:i') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col gap-1">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider w-fit border
                                        ...
                                        @if ($approval->is_expired)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 border border-red-200 dark:text-red-400 dark:border-red-500/30 dark:bg-red-500/10 uppercase tracking-wider w-fit">
                                                Expirada
                                            </span>
                                        @endif
                                    </div>
            ...
            @else
                <div class="p-16 text-center">
                    ...
                    <p class="text-gray-500 dark:text-slate-400 mb-6">Você ainda não fez nenhuma solicitação de aprovação.</p>
                    <button onclick="showApprovalRequestModal()"
    ...
    <div id="approval-modal" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm overflow-y-auto h-full w-full hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="relative top-20 mx-auto p-0 border border-gray-200 dark:border-slate-700 w-full max-w-md shadow-2xl rounded-xl bg-white dark:bg-slate-900 transition-colors duration-200">
            <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between">
                ...
                <button onclick="closeApprovalRequestModal()" class="text-gray-400 hover:text-gray-600 dark:text-slate-400 dark:hover:text-white transition-colors">
                    <x-icon name="xmark" class="w-5 h-5" />
                </button>
            </div>

            <div class="p-6">
                <form id="approval-form" method="POST" action="{{ route('memberpanel.churchcouncil.submit-approval') }}">
                    @csrf
                    <div class="space-y-4">
                        <div>
                            <label for="approval_type" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                                Tipo de Aprovação
                            </label>
                        ...
                        <div>
                            <label for="title" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                                Título
                            </label>
                        ...
                        <div>
                            <label for="description" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                                Descrição
                            </label>
                        ...
                        <div>
                            <label for="amount" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                                Valor (se aplicável)
                            </label>
