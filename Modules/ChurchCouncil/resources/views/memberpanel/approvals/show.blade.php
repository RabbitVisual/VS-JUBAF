    ...
    <div class="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm dark:shadow-lg border border-gray-200 dark:border-slate-800 p-6 relative overflow-hidden transition-colors duration-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                <!-- Basic Information -->
                <div>
                    <h3 class="text-xs font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest mb-6">Informações Básicas</h3>
                    <div class="space-y-6">
                        <div>
                            <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-1">Título</p>
                            <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $approval->title ?? $approval->approval_type_display }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-2">Tipo de Aprovação</p>
                        ...
                        <div>
                            <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-2">Status</p>
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border
                ...
                <!-- Request Information -->
                <div>
                     <h3 class="text-xs font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest mb-6">Informações da Solicitação</h3>
                    <div class="space-y-6">
                        <div>
                            <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-1">Solicitante</p>
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-bold text-gray-900 dark:text-white">{{ $approval->requester->user->name }}</p>
                                    <p class="text-xs text-gray-500 dark:text-slate-500">{{ $approval->requester->user->email }}</p>
                                </div>
                            </div>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-1">Data de Solicitação</p>
                            <p class="text-sm font-bold text-gray-900 dark:text-white">{{ $approval->submitted_at->format('d/m/Y H:i') }}</p>
                        </div>
                        @if($approval->approver)
                            <div>
                                <p class="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-1">Aprovador</p>
                                <div class="flex items-center gap-3">
                                     <div class="w-8 h-8 rounded-full bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-slate-400">
                                        {{ substr($approval->approver->user->name, 0, 1) }}
                                    </div>
                                    <div>
                                        <p class="text-sm font-bold text-gray-900 dark:text-white">{{ $approval->approver->user->name }}</p>
                                        <p class="text-xs text-gray-500 dark:text-slate-500">{{ $approval->approved_at->format('d/m/Y H:i') }}</p>
                                    </div>
                                </div>
                            </div>
                        @endif
            ...
            <div class="mt-8 pt-8 border-t border-gray-100 dark:border-slate-800 relative z-10">
                <h3 class="text-xs font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest mb-4">Descrição</h3>
                <div class="bg-gray-50 dark:bg-slate-950/50 rounded-xl p-6 border border-gray-100 dark:border-slate-800">
                    <p class="text-gray-600 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">{{ $approval->description }}</p>
                </div>
            </div>
            @endif
            ...
            <div class="mt-6 pt-6 border-t border-gray-100 dark:border-slate-800 relative z-10">
                <h3 class="text-xs font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest mb-4">Notas da Decisão</h3>
                <div class="bg-amber-50 dark:bg-amber-900/10 rounded-xl p-6 border border-amber-200 dark:border-amber-500/20">
                     <p class="text-amber-800 dark:text-amber-200/80 whitespace-pre-wrap leading-relaxed italic">"{{ $approval->decision_notes }}"</p>
                </div>
            </div>
            @endif
        </div>
