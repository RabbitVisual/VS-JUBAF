...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">{{ __('churchcouncil::messages.edit_agenda') }}</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">{{ __('churchcouncil::messages.meeting') }}: {{ $meeting->title }}</p>
        </div>
        ...
        <form id="editAgendaForm" class="space-y-8">
            ...
            <div>
                 <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    ...
                    {{ __('churchcouncil::messages.edit_information') }}
                </h3>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="priority" id="priority" required
                                ...
                                <option value="normal" {{ old('priority', $agenda->priority) == 'normal' ? 'selected' : '' }}>Normal</option>
                                <option value="low" {{ old('priority', $agenda->priority) == 'low' ? 'selected' : '' }}>Baixa</option>
                                <option value="high" {{ old('priority', $agenda->priority) == 'high' ? 'selected' : '' }}>Alta</option>
                                <option value="urgent" {{ old('priority', $agenda->priority) == 'urgent' ? 'selected' : '' }}>Urgente</option>
                            </select>
                        ...
                        <div class="relative">
                            <select name="presented_by" id="presented_by"
                                ...
                                @foreach($members as $member)
                                    <option value="{{ $member->id }}" {{ old('presented_by', $agenda->presented_by) == $member->id ? 'selected' : '' }}>
                     ...
                     <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            <select name="status" id="status" required
                                class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none">
                                <option value="pending" {{ old('status', $agenda->status) == 'pending' ? 'selected' : '' }}>Pendente</option>
                                <option value="discussed" {{ old('status', $agenda->status) == 'discussed' ? 'selected' : '' }}>Em Discussão</option>
                                <option value="approved" {{ old('status', $agenda->status) == 'approved' ? 'selected' : '' }}>Aprovada</option>
                                <option value="rejected" {{ old('status', $agenda->status) == 'rejected' ? 'selected' : '' }}>Rejeitada</option>
                                <option value="postponed" {{ old('status', $agenda->status) == 'postponed' ? 'selected' : '' }}>Adiada</option>
                            </select>
                            <x-icon name="chevron-down" class="w-5 h-5 absolute right-3 top-3 text-gray-400 pointer-events-none" />
                        </div>
                    </div>
                        ...
                        <label class="flex items-start gap-3 p-4 border border-dashed border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50/60 dark:bg-gray-800/40 cursor-pointer">
                            ...
                            <span>
                                ...
                                <span class="block text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                    Quando marcado, esta pauta será tratada como recomendação à assembleia; o resultado final será registrado após a votação da igreja.
                                </span>
                            </span>
                        </label>
                    </div>
            ...
            @if ($agenda->versions && $agenda->versions->count() > 0)
                <div class="mt-10 border-t border-dashed border-gray-100 dark:border-gray-700 pt-6">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">
                        As versões registram como o texto e o status desta pauta foram sendo refinados ao longo do processo.
                    </p>
            ...
            <div class="flex flex-col md:flex-row justify-end gap-3 pt-6 border-t border-gray-100 dark:border-gray-700">
                <a href="{{ route('admin.churchcouncil.agendas.index', $meeting) }}"
                   class="px-6 py-2.5 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 font-bold transition-all text-center">
                    {{ __('churchcouncil::messages.cancel') }}
                </a>
                <button type="submit"
                    ...
                    {{ __('churchcouncil::messages.save_changes') }}
                </button>
            </div>
        </form>
    </div>
...
document.getElementById('editAgendaForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('{{ route("admin.churchcouncil.agendas.update", ["meeting" => $meeting->id, "agenda" => $agenda->id]) }}', {
            method: 'POST',
            headers: {
