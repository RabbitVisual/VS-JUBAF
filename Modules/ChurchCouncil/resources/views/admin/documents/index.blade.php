    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4">
            <nav class="flex items-center gap-2 text-sm">
                <a href="{{ route('admin.churchcouncil.index') }}" class="text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 font-medium transition-colors flex items-center gap-1">
                    <x-icon name="users-rectangle" style="duotone" class="w-4 h-4" />
                    {{ __('churchcouncil::messages.council_title') }}
                </a>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
            <form action="{{ route('admin.churchcouncil.documents.index') }}" method="GET" class="flex flex-col md:flex-row md:items-end gap-4">
                <div class="flex-1 md:max-w-xs">
                    <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Tipo de Documento</label>
                    <select name="type" onchange="this.form.submit()"
                        ...
                        <option value="statute" {{ request('type') == 'statute' ? 'selected' : '' }}>{{ __('churchcouncil::messages.doc_type_statute') }}</option>
                        <option value="regiment" {{ request('type') == 'regiment' ? 'selected' : '' }}>{{ __('churchcouncil::messages.doc_type_regiment') }}</option>
                        <option value="regimento_interno" {{ request('type') == 'regimento_interno' ? 'selected' : '' }}>{{ __('churchcouncil::messages.doc_type_regimento_interno') }}</option>
                        <option value="minute" {{ request('type') == 'minute' ? 'selected' : '' }}>{{ __('churchcouncil::messages.doc_type_minute') }}</option>
                        <option value="resolution" {{ request('type') == 'resolution' ? 'selected' : '' }}>{{ __('churchcouncil::messages.doc_type_resolution') }}</option>
                        <option value="declaracao_doutrinaria" {{ request('type') == 'declaracao_doutrinaria' ? 'selected' : '' }}>{{ __('churchcouncil::messages.doc_type_declaracao_doutrinaria') }}</option>
                        <option value="pacto_igrejas" {{ request('type') == 'pacto_igrejas' ? 'selected' : '' }}>{{ __('churchcouncil::messages.doc_type_pacto_igrejas') }}</option>
                        <option value="other" {{ request('type') == 'other' ? 'selected' : '' }}>{{ __('churchcouncil::messages.doc_type_other') }}</option>
                    </select>
                </div>
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.churchcouncil.documents.index') }}"
                       class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-bold text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-colors">
                        ...
                        {{ __('churchcouncil::messages.clear_filters') }}
                    </a>
                </div>
            </form>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700">
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Documento</th>
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tipo</th>
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Data</th>
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Enviado por</th>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="flex items-start gap-3">
                                            <div class="p-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-blue-600 dark:text-blue-400 shrink-0">
                                                <x-icon name="file-lines" style="duotone" class="w-6 h-6" />
                                            </div>
                                            <div class="min-w-0">
                                                <span class="block font-bold text-gray-900 dark:text-white">{{ $doc->title }}</span>
                                                @if($doc->description)
                                                    <span class="text-xs text-gray-500 dark:text-gray-400 line-clamp-1 mt-0.5">{{ $doc->description }}</span>
                                                @endif
                                                @if($doc->is_public)
                                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 mt-1">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 capitalize">
                                            {{ \Modules\ChurchCouncil\App\Models\CouncilDocument::getDocumentTypeLabel($doc->document_type) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            @if($doc->uploader && $doc->uploader->photo)
                                                <img src="{{ asset('storage/' . $doc->uploader->photo) }}" alt="" class="w-8 h-8 rounded-full object-cover ring-2 ring-gray-100 dark:ring-gray-700">
                                            @else
                                                <div class="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center text-sm font-bold text-gray-600 dark:text-gray-300 ring-2 ring-gray-100 dark:ring-gray-700">
                                                    {{ strtoupper(substr($doc->uploader->name ?? 'U', 0, 1)) }}
                                                </div>
                                            @endif
                                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $doc->uploader->name ?? 'Usuário' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-1">
                                            <a href="{{ route('admin.churchcouncil.documents.download', $doc) }}"
                                               ...
                                               title="{{ __('churchcouncil::messages.download') }}">
                                                <x-icon name="download" style="duotone" class="w-5 h-5" />
                                            </a>
                                            <a href="{{ route('admin.churchcouncil.documents.edit', $doc) }}"
                                               ...
                                               title="{{ __('churchcouncil::messages.edit') }}">
                                                <x-icon name="pencil" style="duotone" class="w-5 h-5" />
                                            </a>
                                            <form action="{{ route('admin.churchcouncil.documents.destroy', $doc) }}" method="POST" class="inline-block"
            ...
            @else
                <div class="p-12 md:p-16 text-center">
                    <div class="w-20 h-20 rounded-3xl bg-gray-100 dark:bg-gray-700/50 flex items-center justify-center mx-auto mb-5">
