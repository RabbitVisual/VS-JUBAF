        ...
        <div class="relative overflow-hidden rounded-3xl bg-linear-to-br from-gray-900 to-gray-800 text-white shadow-xl border border-gray-700/50">
            ...
            <div class="relative p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p class="text-gray-300 max-w-xl">{{ __('churchcouncil::messages.council_subtitle') }}</p>
                </div>
                <div class="flex flex-shrink-0 flex-wrap items-center gap-3">
                    <a href="{{ route('admin.churchcouncil.members.index') }}" class="px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white font-bold hover:bg-white/20 inline-flex items-center gap-2">
                        <x-icon name="users" class="w-5 h-5" /> {{ __('churchcouncil::messages.members') }}
                    </a>
                    <a href="{{ route('admin.churchcouncil.ministries.dashboard') }}" class="px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white font-bold hover:bg-white/20 inline-flex items-center gap-2">
                        <x-icon name="traffic-light" class="w-5 h-5" /> Ministérios
                    </a>
                    <a href="{{ route('admin.churchcouncil.documents.index') }}" class="px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white font-bold hover:bg-white/20 inline-flex items-center gap-2">
                        <x-icon name="file-lines" class="w-5 h-5" /> {{ __('churchcouncil::messages.documents') }}
                    </a>
                    <a href="{{ route('admin.churchcouncil.projects.index') }}" class="px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white font-bold hover:bg-white/20 inline-flex items-center gap-2">
                        <x-icon name="diagram-project" class="w-5 h-5" /> {{ __('churchcouncil::messages.projects') }}
                    </a>
                    <a href="{{ route('admin.churchcouncil.meetings.create') }}" class="px-6 py-3 rounded-xl bg-white text-gray-900 font-bold hover:bg-gray-100 shadow-lg shadow-white/10 inline-flex items-center gap-2">
                        <x-icon name="plus" class="w-5 h-5 text-blue-600" /> {{ __('churchcouncil::messages.new_meeting') }}
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-200 dark:border-gray-700 relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <p class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ __('churchcouncil::messages.active_members') }}</p>
                    ...
                    <div class="mt-4 flex items-center text-sm text-gray-500 dark:text-gray-400 font-medium">
                        ...
                        {{ __('churchcouncil::messages.counselors_registered') }}
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-200 dark:border-gray-700 relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <p class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ __('churchcouncil::messages.upcoming_meetings') }}</p>
                    <h3 class="text-3xl font-black text-gray-900 dark:text-white mt-2">
                        {{ number_format($stats['upcoming_meetings'] ?? 0) }}
                    </h3>
                    <div class="mt-4 flex items-center text-sm text-gray-500 dark:text-gray-400 font-medium">
                        <span class="inline-block w-2 h-2 rounded-full bg-green-500 mr-2 animate-pulse"></span>
                        {{ __('churchcouncil::messages.scheduled') }}
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-200 dark:border-gray-700 relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <p class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ __('churchcouncil::messages.pending_approvals') }}</p>
                    <h3 class="text-3xl font-black text-gray-900 dark:text-white mt-2">
                        {{ number_format($stats['pending_approvals'] ?? 0) }}
                    </h3>
                    <div class="mt-4 flex items-center text-sm text-gray-500 dark:text-gray-400 font-medium">
                        <span class="inline-block w-2 h-2 rounded-full bg-yellow-500 mr-2"></span>
                        {{ __('churchcouncil::messages.pending') }}
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-200 dark:border-gray-700 relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <p class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ __('churchcouncil::messages.completed_meetings') }}</p>
                    <h3 class="text-3xl font-black text-gray-900 dark:text-white mt-2">
                        {{ number_format($stats['completed_meetings'] ?? 0) }}
                    </h3>
                    <div class="mt-4 flex items-center text-sm text-gray-500 dark:text-gray-400 font-medium">
                        <span class="inline-block w-2 h-2 rounded-full bg-purple-500 mr-2"></span>
                        {{ __('churchcouncil::messages.total_history') }}
                    </div>
                </div>
            </div>
        </div>
                        ...
                        @foreach ($membersByRole as $item)
                            <div class="group">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-sm font-bold text-gray-700 dark:text-gray-300 capitalize flex items-center gap-2">
                                        <div class="w-2 h-2 rounded-full bg-indigo-500"></div>
                                        {{ str_replace('_', ' ', $item->council_role_display ?? $item->council_role) }}
                                    </span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-lg">
                                        {{ $item->count }}
                                    </span>
                                </div>
                                <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                     <div class="bg-indigo-500 h-1.5 rounded-full transition-all duration-500 group-hover:bg-indigo-400"
                                          style="width: {{ ($stats['total_members'] > 0) ? ($item->count / $stats['total_members'] * 100) : 0 }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                            ...
                            <div class="group">
                                ...
                                <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                     <div class="bg-purple-500 h-1.5 rounded-full transition-all duration-500 group-hover:bg-purple-400"
                                          style="width: {{ $totalMeetings > 0 ? ($item->count / $totalMeetings * 100) : 0 }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @foreach ($recentMeetings->take(5) as $meeting)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-bold text-gray-900 dark:text-white block">{{ Str::limit($meeting->title, 40) }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex -space-x-2 overflow-hidden">
                                            @foreach($meeting->participant_members->take(3) as $participant)
                                                @php
                                                    $pName = $participant->user->name ?? $participant->name ?? 'Membro';
                                                ...
                                                @if($pPhoto)
                                                    <img class="inline-block h-6 w-6 rounded-full ring-2 ring-white dark:ring-gray-800 object-cover"
                                                         ...
                                                         title="{{ $pName }}">
                                                @else
                                                    <div class="inline-block h-6 w-6 rounded-full ring-2 ring-white dark:ring-gray-800 bg-gray-200 dark:bg-gray-600 flex items-center justify-center text-xs font-bold text-gray-600 dark:text-gray-300" title="{{ $pName }}">
                                                        {{ substr($pName, 0, 1) }}
                                                    </div>
                                                @endif
                                            @endforeach
                                            @if($meeting->participant_members->count() > 3)
                                                <div class="inline-block h-6 w-6 rounded-full ring-2 ring-white dark:ring-gray-800 bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-gray-400">
