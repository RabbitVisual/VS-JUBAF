...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-500 dark:text-gray-400 mt-1 font-medium">Analise e aprove projetos submetidos pelos ministérios.</p>
                </div>
                <a href="{{ route('admin.churchcouncil.projects.create') }}"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
            <form action="{{ route('admin.churchcouncil.projects.index') }}" method="GET" class="flex flex-col md:flex-row md:items-end gap-4">
                <div class="flex-1 md:max-w-xs">
                    ...
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="submitted" {{ request('status') == 'submitted' ? 'selected' : '' }}>Submetidos</option>
                        <option value="under_review" {{ request('status') == 'under_review' ? 'selected' : '' }}>Em Revisão</option>
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.churchcouncil.projects.index') }}"
        ...
        @if($projects->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($projects as $project)
                    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col hover:border-blue-200 dark:hover:border-blue-800 transition-all overflow-hidden group">
                        <div class="p-6 flex-1 flex flex-col">
                            <div class="flex justify-between items-start gap-3 mb-3">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold capitalize shrink-0
                            ...
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2 line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $project->title }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400 line-clamp-3 mb-4 flex-1">{{ $project->description }}</p>

                            <div class="space-y-2 mt-auto">
                                <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                    @if($project->proposer)
                                        @if($project->proposer->photo ?? null)
                                            <img src="{{ asset('storage/' . $project->proposer->photo) }}" alt="" class="w-6 h-6 rounded-full object-cover">
                                        @else
                                            <div class="w-6 h-6 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center text-xs font-bold text-gray-600 dark:text-gray-300">
                                                {{ strtoupper(substr($project->proposer->name ?? 'U', 0, 1)) }}
                                            </div>
                                        @endif
                                        <span class="font-medium text-gray-700 dark:text-gray-300">{{ $project->proposer->name }}</span>
                                    @else
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    @if($project->ministry)
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300">
                                    ...
                                    @if($project->estimated_cost && (float) $project->estimated_cost > 0)
                                        <span class="inline-flex items-center gap-1 text-xs font-bold text-gray-700 dark:text-gray-300">
                                            <x-icon name="money-bill-wave" style="duotone" class="w-3.5 h-3.5 text-green-600 dark:text-green-400" />
                        ...
                        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
                            ...
                            <a href="{{ route('admin.churchcouncil.projects.show', $project) }}"
                               class="inline-flex items-center gap-1.5 text-sm font-bold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors">
                                {{ __('churchcouncil::messages.view') }}
            ...
            @endif
        @else
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-12 md:p-16 text-center">
                <div class="w-20 h-20 rounded-3xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center mx-auto mb-5">
                ...
                <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-sm mx-auto">Nenhum projeto ou proposta foi encontrado com os filtros selecionados. Crie uma nova proposta ou limpe os filtros.</p>
