    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="md:col-span-2 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
                ...
                @else
                    <p class="text-sm text-gray-500 dark:text-gray-400">Nenhuma ação registrada além da abertura do caso.</p>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-4">
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
                ...
                <p class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide
                ...
                @if($case->current_stage)
                    <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                        Etapa atual: <span class="font-semibold text-gray-800 dark:text-gray-100">{{ $case->current_stage }}</span>
                    </p>
                @endif
...
document.getElementById('disciplineActionForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('{{ route("admin.churchcouncil.discipline.store-action", $case) }}', {
            method: 'POST',
            headers: {
...
document.getElementById('disciplineFileForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('{{ route("admin.churchcouncil.discipline.files.store", $case) }}', {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'X-CSRF-TOKEN': document.querySelector(\'meta[name="csrf-token"]\').content,
