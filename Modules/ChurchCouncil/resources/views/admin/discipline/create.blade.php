            ...
            <div class="space-y-4">
                ...
                <div>
                    <label for="summary" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        Resumo pastoral do caso <span class="text-red-500">*</span>
                    </label>
                    <textarea name="summary" id="summary" rows="6"
                              ...
                              placeholder="Descreva de forma pastoral a situação, sem expor detalhes desnecessários.">{{ old('summary') }}</textarea>
                </div>
            </div>
...
document.getElementById('createDisciplineCaseForm').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (result.success) {
            window.location.href = result.redirect || '{{ route("admin.churchcouncil.discipline.index") }}';
        } else {
