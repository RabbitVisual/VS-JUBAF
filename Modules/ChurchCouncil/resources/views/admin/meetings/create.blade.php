        ...
        <form id="createMeetingForm" class="space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="meeting_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        ...
                        <div class="relative">
                            <select name="meeting_type" id="meeting_type" required
                                ...
                                <option value="ordinary" {{ old('meeting_type') == 'ordinary' ? 'selected' : '' }}>Ordinária</option>
                                <option value="extraordinary" {{ old('meeting_type') == 'extraordinary' ? 'selected' : '' }}>Extraordinária</option>
                                <option value="emergency" {{ old('meeting_type') == 'emergency' ? 'selected' : '' }}>Emergencial</option>
                            </select>
                    ...
                    <div>
                        <label for="scheduled_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        ...
                        <div class="relative">
                            <select name="president_id" id="president_id"
                                ...
                                @foreach($presidents as $president)
                                    <option value="{{ $president->id }}" {{ old('president_id') == $president->id ? 'selected' : '' }}>
            ...
            <div class="pt-6 border-t border-gray-100 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    @foreach($councilMembers as $member)
                        <label class="group flex items-center p-3 border border-gray-200 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-all hover:border-blue-300 dark:hover:border-blue-700">
                            ...
                            <div class="ml-3 flex items-center">
                                @if($member->user->photo)
                                    <img src="{{ asset('storage/' . $member->user->photo) }}" class="w-8 h-8 rounded-full mr-2 object-cover">
                                @else
                                    <div class="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center mr-2 text-xs font-bold text-gray-600 dark:text-gray-300">
                                        {{ substr($member->user->name, 0, 1) }}
                                    </div>
                                @endif
                                <div>
                                    <span class="block text-sm font-medium text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">{{ $member->user->name }}</span>
                                    <span class="block text-xs text-gray-500 dark:text-gray-400">{{ $member->role_display }}</span>
                                </div>
                            </div>
                        </label>
                    @endforeach
...
document.getElementById('createMeetingForm').addEventListener('submit', async function(e) {
    ...
    const participantIds = Array.from(this.querySelectorAll('input[name="participant_ids[]"]:checked')).map(cb => cb.value);
    ...
    try {
        const response = await fetch('{{ route("admin.churchcouncil.meetings.store") }}', {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            window.location.href = result.redirect || '{{ route("admin.churchcouncil.meetings.index") }}';
        } else {
            alert(result.message || '{{ __('churchcouncil::messages.meeting_start_error') }}');
        }
    } catch (error) {
        window.dispatchEvent(new CustomEvent('stop-loading'));
        console.error(error);
