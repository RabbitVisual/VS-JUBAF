...
<div class="space-y-6 max-w-3xl">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Registrar decisão da assembleia</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">
                Pauta: {{ $agenda->title }} (Reunião: {{ $agenda->meeting->title ?? '—' }})
            </p>
        </div>
        <a href="{{ route('admin.churchcouncil.assembly.index') }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        <form id="assemblyDecisionForm" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label for="assembly_votes_for" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        Votos a favor
                    </label>
                ...
                <div>
                    <label for="assembly_votes_against" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        Votos contra
                    </label>
                ...
                <div>
                    <label for="assembly_votes_abstain" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        Abstenções
                    </label>
...
document.getElementById('assemblyDecisionForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('{{ route("admin.churchcouncil.assembly.store-decision", $agenda) }}', {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            window.location.href = result.redirect || '{{ route("admin.churchcouncil.assembly.index") }}';
        } else {
