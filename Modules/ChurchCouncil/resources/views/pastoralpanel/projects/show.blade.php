...
<div class="space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div>
            <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-1">
                ...
                <a href="{{ route('pastor.conselho.projects.index') }}" class="hover:text-white transition-colors">{{ __('churchcouncil::messages.projects') }}</a>
                <x-icon name="chevron-right" class="w-3.5 h-3.5" />
        ...
        <a href="{{ route('pastor.conselho.projects.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-200 dark:bg-slate-700 text-gray-800 dark:text-white font-medium hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 space-y-6">
        ...
        <div>
            <h2 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-2">{{ __('churchcouncil::messages.description') }}</h2>
            <p class="text-gray-700 dark:text-gray-300">{{ $project->description }}</p>
        </div>
        @endif
        @if($project->justification)
        <div>
            <h2 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-2">Justificativa</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-200 dark:border-slate-700">
            <div>
                ...
                <p class="text-gray-900 dark:text-white font-medium">{{ $project->proposer->name ?? __('churchcouncil::messages.no_info') }}</p>
            </div>
            @if($project->ministry)
            <div>
                <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Ministério</p>
        ...
        <div class="pt-4 border-t border-gray-200 dark:border-slate-700">
            <h2 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-2">Parecer do conselho</h2>
