...
@section('title', __('churchcouncil::messages.documents'))
    ...
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div>
            <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-1">
                ...
                <span class="text-white font-bold">{{ __('churchcouncil::messages.documents') }}</span>
            </nav>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <x-icon name="file-lines" class="w-7 h-7 text-amber-500" />
                {{ __('churchcouncil::messages.documents') }}
            </h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">{{ __('churchcouncil::messages.document') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        <div class="divide-y divide-gray-200 dark:divide-slate-700">
            @forelse($documents as $doc)
                <div class="px-6 py-4 flex flex-wrap items-center justify-between gap-4 hover:bg-gray-50 dark:hover:bg-slate-700/30">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <a href="{{ route('pastor.conselho.documents.show', $doc) }}" class="font-medium text-gray-900 dark:text-white hover:underline">{{ $doc->title }}</a>
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                                ...
                                @if($doc->meeting) · {{ $doc->meeting->title }} @endif
                            </p>
                        </div>
                    </div>
                    <a href="{{ route('pastor.conselho.documents.download', $doc) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-medium text-sm transition-colors">
                        <x-icon name="arrow-down-to-bracket" class="w-4 h-4" /> {{ __('churchcouncil::messages.download') }}
                    </a>
                </div>
            @empty
                <div class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                    <x-icon name="file-lines" class="w-12 h-12 mx-auto mb-2 opacity-50" />
