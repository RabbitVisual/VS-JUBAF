...
<div class="space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('admin.churchcouncil.meetings.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-medium transition-colors">
    ...
    <div class="flex flex-wrap items-center gap-2">
        <form method="GET" action="{{ route('pastor.conselho.meetings.index') }}" class="flex flex-wrap items-center gap-3">
            <select name="status" class="rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm">
            ...
            <select name="type" class="rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm">
                <option value="">Tipo</option>
                <option value="ordinary" {{ request('type') === 'ordinary' ? 'selected' : '' }}>Ordinária</option>
                <option value="extraordinary" {{ request('type') === 'extraordinary' ? 'selected' : '' }}>Extraordinária</option>
                <option value="emergency" {{ request('type') === 'emergency' ? 'selected' : '' }}>Emergencial</option>
            </select>
            <button type="submit" class="px-4 py-2 rounded-lg bg-slate-200 dark:bg-slate-600 text-gray-800 dark:text-white text-sm font-medium">{{ __('churchcouncil::messages.filter') }}</button>
        </form>
        <a href="{{ route('pastor.conselho.meetings.index') }}" class="text-sm text-amber-500 hover:underline">{{ __('churchcouncil::messages.clear_filters') }}</a>
    </div>
    @endif
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        <div class="divide-y divide-gray-200 dark:divide-slate-700">
            @forelse($meetings as $meeting)
                <a href="{{ route('pastor.conselho.meetings.show', $meeting) }}" class="block px-6 py-4 hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
