    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
            <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-1">
                ...
                <a href="{{ route('pastor.conselho.meetings.index') }}" class="hover:text-white transition-colors">{{ __('churchcouncil::messages.meetings') }}</a>
                <x-icon name="chevron-right" class="w-3.5 h-3.5" />
            ...
            <div class="mt-2 flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                ...
                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-700 dark:text-gray-300">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('pastor.conselho.meetings.minutes-pdf', $meeting) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-medium transition-colors">
            ...
            <a href="{{ route('pastor.conselho.meetings.convocation-pdf', $meeting) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-200 dark:bg-slate-700 text-gray-800 dark:text-white font-medium hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">
                <x-icon name="file-pdf" class="w-5 h-5" /> Convocação (PDF)
            </a>
            @endif
            <a href="{{ route('pastor.conselho.meetings.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-200 dark:bg-slate-700 text-gray-800 dark:text-white font-medium hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
                ...
                <div class="mb-4">
                    <h3 class="text-sm font-bold text-gray-900 dark:text-white mb-2">{{ __('churchcouncil::messages.initial_agenda_description') }}</h3>
                ...
                <div class="mb-4 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                    <h3 class="text-sm font-bold text-amber-800 dark:text-amber-200 mb-2">{{ __('churchcouncil::messages.main_objectives') }}</h3>
                    <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{{ $meeting->objectives }}</p>
                </div>
                @endif
                @if($meeting->president)
                <div class="flex items-center gap-3 pt-2">
                    @if($meeting->president->user->photo ?? null)
                        <img src="{{ asset('storage/' . $meeting->president->user->photo) }}" alt="" class="w-10 h-10 rounded-full object-cover">
                    @else
                        <div class="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center">
                            <span class="text-sm font-bold text-gray-500">{{ substr($meeting->president->user->name ?? 'P', 0, 1) }}</span>
                        </div>
                    @endif
                    <div>
                        <p class="text-sm font-bold text-gray-900 dark:text-white">{{ $meeting->president->user->name ?? '' }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="p-4 rounded-xl border border-gray-200 dark:border-slate-600 bg-gray-50 dark:bg-slate-700/30">
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ Str::limit($agenda->description, 120) }}</p>
                        @endif
                        @if($agenda->decision)
                        <p class="text-sm font-medium text-gray-800 dark:text-gray-200 mt-2">{{ __('churchcouncil::messages.decision') }}: {{ Str::limit($agenda->decision, 80) }}</p>
                        @endif
                    </div>
                    @endforeach
                </div>
                @else
                <p class="text-gray-500 dark:text-gray-400 text-sm">{{ __('churchcouncil::messages.no_agendas_yet') }}</p>
                @endif
            </div>
        </div>
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
                <h3 class="text-sm font-bold text-gray-900 dark:text-white mb-3">{{ __('churchcouncil::messages.minutes') }} (versões)</h3>
