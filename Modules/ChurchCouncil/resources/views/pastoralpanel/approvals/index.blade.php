...
@section('title', __('churchcouncil::messages.approvals'))
    ...
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                {{ __('churchcouncil::messages.pending_approvals') }}
            </h1>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        <div class="divide-y divide-gray-200 dark:divide-slate-700">
            @forelse($pendingApprovals as $approval)
                <a href="{{ route('pastor.conselho.approvals.show', $approval) }}" class="block px-6 py-4 hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <p class="font-bold text-gray-900 dark:text-white">{{ $approval->approval_type_display ?? 'Solicitação' }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                                {{ $approval->requester->name ?? '' }} · {{ $approval->submitted_at ? $approval->submitted_at->format('d/m/Y H:i') : '' }}
                            </p>
                        </div>
            ...
            @empty
                <div class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                    ...
                    <p>{{ __('churchcouncil::messages.pending_items') }}</p>
                </div>
            @endforelse
        </div>
