...
@section('content')
    <div class="space-y-6">
        ...
        @if (isset($meetings) && $meetings->count() > 0)
            <form method="GET" action="{{ route('pastor.conselho.agendas.index') }}"
                class="flex flex-wrap items-center gap-3">
                <label
                    class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('churchcouncil::messages.meeting') }}:</label>
                    ...
                    <thead class="bg-gray-50 dark:bg-slate-700/50">
                        <tr>
                            <th
                                ...
                                {{ __('churchcouncil::messages.meeting') }}</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                {{ __('churchcouncil::messages.agenda_title') }}</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                {{ __('churchcouncil::messages.current_status') }}</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                {{ __('churchcouncil::messages.decision') }}</th>
                        </tr>
                    </thead>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    {{ __('churchcouncil::messages.no_agenda_found') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
