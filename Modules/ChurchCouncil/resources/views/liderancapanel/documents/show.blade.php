        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-1">
                    ...
                    <a href="{{ route('pastor.conselho.documents.index') }}"
                        class="hover:text-white transition-colors">{{ __('churchcouncil::messages.documents') }}</a>
                    <x-icon name="chevron-right" class="w-3.5 h-3.5" />
            ...
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('pastor.conselho.documents.download', $document) }}"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
            ...
            @if ($document->meeting)
                <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">
                    {{ __('churchcouncil::messages.meeting') }}: <a
                        href="{{ route('pastor.conselho.meetings.show', $document->meeting) }}"
