...
class CouncilApproval extends Model
{
    protected $fillable = [
        'approvable_type',
        'approvable_id',
    ...
    /**
     * Get the council member who approved/rejected this request
     */
    public function approver(): BelongsTo
    {
        return $this->belongsTo(CouncilMember::class, 'approved_by');
    }
     ...
     * Approve the request
     */
    public function approve(CouncilMember $approvedBy, ?string $notes = null): bool
    {
     ...
     * Reject the request
     */
    public function reject(CouncilMember $rejectedBy, string $reason): bool
    {
        if ($this->status !== self::STATUS_PENDING) {
            return false;
        }
     ...
     * Request revision
     */
    public function requestRevision(CouncilMember $reviewedBy, string $notes): bool
    {
        if ($this->status !== self::STATUS_PENDING) {
            return false;
        }
    ...
    private function executeApproval(): void
    {
        ...
        switch ($this->approval_type) {
            ...
            case self::TYPE_MINISTRY_PLAN:
                if (class_exists(\Modules\Ministries\App\Models\MinistryPlan::class) && $approvable instanceof \Modules\Ministries\App\Models\MinistryPlan) {
                    ...
                    if (class_exists(\Modules\ChurchCouncil\App\Services\CouncilAuditService::class)) {
                        app(\Modules\ChurchCouncil\App\Services\CouncilAuditService::class)->log('ministry_plan_approved', $approvable, [
            ...
            case self::TYPE_MEMBERSHIP_TRANSFER_OUT:
                if ($approvable instanceof \Modules\ChurchCouncil\App\Models\TransferLetter) {
                    $approvable->update([
                        'status' => \Modules\ChurchCouncil\App\Models\TransferLetter::STATUS_SENT,
                        'issued_at' => now(),
                    ]);
     ...
     * approved_by on pivot is user_id; we have council_member id in $this->approved_by.
     */
    ...
    /**
     * Mark financial entry as council-approved (status or flag). CBAV2026: set expense_status = approved.
     */
    private function executeFinancialRequestApproval(\Modules\Treasury\App\Models\FinancialEntry $entry): void
    {
        $updates = [];
        if (\Schema::hasColumn($entry->getTable(), 'council_approval_id')) {
            $updates['council_approval_id'] = $this->id;
        }
        if (\Schema::hasColumn($entry->getTable(), 'council_approved_at')) {
            $updates['council_approved_at'] = now();
        }
