...
@section('title', 'Novo Voluntário | Ação Social')
...
@section('content')
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Cadastrar Voluntário</h1>
            <p class="text-sm text-gray-500">Adicione um membro à equipe de ação social.</p>
        </div>
    </div>

    <form action="{{ route('socialaction.admin.volunteers.store') }}" method="POST" class="max-w-3xl mx-auto space-y-6">
        @csrf
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Membro da Igreja <span class="text-red-500">*</span></label>
                    <select name="user_id" required class="w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700/50 dark:text-white shadow-sm focus:border-sky-500 focus:ring-sky-500 py-3 px-4">
                ...
                <div>
                    ...
                    <select name="role" required class="w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700/50 dark:text-white shadow-sm focus:border-sky-500 focus:ring-sky-500 py-3 px-4">
                        <option value="helper" {{ old('role') === 'helper' ? 'selected' : '' }}>Auxiliar</option>
                        <option value="leader" {{ old('role') === 'leader' ? 'selected' : '' }}>Líder</option>
                    </select>
                    @error('role')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Telefone de Contato</label>
                    <input type="text" name="phone" value="{{ old('phone') }}" placeholder="(xx) xxxxx-xxxx"
                ...
                @foreach($availabilityLabels as $key => $label)
                    <label class="flex flex-col items-center gap-1 p-3 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-green-50 dark:hover:bg-green-900/20 hover:border-green-300 dark:hover:border-green-700 transition-all has-[:checked]:bg-green-50 has-[:checked]:border-green-400 has-[:checked]:dark:bg-green-900/30 text-center">
                        <input type="checkbox" name="availability[]" value="{{ $key }}" class="rounded border-gray-300 text-green-600 focus:ring-green-500"
                            {{ in_array($key, old('availability', [])) ? 'checked' : '' }}>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Breve Bio / Testemunho de chamado</label>
                    <textarea name="bio" rows="3" placeholder="Opcional: histórico de serviço, chamado..." class="w-full rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700/50 dark:text-white shadow-sm focus:border-sky-500 focus:ring-sky-500 py-3 px-4 resize-none">{{ old('bio') }}</textarea>
                </div>
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">Voluntário Ativo (disponível para escalas)</span>
                </label>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white rounded-xl font-bold shadow-lg shadow-sky-500/30 transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center gap-2">
                <x-icon name="check" /> Salvar Voluntário
            </button>
        </div>
    </form>
@endsection
