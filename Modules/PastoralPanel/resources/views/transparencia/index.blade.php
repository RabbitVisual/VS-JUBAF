...
@section('content')
<div class="space-y-8">
    <div class="relative overflow-hidden rounded-3xl bg-slate-800 dark:bg-slate-900 border border-amber-900/30 text-white shadow-xl">
        <div class="absolute right-0 top-0 h-full w-1/2 bg-gradient-to-l from-amber-600/20 to-transparent"></div>
        <div class="relative p-8 md:p-10">
            <div class="flex items-center gap-3 mb-2">
                <span class="px-3 py-1.5 rounded-full bg-amber-500/20 border border-amber-400/30 text-amber-200 text-xs font-bold uppercase tracking-wider">Transparência</span>
                <span class="px-3 py-1.5 rounded-full bg-emerald-500/20 border border-emerald-400/30 text-emerald-200 text-xs font-bold uppercase tracking-wider">Somente leitura</span>
            </div>
            <h1 class="text-3xl md:text-4xl font-black tracking-tight mb-2">Saúde Financeira</h1>
            <p class="text-slate-300 max-w-xl">Visão de receitas, despesas e balancetes. Lançamentos e configurações são feitos no painel técnico.</p>
        </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-between">
            <div>
                <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Receita do Mês</p>
                <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white">R$ {{ number_format($monthlyIncome ?? 0, 2, ',', '.') }}</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white">R$ {{ number_format($monthlyExpense ?? 0, 2, ',', '.') }}</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-xl font-bold {{ ($monthlyBalance ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-xl font-bold {{ ($yearlyBalance ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                    ...
                    <div class="space-y-3">
                        @foreach($incomeByCategory as $item)
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-700 dark:text-gray-300 capitalize">{{ str_replace('_', ' ', $item->category) }}</span>
                                <span class="font-bold text-gray-900 dark:text-white">R$ {{ number_format($item->total, 2, ',', '.') }}</span>
                            </div>
                        @endforeach
                    </div>
                @else
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                        @foreach($recentEntries as $entry)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                                <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{{ $entry->entry_date->format('d/m/Y') }}</td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium {{ $entry->type === 'income' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' }}">
                                        {{ $entry->type === 'income' ? 'Entrada' : 'Saída' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{{ $entry->title }}</td>
                                <td class="px-6 py-4 text-right text-sm font-bold {{ $entry->type === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
            ...
            @foreach($activeCampaigns as $campaign)
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-5">
                    ...
                    <div class="mt-2 flex justify-between text-sm text-gray-500 dark:text-gray-400">
                        <span>Arrecadado: R$ {{ number_format($campaign->current_amount ?? 0, 2, ',', '.') }}</span>
                    ...
                    <div class="mt-2 w-full bg-gray-100 dark:bg-slate-700 rounded-full h-2">
                        <div class="bg-amber-500 h-2 rounded-full transition-all" style="width: {{ min(100, $campaign->progress_percentage) }}%"></div>
                    </div>
                    @endif
                </div>
            @endforeach
