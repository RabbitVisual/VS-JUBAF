...
@section('title', 'Criar Nível de Gamificação')
...
<div class="space-y-8">
        ...
        <div class="relative p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <div class="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    <span class="px-3 py-1 rounded-full bg-green-500/20 border border-green-400/30 text-green-300 text-xs font-bold uppercase tracking-wider">Novo Nível</span>
                </div>
                <h1 class="text-3xl md:text-4xl font-black tracking-tight mb-2">Criar Nível</h1>
                <p class="text-gray-300 max-w-xl">Crie um novo nível para o sistema de gamificação. Defina nome, ícone, cor e faixa de pontos.</p>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden relative">
        ...
        <div class="relative p-8">
            <form action="{{ route('admin.gamification-levels.store') }}" method="POST" class="space-y-8" onsubmit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Criando nível...' } }))">
                @csrf
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" id="name" name="name" required value="{{ old('name') }}" placeholder="Ex: Novato"
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" placeholder="Descrição opcional do nível"
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 focus:border-transparent resize-none transition-all">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div>
                            ...
                            <select id="icon" name="icon" required class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
                                @foreach($icons as $iconKey => $iconName)
                                    <option value="{{ $iconKey }}" {{ old('icon') == $iconKey ? 'selected' : '' }}>{{ $iconName }}</option>
                                @endforeach
                            </select>
                            @error('icon')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" id="points_min" name="points_min" required min="0" value="{{ old('points_min', 0) }}"
                                class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
                            @error('points_min')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Pontos mínimos e máximos definem o intervalo deste nível. Deixe máximo vazio para "ou mais".</p>
                        </div>
                        <div>
                            <label for="points_max" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Pontos Máximos</label>
                            <input type="number" id="points_max" name="points_max" min="0" value="{{ old('points_max') }}" placeholder="Opcional (ilimitado)"
                                class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
                            @error('points_max')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            @error('order')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Ordem define a sequência na listagem. Números menores aparecem primeiro.</p>
                        </div>
