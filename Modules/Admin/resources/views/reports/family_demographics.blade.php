...
@section('content')
    <div class="space-y-8" x-data="familyDemographicsDashboard()">
        <!-- Hero -->
        <div
            class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-900 via-purple-900 to-indigo-900 text-white shadow-xl border border-indigo-700/50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div
                class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Núcleos
                        identificados</p>
            ...
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Membros com
                        vínculos</p>
            ...
            <div
                ...
                <div>
                    <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total de vínculos
                    </p>
        ...
        <!-- Destaques liderancaais -->
        <div
            class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div
                class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white">Destaques liderancaais</h2>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Insights para planejamento e conselho</p>
                    </div>
                </div>
                <button type="button" @click="runEliasAnalysis()" :disabled="eliasLoading"
                    ...
                    <span x-text="eliasLoading ? 'Analisando...' : 'Análise liderancaal do Elias'"></span>
                </button>
            </div>
            <div class="p-6">
                <ul class="space-y-3">
                    @foreach ($liderancaalHighlights as $highlight)
                        <li
                            class="flex items-start gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700">
                            <x-icon name="circle-check"
                                class="w-5 h-5 text-emerald-500 dark:text-emerald-400 flex-shrink-0 mt-0.5" />
                            <span class="text-gray-700 dark:text-gray-300 font-medium">{{ $highlight }}</span>
                        </li>
                    @endforeach
                </ul>
                <div x-show="eliasReply" x-cloak
                    class="mt-6 p-6 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800">
                    <h3 class="text-sm font-bold text-indigo-900 dark:text-indigo-300 uppercase tracking-wider mb-3">
                        Sugestões do Elias</h3>
                    <div class="prose prose-sm dark:prose-invert max-w-none text-gray-700 dark:text-gray-300 whitespace-pre-line"
                        x-html="eliasReply"></div>
                </div>
            </div>
        </div>
    </div>
            ...
            if (ctxBar && typeof Chart !== 'undefined') {
                if (labels.length === 0) {
                    ctxBar.parentElement.innerHTML =
                        '<p class="text-gray-500 dark:text-gray-400 text-center">Nenhum dado por região. Cadastre bairro/cidade nos endereços dos membros.</p>';
                } else {
                    new Chart(ctxBar, {
                        type: 'bar',
                        data: {
                ...
                return {
                    ...
                    runEliasAnalysis() {
                        ...
                        window.dispatchEvent(new CustomEvent('loading-overlay:show', {
                            detail: {
                                message: 'Elias analisando dados demográficos...'
                            }
                        }));
                        fetch('{{ route('admin.reports.family-demographics.elias') }}', {
                            ...
                            .then(data => {
                                ...
                                if (data.success && data.reply) {
                                    this.eliasReply = data.reply.replace(/\*\*(.*?)\*\*/g,
