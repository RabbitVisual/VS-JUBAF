...
<div class="space-y-8">
        ...
        <div class="relative p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="flex-1">
                ...
                <div class="mt-8 flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('pastor.dashboard') }}" class="px-5 py-3 rounded-xl bg-amber-500/20 backdrop-blur-md border border-amber-400/30 text-amber-200 font-bold hover:bg-amber-500/30 transition-colors flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        ...
        <!-- EBD/Classes -->
        <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 relative overflow-hidden group">
            <div class="absolute right-0 top-0 w-32 h-32 bg-orange-50 dark:bg-orange-900/20 rounded-bl-full -mr-8 -mt-8 transition-transform group-hover:scale-110"></div>
            <div class="relative">
                <div class="flex justify-between items-start mb-4">
                ...
                <p class="text-gray-500 dark:text-gray-400 text-sm font-medium uppercase tracking-wider">Ensino / EBD</p>
                <h3 class="text-3xl font-black text-gray-900 dark:text-white mt-1">{{ $stats['ebd_active_classes'] ?? 0 }}</h3>
                <p class="text-sm text-gray-400 mt-2">{{ $stats['ebd_students'] ?? 0 }} alunos matriculados</p>
            </div>
        </div>

        <!-- Gamificação (quando disponível) -->
        @if(isset($gamificationStats) && (($gamificationStats['total_badges'] ?? 0) > 0 || ($gamificationStats['total_levels'] ?? 0) > 0))
        <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 relative overflow-hidden group">
            ...
            <div class="relative">
                <div class="flex justify-between items-start mb-4">
                    ...
                    @if(Route::has('admin.badges.index'))
                        <a href="{{ route('admin.badges.index') }}" class="text-xs font-bold text-amber-600 dark:text-amber-400 hover:underline">Ver</a>
                    @endif
                </div>
                <p class="text-gray-500 dark:text-gray-400 text-sm font-medium uppercase tracking-wider">Gamificação</p>
                <div class="flex items-baseline gap-3 mt-1">
                    <span class="text-2xl font-black text-gray-900 dark:text-white">{{ $gamificationStats['total_badges'] ?? 0 }}</span>
                    <span class="text-sm text-gray-400">badges</span>
                    <span class="text-gray-300 dark:text-gray-600">·</span>
                    <span class="text-2xl font-black text-gray-900 dark:text-white">{{ $gamificationStats['total_levels'] ?? 0 }}</span>
                    <span class="text-sm text-gray-400">níveis</span>
                </div>
                <p class="text-sm text-gray-400 mt-2">{{ $gamificationStats['users_with_badges'] ?? 0 }} usuários com badges</p>
            </div>
        </div>
        @endif
    </div>
