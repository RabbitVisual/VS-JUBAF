...
- **Notifications:** Integre o aceite de parentesco no motor de notificações. Ao aceitar, gere um log no `diretoriaAuditService` (Mudança de composição familiar).
...
flowchart LR
    subgraph backend [Backend]
        ...
        diretoriaAudit[diretoriaAuditService]
    ...
    RelationshipController --> diretoriaAudit
    Show --> CbavBot
...
- **Aceitar/Recusar:** novas rotas e métodos em um controller dedicado (ex.: `UserRelationshipController` no Admin):
    ...
    - Ao aceitar: atualizar `status` para `accepted`; opcionalmente chamar `diretoriaAuditService::log('family_relationship_accepted', $userRelationship, ['user_id' => ..., 'related_user_id' => ...])` (o serviço espera um Model; usar o próprio `UserRelationship` como entidade em [Modules/Diretoria/app/Services/diretoriaAuditService.php](../../../../../../../../Users/Administrator/.cursor/plans/Modules/Diretoria/app/Services/diretoriaAuditService.php)).
...
- **diretoriaAuditService:** ao aceitar o vínculo, chamar `app(diretoriaAuditService::class)->log('family_relationship_accepted', $userRelationship, ['user_id' => $userRelationship->user_id, 'related_user_id' => $userRelationship->related_user_id])` (entidade = `UserRelationship`; o [diretoriaAuditService](../../../../../../../../Users/Administrator/.cursor/plans/Modules/Diretoria/app/Services/diretoriaAuditService.php) grava `entity_type` e `entity_id` do model passado).
