...
<div class="space-y-8" x-data="{ tab: 'general' }">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-4 space-y-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-200 dark:border-gray-700 relative overflow-hidden">
                <div class="absolute right-0 top-0 w-32 h-32 {{ $levelColors['bg'] }} rounded-bl-full -mr-8 -mt-8"></div>
                <div class="relative">
                <h3 class="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-6">Status de Engajamento</h3>

                <div class="flex items-center gap-6 mb-8 group">
                    <div class="relative">
                        <div class="w-16 h-16 {{ $levelColors['bg'] }} rounded-2xl flex items-center justify-center {{ $levelColors['text'] }} group-hover:rotate-12 transition-transform duration-300">
                            <x-icon name="{{ $level['icon'] ?? 'star' }}" class="w-8 h-8" />
                        </div>
                    </div>
                    <div>
                        <div class="text-2xl font-black text-gray-900 dark:text-white tracking-tight">{{ $level['name'] }}</div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="h-3 w-full bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden p-0.5">
                        <div class="h-full bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full transition-all duration-700" style="width: {{ $progress }}%"></div>
                    </div>
                </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-xl border border-gray-200 dark:border-gray-700">
                <h3 class="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-6">Conquistas & Badges</h3>
        ...
        <div class="lg:col-span-8">
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-200 dark:border-gray-700 min-h-[600px] overflow-hidden">
                ...
                <div x-show="tab === 'church'" x-cloak class="p-8 space-y-10 animate-fade-in">
                    <section class="space-y-8">
                        <div class="flex items-center justify-between mb-4">
                            <h4 class="text-xs font-black text-gray-400 uppercase tracking-widest">Jornada Espiritual</h4>
                        </div>

                        <div class="relative pl-8 border-l-2 border-gray-100 dark:border-gray-700 space-y-12">
                            <!-- Baptism -->
                            <div class="relative">
                                <div class="absolute -left-[41px] top-0 w-5 h-5 rounded-full bg-white dark:bg-gray-800 border-4 border-cyan-500 text-cyan-500 flex items-center justify-center"></div>
                                <div class="space-y-1">
                            ...
                            <!-- Membership -->
                            <div class="relative">
                                <div class="absolute -left-[41px] top-0 w-5 h-5 rounded-full bg-white dark:bg-gray-800 border-4 border-blue-600 text-blue-600 flex items-center justify-center"></div>
                                <div class="space-y-1">
                ...
                <div x-show="tab === 'financial'" x-cloak class="p-8 space-y-8 animate-fade-in">
                    @if(auth()->user()->isAdmin() || auth()->user()->isPastor() || \Modules\Treasury\App\Models\TreasuryPermission::where('user_id', auth()->id())->exists())
                ...
                <div x-show="tab === 'family'" x-cloak class="p-8 space-y-6 animate-fade-in" x-data="{ analysisOpen: false, analysisText: '', analysisLoading: false }">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <button type="button" @click="analysisLoading = true; analysisOpen = true; fetch('{{ route('admin.users.family-tree-analysis', $user) }}').then(r => r.json()).then(d => { analysisText = d.analysis || d.message || ''; analysisLoading = false; }).catch(() => { analysisText = 'Não foi possível carregar a análise.'; analysisLoading = false; })"
                    ...
                    <div x-show="analysisOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" x-transition>
                        <div @click.outside="analysisOpen = false" class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col">
                            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                                ...
                                <button type="button" @click="analysisOpen = false" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500">
                            ...
                            <div class="p-6 overflow-y-auto flex-1">
                                ...
                                <template x-if="!analysisLoading && analysisText">
                                    <p class="text-gray-700 dark:text-gray-300 whitespace-pre-wrap" x-text="analysisText"></p>
                                </template>
                                <template x-if="!analysisLoading && !analysisText">
