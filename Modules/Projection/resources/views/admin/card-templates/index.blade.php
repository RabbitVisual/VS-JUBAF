...
@section('title', 'Templates de card | Projeção')
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <nav class="flex items-center gap-2 text-[10px] font-black text-blue-600 dark:text-blue-500 uppercase tracking-widest mb-1">
                    ...
                    <span class="text-gray-400 dark:text-gray-500">Templates de card</span>
                </nav>
                <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Templates de card</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Estilos reutilizáveis para banners e slides na ordem de culto (oração, leitura, avisos).</p>
            </div>
        </div>
        <a href="{{ route('admin.projection.card-templates.create') }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-all">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        ...
                        @if(!empty($t->title_label))
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Título: {{ $t->title_label }}</p>
                        @endif
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        <a href="{{ route('admin.projection.card-templates.edit', $t->id) }}" class="p-2 rounded-xl border border-gray-200 dark:border-gray-600 text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors" title="Editar">
                            <x-icon name="pencil" class="w-5 h-5" />
                        </a>
                        <form action="{{ route('admin.projection.card-templates.destroy', $t->id) }}" method="POST" class="inline" onsubmit="return confirm('Excluir este template? Ele não será mais usado em novos banners.');">
                            @csrf
                            @method('DELETE')
        ...
        @empty
            <div class="col-span-full flex flex-col items-center justify-center py-16 px-6 bg-white dark:bg-gray-800 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                <div class="w-16 h-16 rounded-2xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center mb-5">
                    <x-icon name="image" class="w-8 h-8 text-blue-600 dark:text-blue-400" />
                </div>
                <h3 class="text-xl font-black text-gray-900 dark:text-white mb-1">Nenhum template de card</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 text-center max-w-sm mb-6">Crie templates para estilizar banners na ordem de culto (ex.: Momento de Oração, Leitura da Palavra).</p>
                <a href="{{ route('admin.projection.card-templates.create') }}" class="inline-flex items-center px-5 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-lg shadow-blue-500/20 transition-all">
                    <x-icon name="plus" class="w-5 h-5 mr-2" />
