...
@section('title', 'Editar tema | Projeção')

@section('content')
<div class="space-y-8 max-w-2xl" x-data="themeForm('{{ old('background_type', $theme['background_type'] ?? 'solid') }}')">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Editar tema</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ $theme['name'] ?? '' }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 md:p-8">
        ...
        @endif
        <form action="{{ route('admin.projection.themes.update', $theme['id']) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <select name="background_type" x-model="backgroundType"
                    class="block w-full px-4 py-3 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 dark:text-white">
                    <option value="solid" {{ old('background_type', $theme['background_type'] ?? '') === 'solid' ? 'selected' : '' }}>Cor sólida</option>
                    <option value="gradient" {{ old('background_type', $theme['background_type'] ?? '') === 'gradient' ? 'selected' : '' }}>Gradiente</option>
                    <option value="image" {{ old('background_type', $theme['background_type'] ?? '') === 'image' ? 'selected' : '' }}>Imagem</option>
                    <option value="video" {{ old('background_type', $theme['background_type'] ?? '') === 'video' ? 'selected' : '' }}>Vídeo</option>
                </select>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="background_value" id="theme_bg_value" value="{{ old('background_value', $theme['background_value'] ?? '') }}"
                    class="block w-full px-4 py-3 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 dark:text-white"
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400" x-show="backgroundType === 'solid'" x-transition>Ex.: #0f172a, #1e3a8a</p>
                <p class="text-xs text-gray-500 dark:text-gray-400" x-show="backgroundType === 'gradient'" x-transition>Valor CSS completo. Ex.: <code class="bg-gray-200 dark:bg-gray-700 px-1 rounded">linear-gradient(to bottom, #1e293b, #0f172a)</code></p>
                <p class="text-xs text-gray-500 dark:text-gray-400" x-show="backgroundType === 'image'" x-transition>Use a <strong>URL direta da imagem</strong> (.jpg, .png). No Unsplash: abra a foto → botão direito na imagem → &quot;Copiar endereço da imagem&quot; (não use o link da página).</p>
            </div>
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_default" value="1" id="is_default" {{ old('is_default', $theme['is_default'] ?? false) ? 'checked' : '' }}
