...
@section('title', 'Aprovações Pendentes - Diretoria')
...
@section('content')
    <div
        ...
        <div class="max-w-7xl mx-auto p-6 space-y-6">
            ...
            <!-- Approvals Table -->
            <div
                class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm dark:shadow-lg border border-gray-200 dark:border-slate-800 overflow-hidden relative transition-colors duration-200">
                ...
                <div class="overflow-x-auto relative z-10">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                        <thead class="bg-gray-50 dark:bg-slate-950/50">
                            <tr>
                                <th
                                    class="px-6 py-4 text-left text-xs font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest">
                                    Solicitação</th>
                                <th
                                    class="px-6 py-4 text-left text-xs font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest">
                                    Tipo</th>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div>
                                            ...
                                            <div class="text-sm text-gray-500 dark:text-slate-500 mt-1 leading-snug">
                                                {{ Str::limit($approval->description, 60) }}</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded text-[10px] font-black uppercase tracking-wider border
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div
                                                class="h-6 w-6 rounded-full bg-gray-200 dark:bg-slate-800 border border-gray-300 dark:border-slate-700 flex items-center justify-center text-xs font-bold text-gray-600 dark:text-slate-400 mr-2">
                                                {{ substr($approval->requester->user->name, 0, 1) }}
                                            </div>
                                            <span
                                                class="text-sm font-medium text-gray-700 dark:text-slate-300">{{ $approval->requester->user->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm font-bold text-gray-700 dark:text-slate-300">{{ $approval->submitted_at->format('d/m/Y') }}</span>
                                        <div class="text-xs text-gray-500 dark:text-slate-500 font-medium">
                                            {{ $approval->submitted_at->format('H:i') }}</div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            <div
                                                class="w-16 h-16 bg-gray-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4">
                                                <x-icon name="circle-check"
                                                    class="h-8 w-8 text-gray-400 dark:text-slate-600" />
                                            </div>
                                            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">Nenhuma
                                                aprovação pendente</h3>
                ...
                @if ($approvals->hasPages())
                    <div
                        class="bg-gray-50 dark:bg-slate-900 px-4 py-4 border-t border-gray-200 dark:border-slate-800 sm:px-6">
