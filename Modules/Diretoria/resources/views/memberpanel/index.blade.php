...
@section('content')
    <div
        ...
        <div
            ...
            <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-8">
                ...
                <div class="relative group">
                    <div
                        class="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-white dark:border-blue-500 overflow-hidden shadow-[0_0_20px_rgba(59,130,246,0.3)] bg-white dark:bg-slate-800">
                        @if (auth()->user()->photo)
                            <img class="w-full h-full object-cover" src="{{ asset('storage/' . auth()->user()->photo) }}"
                                alt="{{ auth()->user()->name }}">
                        @else
                            <div
                                class="w-full h-full flex items-center justify-center text-3xl font-black text-blue-600 dark:text-white bg-blue-100 dark:bg-slate-800">
                                {{ substr(auth()->user()->name, 0, 1) }}
                            </div>
                        @endif
                    </div>
                    <div
                        class="absolute bottom-0 right-0 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full border border-white dark:border-slate-900">
                        {{ $member->role_display }}
                    </div>
                </div>
        ...
        <div class="max-w-7xl mx-auto p-6 space-y-8" data-tour="Diretoria-area">
            @if (session('success'))
                <div
            ...
            @endif
            <!-- Stats Grid -->
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" data-tour="Diretoria-menu">
                <!-- Meetings -->
                <div
                    class="relative bg-white dark:bg-slate-900 rounded-xl p-6 border border-gray-200 dark:border-slate-800 hover:border-blue-500/50 transition-all shadow-sm dark:shadow-lg group">
                    <div class="flex items-center justify-between mb-4">
                        <div
                            class="w-12 h-12 bg-blue-50 dark:bg-blue-500/10 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
                            <x-icon name="calendar" class="w-6 h-6 text-blue-600 dark:text-blue-500" />
                        </div>
                        <span
                            class="text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 px-2 py-1 rounded">Agendadas</span>
                    </div>
                    <div class="flex flex-col">
                        <span
                            class="text-3xl font-bold text-gray-900 dark:text-white mb-1">{{ $upcomingMeetings->count() }}</span>
                        <span class="text-sm text-gray-500 dark:text-slate-400">Próximas Reuniões</span>
                    </div>
                    <a href="{{ route('memberpanel.Diretoria.meetings.index') }}" class="absolute inset-0"></a>
                </div>

                <!-- Agendas -->
                <div
                    class="relative bg-white dark:bg-slate-900 rounded-xl p-6 border border-gray-200 dark:border-slate-800 hover:border-emerald-500/50 transition-all shadow-sm dark:shadow-lg group">
                    <div class="flex items-center justify-between mb-4">
                        <div
                            class="w-12 h-12 bg-emerald-50 dark:bg-emerald-500/10 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
                            <x-icon name="file-lines" class="w-6 h-6 text-emerald-600 dark:text-emerald-500" />
                        </div>
                        <span
                            class="text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 rounded">Propostas</span>
                    </div>
                    <div class="flex flex-col">
                        <span
                            class="text-3xl font-bold text-gray-900 dark:text-white mb-1">{{ $myAgendas->count() }}</span>
                        <span class="text-sm text-gray-500 dark:text-slate-400">Minhas Pautas</span>
                    </div>
                    <a href="{{ route('memberpanel.Diretoria.agendas.index') }}" class="absolute inset-0"></a>
                </div>

                <!-- Approvals -->
                <div
                    class="relative bg-white dark:bg-slate-900 rounded-xl p-6 border border-gray-200 dark:border-slate-800 hover:border-amber-500/50 transition-all shadow-sm dark:shadow-lg group">
                    <div class="flex items-center justify-between mb-4">
                        <div
                            class="w-12 h-12 bg-amber-50 dark:bg-amber-500/10 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
                            <x-icon name="clock" class="w-6 h-6 text-amber-600 dark:text-amber-500" />
                        </div>
                        <span
                            class="text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-2 py-1 rounded">Pendentes</span>
                    </div>
                    <div class="flex flex-col">
                        <span
                            class="text-3xl font-bold text-gray-900 dark:text-white mb-1">{{ $pendingApprovals->count() }}</span>
                        <span class="text-sm text-gray-500 dark:text-slate-400">Aprovações</span>
                    </div>
                    <a href="{{ route('memberpanel.Diretoria.approvals.pending') }}" class="absolute inset-0"></a>
                </div>

                <!-- Votes -->
                <div
                    class="bg-white dark:bg-slate-900 rounded-xl p-6 border border-gray-200 dark:border-slate-800 hover:border-purple-500/50 transition-all shadow-sm dark:shadow-lg group cursor-default">
                    <div class="flex items-center justify-between mb-4">
                        <div
                            class="w-12 h-12 bg-purple-50 dark:bg-purple-500/10 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div
                    class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-lg overflow-hidden flex flex-col transition-colors duration-200">
                    <div
                        class="px-6 py-5 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-900/50">
                        ...
                        <a href="{{ route('memberpanel.Diretoria.meetings.index') }}"
                            class="text-xs font-bold text-blue-600 dark:text-blue-500 hover:text-blue-500 dark:hover:text-blue-400 uppercase tracking-widest">Ver
                            Todas</a>
                    </div>
                    <div class="divide-y divide-gray-200 dark:divide-slate-800 flex-1">
                        ...
                        @empty
                            <div class="p-10 text-center">
                                ...
                                <p class="text-sm font-medium text-gray-500 dark:text-slate-400">Nenhuma reunião agendada
                                </p>
                            </div>
                        @endforelse
                    </div>
                ...
                <div
                    ...
                    <div
                        ...
                        <a href="{{ route('memberpanel.Diretoria.agendas.index') }}"
                            class="text-xs font-bold text-emerald-600 dark:text-emerald-500 hover:text-emerald-500 dark:hover:text-emerald-400 uppercase tracking-widest">Ver
                            Todas</a>
                    </div>
                    <div class="divide-y divide-gray-200 dark:divide-slate-800 flex-1">
                            ...
                            <div class="px-6 py-4 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                                <div class="flex items-start gap-4">
                                    <div class="flex-shrink-0 mt-1">
                                        <div
                                            class="w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p
                                            class="text-sm font-bold text-gray-900 dark:text-white truncate group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                                            {{ $agenda->title }}</p>
                                        <div class="flex flex-col gap-1 mt-1">
                                            @if ($agenda->meeting)
                                                <span
                                                    class="text-xs font-medium text-gray-500 dark:text-slate-400 truncate">Reunião:
                                                    {{ $agenda->meeting->title }}</span>
                                            @endif
                                            <span
                                                class="text-[10px] uppercase tracking-wider text-gray-400 dark:text-slate-500">{{ $agenda->created_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        @empty
                            <div class="p-10 text-center">
                                ...
                                <p class="text-sm font-medium text-gray-500 dark:text-slate-400">Nenhuma atividade recente
                                </p>
                            </div>
                        @endforelse
                    </div>
            ...
            <!-- Quick Actions -->
            <div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4 pl-2 border-l-4 border-blue-500">Acesso
                    Rápido</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <a href="{{ route('memberpanel.Diretoria.agendas.create') }}"
                        ...
                        <div
                            class="w-12 h-12 rounded-lg bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                            <x-icon name="plus" class="w-6 h-6 text-blue-600 dark:text-blue-500" />
                        </div>
                        <div>
                            <span
                                class="block text-sm font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">Criar
                                Pauta</span>
                    ...
                    <a href="{{ route('memberpanel.Diretoria.approvals.index') }}"
                        ...
                        <div
                            class="w-12 h-12 rounded-lg bg-green-50 dark:bg-green-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                            <x-icon name="circle-check" class="w-6 h-6 text-green-600 dark:text-green-500" />
                        </div>
                        <div>
                            <span
                                class="block text-sm font-bold text-gray-900 dark:text-white group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">Aprovações</span>
                    ...
                    <a href="{{ route('memberpanel.Diretoria.meetings.index') }}"
                        ...
                        <div
                            class="w-12 h-12 rounded-lg bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                            <x-icon name="calendar" class="w-6 h-6 text-amber-600 dark:text-amber-500" />
                        </div>
                        <div>
                            <span
                                class="block text-sm font-bold text-gray-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">Reuniões</span>
