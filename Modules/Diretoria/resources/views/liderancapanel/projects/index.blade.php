...
@section('title', __('Diretoria::messages.projects'))
        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-1">
                    ...
                    <span class="text-white font-bold">{{ __('Diretoria::messages.projects') }}</span>
                </nav>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <x-icon name="diagram-project" class="w-7 h-7 text-amber-500" />
                    {{ __('Diretoria::messages.projects') }}
                </h1>
                <p class="text-gray-500 dark:text-gray-400 mt-1">{{ __('Diretoria::messages.project') }}</p>
            </div>
            <a href="{{ route('lideranca.conselho.index') }}"
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                                    {{ $project->proposer->name ?? __('Diretoria::messages.no_info') }}
                                    @if ($project->ministry)
