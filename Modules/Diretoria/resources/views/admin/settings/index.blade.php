...
@section('content')
    <div class="space-y-8">
        ...
        <div
            ...
            <div class="relative p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="flex-shrink-0">
                    <button type="submit" form="Diretoria-settings-form"
        ...
        <form id="Diretoria-settings-form" method="POST" action="{{ route('admin.Diretoria.settings.update') }}"
            class="space-y-6"
            onsubmit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Salvando...' } }))">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="diretoria_name"
                            class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                            Nome da Diretoria
                        </label>
                        <input type="text" name="diretoria_name" id="diretoria_name"
                            value="{{ old('diretoria_name', $settings['diretoria_name'] ?? 'Diretoria') }}"
                        ...
                        <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Exibido em títulos, PDFs de atas e
                            convocações, painel do membro e sidebar. Ex.: &quot;Diretoria&quot; ou nome completo da sua
                            entidade.</p>
                        @error('diretoria_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>

                    <div>
                        <label for="meeting_frequency"
                            class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                        ...
                        <select name="meeting_frequency" id="meeting_frequency"
                            class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                            <option value="weekly"
                                {{ ($settings['meeting_frequency'] ?? 'monthly') === 'weekly' ? 'selected' : '' }}>Semanal
                            </option>
                            <option value="biweekly"
                                {{ ($settings['meeting_frequency'] ?? 'monthly') === 'biweekly' ? 'selected' : '' }}>
                                Quinzenal</option>
                            <option value="monthly"
                                {{ ($settings['meeting_frequency'] ?? 'monthly') === 'monthly' ? 'selected' : '' }}>Mensal
                            </option>
                            <option value="quarterly"
                                {{ ($settings['meeting_frequency'] ?? 'monthly') === 'quarterly' ? 'selected' : '' }}>
                                Trimestral</option>
                        </select>
                        <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Usado em editais de convocação (PDF) e
                            referência de periodicidade das reuniões.</p>
                    </div>

                    <div>
                        <label for="quorum_percentage"
                            class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                            Quorum Mínimo (%)
                        </label>
                        <div class="relative">
                            <input type="number" name="quorum_percentage" id="quorum_percentage" min="1"
                                max="100" value="{{ old('quorum_percentage', $settings['quorum_percentage'] ?? 50) }}"
                        ...
                        <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Percentual mínimo de membros presentes
                            para reunião ter quorum. Usado em relatórios e validações de reunião.</p>
                    </div>

                    <div>
                        <label for="voting_deadline_days"
                            class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                            Prazo para Votação (dias)
                        </label>
                        <input type="number" name="voting_deadline_days" id="voting_deadline_days" min="1"
                            max="30"
                            value="{{ old('voting_deadline_days', $settings['voting_deadline_days'] ?? 7) }}"
                            class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                        <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Prazo padrão em dias para encerramento de
                            votações em pautas (1 a 30 dias).</p>
                    </div>
                </div>
            </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                <label for="auto_approve_budget_limit"
                    class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="auto_approve_budget_limit" id="auto_approve_budget_limit" min="0"
                        step="0.01"
                ...
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Despesas acima deste valor na Tesouraria geram
                    solicitação de aprovação da diretoria. Abaixo do limite, a despesa segue sem exigir aprovação formal.
                </p>
            </div>
            ...
            <div>
                ...
                <input type="number" name="approval_deadline_days" id="approval_deadline_days" min="1"
                    ...
                    class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm">
                <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Prazo máximo em dias para a diretoria responder
                    a
        ...
        <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">Selecione quais tipos de aprovação a diretoria
                utiliza.
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                @foreach ($approvalTypes as $key => $label)
                    <label
                        class="relative flex items-start p-3 rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-colors">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
        ...
        <div class="space-y-4">
            <div
                class="flex items-start p-4 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center h-5">
            ...
            <div
                ...
                <div class="ml-3">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Enviar lembretes automáticos 24h antes de
                        reuniões agendadas.</p>
                </div>
            </div>
            ...
            <div
                ...
                <div class="ml-3">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Notificar membros quando houver votações
                        pendentes ou prestes a expirar.</p>
                </div>
            </div>
