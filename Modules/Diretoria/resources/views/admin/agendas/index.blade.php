...
@section('content')
    <div class="space-y-8">
        ...
        <!-- Filters -->
        <div
            class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 relative overflow-hidden group">
            <div
            ...
            <div class="relative">
                <form method="GET" action="{{ route('admin.Diretoria.agendas.index', $meeting) }}"
                    ...
                    <div class="md:col-span-3">
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="postponed" {{ request('status') === 'postponed' ? 'selected' : '' }}>Adiada
                            </option>
                        </select>
                    </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                        @if ($agenda->meeting)
                                            <div class="font-medium text-blue-600 dark:text-blue-400">
                                                {{ Str::limit($agenda->meeting->title, 30) }}</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                                {{ $agenda->meeting->scheduled_date->format('d/m/Y') }}
                                            </div>
                                        @else
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400">
                                                Não associada
                                            </span>
                                        @endif
                                    </td>
                                ...
                                @else bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 @endif">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                        <div class="flex items-center">
                                            @if ($agenda->presenter && $agenda->presenter->user && $agenda->presenter->user->photo)
                                                <img src="{{ asset('storage/' . $agenda->presenter->user->photo) }}"
                                                    alt=""
                                                    class="h-6 w-6 rounded-full mr-2 ring-2 ring-white dark:ring-gray-700 object-cover">
                                            @else
                                                <div
                                                    class="h-6 w-6 rounded-full bg-linear-to-br from-blue-500 to-indigo-600 flex items-center justify-center mr-2 ring-2 ring-white dark:ring-gray-700">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-2">
                                            @if ($agenda->meeting)
                                                <a href="{{ route('admin.Diretoria.meetings.show', $agenda->meeting) }}"
                                            ...
                                            <a href="{{ route('admin.Diretoria.agendas.edit', ['meeting' => $agenda->meeting->id, 'agenda' => $agenda->id]) }}"
                                            ...
                                            @if ($agenda->status === 'discussed' && $agenda->meeting && $agenda->meeting->status === 'in_progress')
                                                <form method="POST"
                                                    action="{{ route('admin.Diretoria.agendas.decision', ['meeting' => $agenda->meeting->id, 'agenda' => $agenda->id]) }}"
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Nenhuma pauta
                                                encontrada</h3>
