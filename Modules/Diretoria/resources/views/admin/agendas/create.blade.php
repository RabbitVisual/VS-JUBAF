...
@section('content')
                                ...
                                <select name="presented_by" id="presented_by"
                                    ...
                                    @foreach ($members as $member)
                                        <option value="{{ $member->id }}"
                                            {{ old('presented_by') == $member->id ? 'selected' : '' }}>
                            ...
                            <label
                                ...
                                <span>
                                    ...
                                    <span class="block text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                        Use esta opção para pautas cujo desfecho final depende de votação em assembleia
                                        (ex.: orçamento anual, eleição de oficiais, mudanças estatutárias).
                                    </span>
                                </span>
                            </label>
                        </div>
    ...
    <script>
        document.getElementById('createAgendaForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route('admin.Diretoria.agendas.store', $meeting) }}', {
                    method: 'POST',
                    headers: {
