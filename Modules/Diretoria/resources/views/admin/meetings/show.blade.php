    ...
    <div class="space-y-8">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div class="flex-1 min-w-0">
                <div class="flex flex-col md:flex-row md:items-center gap-4">
                    <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight leading-tight">
                        {{ $meeting->title }}</h1>
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold shadow-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div
                    ...
                    <div class="space-y-6">
                        @if ($meeting->description)
                            <div
                                class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
                                <h4 class="text-sm font-bold text-gray-900 dark:text-white mb-2">Pauta Inicial / Descrição
                                </h4>
                        ...
                        @if ($meeting->objectives)
                            <div
                                class="bg-blue-50 dark:bg-blue-900/10 rounded-xl p-4 border border-blue-100 dark:border-blue-800/30">
                                <h4 class="text-sm font-bold text-blue-900 dark:text-blue-100 mb-2">Objetivos Principais
                                </h4>
                                <p class="text-sm text-blue-800 dark:text-blue-200 leading-relaxed">
                                    {{ $meeting->objectives }}</p>
                            </div>
                        @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    @else
                                        <div
                                            ...
                                            <span
                                                class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ substr($meeting->president->user->name ?? 'P', 0, 1) }}</span>
                                        </div>
                                    @endif
                                    <div>
                            ...
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400">
                                ...
                                <div>
                                    ...
                                    @else
                                        <p class="text-sm text-gray-500 dark:text-gray-400">Agendada para
                                            {{ $meeting->scheduled_date->format('H:i') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        @if ($meeting->minutes)
                            <div class="mt-4">
                                ...
                                <div
                                    class="prose prose-sm max-w-none text-gray-700 dark:prose-invert dark:text-gray-200 bg-gray-50 dark:bg-gray-800/60 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
                        ...
                        @php
                            $currentMinutesVersion =
                                $meeting->minutesVersions
                                    ->sortByDesc('version')
                                    ->firstWhere('state', 'assembly_approved') ??
                                ($meeting->minutesVersions
                                    ->sortByDesc('version')
                                    ->firstWhere('state', 'diretoria_approved') ??
                        ...
                        @if ($meeting->minutesVersions->count() > 0)
                            <div class="mt-6 border-t border-dashed border-gray-200 dark:border-gray-700 pt-4 space-y-4">
                                    ...
                                    <div class="space-y-2">
                                        @foreach ($meeting->minutesVersions as $version)
                                            <div
                                                ...
                                                <span
                                                    class="text-[11px] font-semibold {{ $version->state === 'assembly_approved' ? 'text-green-500' : ($version->state === 'diretoria_approved' ? 'text-blue-500' : 'text-gray-500') }}">
                                ...
                                @if ($currentMinutesVersion)
                                    ...
                                    @endphp
                                    <div
                                        class="mt-2 bg-white dark:bg-gray-900/40 border border-dashed border-emerald-300/70 dark:border-emerald-500/60 rounded-xl p-4">
                                        <div class="flex items-center justify-between gap-3 mb-2">
                                            <div class="flex items-center gap-2">
                                                <div
                                                    class="w-7 h-7 rounded-lg bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center text-emerald-700 dark:text-emerald-300">
                                                    <x-icon name="badge-check" class="w-4 h-4" />
                                                </div>
                                                <div>
                                                    <p
                                                        class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-widest">
                                                        Visto digital na ata
                                                    </p>
                                                    <p class="text-[11px] text-gray-500 dark:text-gray-400">
                                                        Versão atual: v{{ $currentMinutesVersion->version }}
                                                        ({{ ucfirst(str_replace('_', ' ', $currentMinutesVersion->state)) }})
                                                    </p>
                                                </div>
                                            </div>
                                            @if ($canSignMinutes && !$alreadySigned)
                                                <button type="button" id="btn-sign-minutes"
                                                    data-sign-url="{{ route('admin.Diretoria.meetings.minutes-signatures.store', [$meeting, $currentMinutesVersion]) }}"
                ...
                <div
                                    ...
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="flex-1">
                                            <div class="flex items-center gap-3 mb-2">
                                                <span
                                                    class="flex-shrink-0 w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-gray-400">
                                                    {{ $loop->iteration }}
                                                </span>
                                            ...
                                            @if ($agenda->description)
                                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-3 pl-9 line-clamp-2">
                                                    {{ $agenda->description }}</p>
                                            @endif

                                            <div
                                                class="pl-9 flex items-center gap-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                    ...
                    @else
                        <div class="p-8 text-center">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Adicione pautas para organizar a
                                discussão desta reunião.</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="space-y-8">
                ...
                <div
                    ...
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center justify-between">
                            ...
                            <span
                                class="px-2 py-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs rounded-lg">{{ $meeting->participant_members->count() }}</span>
                        </h3>
                    </div>
                    <div class="max-h-[400px] overflow-y-auto">
                        @forelse ($meeting->participant_members as $participant)
                            <div
                                class="px-6 py-3 border-b border-gray-50 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <div class="flex items-center gap-3">
                                    <div class="flex-shrink-0">
                                        ...
                                        @else
                                            <div
                                                class="h-10 w-10 rounded-xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-sm font-bold text-gray-500 dark:text-gray-400">
                ...
                <!-- Quick Stats -->
                <div
                    class="bg-linear-to-br from-blue-600 to-indigo-700 rounded-3xl shadow-lg p-6 text-white relative overflow-hidden">
                    ...
                    <div class="space-y-4 relative z-10">
                        ...
                        <div class="grid grid-cols-3 gap-2 text-center">
                            <div class="bg-green-500/20 rounded-xl p-2 backdrop-blur-sm border border-green-400/20">
                                <span
                                    class="block text-lg font-bold text-green-300">{{ $meeting->agendas->where('status', 'approved')->count() }}</span>
                            ...
                            <div class="bg-red-500/20 rounded-xl p-2 backdrop-blur-sm border border-red-400/20">
                                <span
                                    class="block text-lg font-bold text-red-300">{{ $meeting->agendas->where('status', 'rejected')->count() }}</span>
                            ...
                            <div class="bg-yellow-500/20 rounded-xl p-2 backdrop-blur-sm border border-yellow-400/20">
                                <span
                                    class="block text-lg font-bold text-yellow-300">{{ $meeting->agendas->where('status', 'pending')->count() }}</span>
