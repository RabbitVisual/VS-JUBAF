...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
            <form method="GET" action="{{ route('admin.Diretoria.approvals.index') }}"
                ...
                <div class="md:col-span-3">
                    ...
                    <select name="type" id="type"
                        ...
                        <option value="ministry_membership"
                            {{ request('type') === 'ministry_membership' ? 'selected' : '' }}>Filiação a Ministério</option>
                        <option value="ministry_plan" {{ request('type') === 'ministry_plan' ? 'selected' : '' }}>Plano de
                            Ministério</option>
                        <option value="event_creation" {{ request('type') === 'event_creation' ? 'selected' : '' }}>Criação
                            de Evento</option>
                        <option value="financial_request" {{ request('type') === 'financial_request' ? 'selected' : '' }}>
                            Solicitação Financeira</option>
                        <option value="membership_transfer_out"
                            {{ request('type') === 'membership_transfer_out' ? 'selected' : '' }}>Carta de Transferência
                        </option>
                        <option value="ebd_curriculum" {{ request('type') === 'ebd_curriculum' ? 'selected' : '' }}>
                            Currículo EBD</option>
                        <option value="other" {{ request('type') === 'other' ? 'selected' : '' }}>Outro</option>
                    </select>
                </div>

                <div class="md:col-span-3">
                    <label for="date_from" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Data
                        Inicial</label>
                    <div class="relative">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div
                                    ...
                                    @if ($approval->requester)
                                        <span class="flex items-center">
                                            ...
                                            Solicitante: <span
                                                class="font-medium ml-1 text-gray-700 dark:text-gray-300">{{ $approval->requester->name }}</span>
                                        </span>
                                    @endif
                                </div>

                                @if (
                                    $approval->approval_type === 'ministry_membership' &&
                                        !empty($approval->metadata['ministry_id']) &&
                                        class_exists(\Modules\Ministries\App\Models\Ministry::class))
                                    ...
                                    <p class="mt-1 text-xs font-semibold text-purple-600 dark:text-purple-400">
                                        Ministério:
                                        {{ $ministryObj ? $ministryObj->name : 'ID ' . $approval->metadata['ministry_id'] }}
                                    </p>
                                @endif
                        ...
                        <div class="flex flex-col md:items-end gap-3 flex-shrink-0">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('admin.Diretoria.approvals.show', $approval) }}"
            ...
            @empty
                <div
                    ...
                    <p class="mt-2 text-gray-500 dark:text-gray-400 max-w-md mx-auto">
                        ...
                        @else
                            Não há solicitações pendentes no momento. As solicitações aparecerão aqui quando enviadas pelos
                            membros.
                        @endif
                    </p>
                    @if (request()->hasAny(['status', 'type', 'date_from']))
