        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div
                    ...
                    <div
                        class="prose dark:prose-invert max-w-none text-gray-600 dark:text-gray-300 leading-relaxed bg-gray-50 dark:bg-gray-700/30 rounded-xl p-6 border border-gray-100 dark:border-gray-700/50">
                        ...
                        <div class="mt-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach ($approval->metadata as $key => $value)
                                    <div
                                        class="bg-white dark:bg-gray-700/50 p-4 rounded-xl border border-gray-100 dark:border-gray-600">
                                        <span
                                            class="block text-xs font-bold text-gray-400 uppercase mb-1">{{ str_replace('_', ' ', $key) }}</span>
                ...
                <div
                    class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 opacity-60">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                            <div
                                class="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400">
                                <x-icon name="chat-alt" class="w-5 h-5" />
                            </div>
                            Histórico e Observações
                        </h3>
                        <span class="text-xs font-medium bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded text-gray-500">Em
                            Breve</span>
                    </div>
                    <p class="text-sm text-gray-500 dark:text-gray-400">O histórico de aprovações e comentários será exibido
                        aqui.</p>
                </div>
            </div>
            ...
            <div class="space-y-8">
                    ...
                    <div class="p-6 flex items-center gap-4">
                        <div class="flex-shrink-0">
                            @if ($approval->requester->photo)
                                <img src="{{ asset('storage/' . $approval->requester->photo) }}"
                                    class="w-16 h-16 rounded-3xl object-cover ring-4 ring-gray-50 dark:ring-gray-700">
                            @else
                                <div
                                    class="w-16 h-16 rounded-3xl bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-2xl ring-4 ring-blue-50 dark:ring-blue-900/20">
                                    {{ substr($approval->requester->name, 0, 1) }}
                                </div>
                            @endif
                        </div>
                        <div>
                            <h4 class="text-lg font-bold text-gray-900 dark:text-white">{{ $approval->requester->name }}
                            </h4>
                ...
                <!-- Approval Info -->
                <div
                    class="bg-linear-to-br from-indigo-600 to-purple-700 rounded-3xl shadow-lg p-6 text-white relative overflow-hidden">
                    ...
                    <div class="space-y-4 relative z-10">
                        ...
                        @if ($approval->expires_at)
                            <div class="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
                                ...
                                <span
                                    class="block text-lg font-bold text-yellow-300">{{ $approval->expires_at->format('d/m/Y') }}</span>
                                <span
                                    class="block text-sm text-indigo-200">{{ $approval->expires_at->diffForHumans() }}</span>
                            </div>
                        @endif
                        ...
                        @if ($approval->responded_at)
                            <div class="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
                                ...
                                <span
                                    class="block text-lg font-bold">{{ $approval->responded_at->format('d/m/Y H:i') }}</span>
                                @if ($approval->responder)
                                    <span class="block text-sm text-indigo-200 mt-1">por
                                        {{ $approval->responder->name }}</span>
                                @endif
                            </div>
                        @endif
                    </div>
        ...
        function rejectRequest(id) {
            ...
            if (reason !== null) {
                fetch(`{{ url('admin/conselho/aprovacoes') }}/${id}/rejeitar`, {
                        ...
                        body: JSON.stringify({
                            reason: (reason && reason.trim()) ? reason.trim() : 'Rejeitado pela diretoria'
                        })
                    })
                    .then(response => response.json())
                    .then(data => {
