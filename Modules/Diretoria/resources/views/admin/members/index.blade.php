...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <a href="{{ route('admin.Diretoria.members.create') }}"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
            <form method="GET" action="{{ route('admin.Diretoria.members.index') }}"
                class="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div class="md:col-span-3">
                    <label for="role"
                        class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Cargo</label>
                    <select name="role" id="role"
                        ...
                        <option value="president" {{ request('role') === 'president' ? 'selected' : '' }}>Presidente
                        </option>
                        <option value="vice_president" {{ request('role') === 'vice_president' ? 'selected' : '' }}>
                            Vice-Presidente</option>
                        <option value="secretary" {{ request('role') === 'secretary' ? 'selected' : '' }}>Secretário
                        </option>
                        <option value="treasurer" {{ request('role') === 'treasurer' ? 'selected' : '' }}>Tesoureiro
                        </option>
                        <option value="member" {{ request('role') === 'member' ? 'selected' : '' }}>Membro</option>
                        <option value="lideranca" {{ request('role') === 'lideranca' ? 'selected' : '' }}>lideranca</option>
                    </select>
                </div>
        ...
        <div
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        <div class="flex-shrink-0 h-12 w-12">
                                            ...
                                            @else
                                                <div
                                                    class="h-12 w-12 rounded-xl bg-linear-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 flex items-center justify-center ring-2 ring-gray-50 dark:ring-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-sm font-medium text-gray-900 dark:text-white">{{ $member->role_display }}</span>
                                        <span
                                            class="text-xs text-gray-500 dark:text-gray-400">{{ $member->diretoria_position }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                ...
                                {{ $member->is_active ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' }}">
                                        <span
                                            class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $member->is_active ? 'bg-green-500' : 'bg-red-500' }}"></span>
                                        {{ $member->is_active ? 'Ativo' : 'Inativo' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <div class="text-gray-900 dark:text-white font-medium">
                                        {{ $member->term_start->format('d/m/Y') }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">
                                        @if ($member->term_end)
                                ...
                                <td class="px-6 py-4 text-sm font-medium">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.Diretoria.members.edit', $member) }}"
                                            class="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"
                                            title="Editar">
                                            <x-icon name="pencil" class="w-5 h-5" />
                                        </a>
                                        <form method="POST"
                                            action="{{ route('admin.Diretoria.members.destroy', $member) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div
                                        class="w-20 h-20 bg-gray-50 dark:bg-gray-800/50 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <x-icon name="users" class="w-10 h-10 text-gray-400" />
                                    </div>
                                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">Nenhum membro encontrado
                                    </h3>
            ...
            @if ($members->hasPages())
                <div
                    class="bg-white dark:bg-gray-800 px-6 py-4 border-t border-gray-100 dark:border-gray-700 border-dashed">
