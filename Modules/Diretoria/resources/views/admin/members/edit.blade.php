...
@section('content')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div
                                class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 border border-gray-200 dark:border-gray-600 flex items-center gap-4">
                                <div class="flex-shrink-0">
                                    ...
                                    @else
                                        <div
                                            class="w-12 h-12 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-lg">
                                            {{ substr($member->user->name, 0, 1) }}
                                        </div>
                                    @endif
                                </div>
                                <div>
                                    <h4 class="text-base font-bold text-gray-900 dark:text-white">{{ $member->user->name }}
                                    </h4>
                        ...
                        <div>
                            ...
                            <input type="text" name="diretoria_position" id="diretoria_position" required
                                value="{{ old('diretoria_position', $member->diretoria_position) }}"
                            ...
                            <div class="relative">
                                <select name="diretoria_role" id="diretoria_role" required
                                    ...
                                    <option value="president"
                                        {{ $member->diretoria_role === 'president' ? 'selected' : '' }}>
                                        Presidente</option>
                                    <option value="vice_president"
                                        {{ $member->diretoria_role === 'vice_president' ? 'selected' : '' }}>Vice-Presidente
                                    </option>
                                    <option value="secretary"
                                        {{ $member->diretoria_role === 'secretary' ? 'selected' : '' }}>
                                        Secretário</option>
                                    <option value="treasurer"
                                        {{ $member->diretoria_role === 'treasurer' ? 'selected' : '' }}>
                                        Tesoureiro</option>
                                    <option value="lideranca"
                                        {{ $member->diretoria_role === 'lideranca' ? 'selected' : '' }}>lideranca</option>
                                    <option value="deacon" {{ $member->diretoria_role === 'deacon' ? 'selected' : '' }}>
                                        Diácono</option>
                                    <option value="member" {{ $member->diretoria_role === 'member' ? 'selected' : '' }}>
                                        Membro</option>
                                </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="term_end" id="term_end"
                                ...
                                class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                            <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Deixe em branco se ainda estiver
                                ativo.</p>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            <label
                                class="flex items-center p-4 border border-gray-200 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-colors">
                                ...
                                <div class="ml-3">
                                    ...
                                    <span class="block text-xs text-gray-500 dark:text-gray-400">Desmarcar esta opção
                                        ocultará o membro das listas principais.</span>
                                </div>
                            </label>
                        </div>
                    </div>
    ...
    <script>
        document.getElementById('editMemberForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route('admin.Diretoria.members.update', $member) }}', {
                    method: 'PUT',
                    headers: {
