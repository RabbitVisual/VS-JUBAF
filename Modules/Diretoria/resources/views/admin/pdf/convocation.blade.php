...
<body>
    @php
        ...
        $diretoriaName = $settings['diretoria_name'] ?? 'Diretoria';
    ...
    <h1>{{ $churchName }}</h1>
    <h2>{{ $diretoriaName }}<br><span style="font-size: 11px;">Edital de Convocação</span></h2>

    <div class="content">
        <p>
            Pelo presente edital, ficam convocados todos os membros do
            <strong>{{ $diretoriaName }}</strong>
        ...
        @if ($meeting->agendas->count() > 0)
            <p>
                @foreach ($meeting->agendas as $index => $agenda)
                    {{ $index + 1 }}. {{ $agenda->title }}@if ($index + 1 < $meeting->agendas->count())
                        ;
                    @endif
                @endforeach
            </p>
        ...
        <p>
            Outros assuntos pertinentes poderão ser incluídos na pauta, a critério da presidência, observando-se o
            quórum
    ...
    <p class="muted" style="margin-top: 18px; text-align: left;">
        Gerado em {{ now()->format('d/m/Y H:i') }} pelo sistema VertexCBAV &mdash; Módulo Diretoria.
    </p>
</body>
