    ...
    <div class="p-6 space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.Diretoria.planning.index') }}"
        ...
        @if (count($types) > 0)
            <form method="GET" class="flex flex-wrap gap-4 items-end">
                <div>
                    <label for="type"
                        class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Tipo</label>
                    <select name="type" id="type"
                        class="rounded-lg border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                ...
                <button type="submit"
                    class="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 text-sm">Filtrar</button>
            </form>
        @endif

        <div class="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mt-2">
            <span class="inline-flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-green-500"></span> Relatório
                entregue</span>
            <span class="inline-flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-amber-400"></span> Pendente /
                em tolerância</span>
            <span class="inline-flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-rose-500"></span> Sem
                relatório no mês</span>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mt-4">
            @foreach ($ministries as $ministry)
                @php
                    ...
                    $planLabel = $plan
                        ? ($plan->status === 'under_diretoria_review'
                            ? 'Em revisão'
                ...
                @endphp
                <div
                    class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-200 dark:border-gray-700 p-5 hover:shadow-lg transition-shadow">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <span
                            class="inline-flex px-2 py-1 rounded-lg text-xs font-medium {{ $planClass }}">{{ $planLabel }}</span>
                        @if ($plan && $plan->status === 'under_diretoria_review' && $plan->diretoria_approval_id)
                            <a href="{{ url('/admin/conselho/aprovacoes/' . $plan->diretoria_approval_id) }}"
                                class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline">Aprovar
                                plano</a>
                        @endif
                    </div>
                </div>
