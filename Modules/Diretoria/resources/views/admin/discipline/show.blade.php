...
@section('content')
    <div class="space-y-6 max-w-4xl">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <a href="{{ route('admin.Diretoria.discipline.index') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="space-y-4">
                <div
                    ...
                    @if ($case->status === \Modules\Diretoria\App\Models\DisciplineCase::STATUS_OPENED) bg-yellow-50 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300
                    @elseif($case->status === \Modules\Diretoria\App\Models\DisciplineCase::STATUS_UNDER_CARE)
                        bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300
                    @elseif($case->status === \Modules\Diretoria\App\Models\DisciplineCase::STATUS_RECOMMENDED_TO_ASSEMBLY)
                        bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300
                    @elseif($case->status === \Modules\Diretoria\App\Models\DisciplineCase::STATUS_DECIDED_BY_ASSEMBLY)
                        ...
                        <ul class="space-y-2 text-xs text-gray-700 dark:text-gray-200">
                            @foreach ($case->files as $file)
                                <li class="flex items-center justify-between gap-2">
                                    ...
                                    <a href="{{ route('admin.Diretoria.discipline.files.download', [$case, $file]) }}"
        ...
        document.getElementById('disciplineActionForm').addEventListener('submit', async function(e) {
                    ...
                    try {
                        const response = await fetch(
                            '{{ route('admin.Diretoria.discipline.store-action', $case) }}', {
                                method: 'POST',
                                headers: {
                ...
                document.getElementById('disciplineFileForm').addEventListener('submit', async function(e) {
                        ...
                        try {
                            const response = await fetch(
                                '{{ route('admin.Diretoria.discipline.files.store', $case) }}', {
                                    method: 'POST',
                                    headers: {
                                        'Accept': 'application/json',
                                        'X-CSRF-TOKEN': document.querySelector(\'meta[name="csrf-token"]\').content,
