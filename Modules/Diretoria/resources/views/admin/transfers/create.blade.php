...
@section('content')
    <div class="space-y-6 max-w-3xl">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Registrar carta de transferência
                </h1>
    ...
    <script>
        document.getElementById('createTransferLetterForm').addEventListener('submit', async function(e) {
                    ...
                    try {
                        const response = await fetch('{{ route('admin.Diretoria.transfers.store') }}', {
                                method: 'POST',
                                headers: {
                            ...
                            if (result.success) {
                                window.location.href = result.redirect ||
                                    '{{ route('admin.Diretoria.transfers.index') }}';
                            } else {
