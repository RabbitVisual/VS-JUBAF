...
@section('content')
    <div class="space-y-6 max-w-3xl">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 dark:text-gray-400 mt-1">
                    @if ($letter->direction === \Modules\Diretoria\App\Models\TransferLetter::DIRECTION_OUTGOING)
            ...
            <a href="{{ route('admin.Diretoria.transfers.index') }}"
        ...
        <div
            class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-4">
            <div class="flex items-center gap-3">
                <span
                    class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide
                @if ($letter->status === \Modules\Diretoria\App\Models\TransferLetter::STATUS_PENDING_diretoria) bg-yellow-50 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300
                @elseif($letter->status === \Modules\Diretoria\App\Models\TransferLetter::STATUS_PENDING_ASSEMBLY)
                    bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300
                @elseif($letter->status === \Modules\Diretoria\App\Models\TransferLetter::STATUS_SENT)
                    bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300
                @elseif($letter->status === \Modules\Diretoria\App\Models\TransferLetter::STATUS_ACKNOWLEDGED)
            ...
            @if ($letter->file_path)
                <div class="flex items-center justify-between mt-2">
                    ...
                    <a href="{{ route('admin.Diretoria.transfers.download', $letter) }}"
    ...
    <script>
        ...
        if (uploadForm) {
            uploadForm.addEventListener('submit', async function(e) {
                    ...
                    try {
                        const response = await fetch('{{ route('admin.Diretoria.transfers.upload', $letter) }}', {
                                method: 'POST',
                                headers: {
