...
@section('content')
    <div class="space-y-8">
            ...
            <div class="relative p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <div class="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <span class="px-3 py-1 rounded-full bg-green-500/20 border border-green-400/30 text-green-300 text-xs font-bold uppercase tracking-wider">Nova Categoria</span>
                    </div>
                    <h1 class="text-3xl md:text-4xl font-black tracking-tight mb-2">Nova Categoria</h1>
                    <p class="text-gray-300 max-w-xl">Crie uma categoria para classificar os bens (ex: Móveis, Eletrônicos).</p>
                </div>
        ...
        <form action="{{ route('assets.admin.categories.store') }}" method="POST" class="space-y-8" onsubmit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Salvando categoria...' } }))">
            @csrf
