...
@section('content')
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white shadow-xl border border-gray-700/50">
            ...
            <div class="relative p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 class="text-3xl md:text-4xl font-black tracking-tight mb-2">Editar Categoria</h1>
                    <p class="text-gray-300 max-w-xl">{{ $category->name }}</p>
                </div>
                <a href="{{ route('assets.admin.categories.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 text-white font-bold hover:bg-white/20 transition-colors">
        ...
        <form action="{{ route('assets.admin.categories.update', $category->id) }}" method="POST" class="space-y-8" onsubmit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Atualizando categoria...' } }))">
            @csrf
            @method('PUT')
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 md:p-8 relative overflow-hidden">
                ...
                <div class="relative flex items-start gap-4 mb-6">
                    <div class="w-12 h-12 rounded-2xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400">
                        <x-icon name="tags" class="w-6 h-6" />
                    </div>
                    <div>
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white">Dados da Categoria</h2>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Altere nome e descrição.</p>
                    </div>
                </div>
                <div class="relative space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" class="w-full rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">{{ old('description', $category->description) }}</textarea>
                    </div>
                    <div class="flex justify-end gap-3 pt-4">
                        <button type="button" onclick="if(confirm('Tem certeza que deseja excluir esta categoria?')) document.getElementById('delete-form').submit();" class="px-6 py-3 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-900/50 font-bold rounded-xl transition-colors">
                            Excluir
                        </button>
        ...
        <form id="delete-form" action="{{ route('assets.admin.categories.destroy', $category->id) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
