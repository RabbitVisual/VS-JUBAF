...
@section('title', 'Meus Desafios Bíblicos')
...
@section('content')
    <div class="max-w-7xl mx-auto space-y-12">
        ...
        <div class="relative rounded-3xl overflow-hidden shadow-2xl">
            <div class="absolute inset-0 bg-linear-to-br from-amber-900 via-orange-900 to-slate-900 opacity-95 z-10"></div>
            ...
            <div class="relative z-20 px-8 py-12 md:px-12 md:py-16 flex flex-col md:flex-row items-center justify-between gap-10">
                <div class="max-w-2xl text-center md:text-left">
                    <span class="inline-block py-1 px-3 rounded-full bg-amber-500/20 border border-amber-400/30 text-amber-200 text-xs font-semibold uppercase tracking-wider mb-4">
                        Discipulado Digital
                    </span>
                    <h1 class="text-4xl md:text-5xl font-black text-white leading-tight mb-4 tracking-tight">
                        Seus Desafios <br>
                        <span class="text-transparent bg-clip-text bg-linear-to-r from-amber-300 to-orange-300">Pelas Escrituras</span>
                    </h1>
                ...
                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 shadow-xl w-full md:w-auto min-w-[300px] text-center md:text-left">
                    <div class="flex items-center gap-4 mb-4">
                        <div class="p-3 bg-amber-500/30 rounded-xl text-white">
                            <x-icon name="plus" class="w-6 h-6" />
                        </div>
                        <div>
                            <h3 class="font-bold text-white">Aceitar Novo Desafio</h3>
                            <p class="text-xs text-amber-200">Explore novos temas e livros</p>
                        </div>
                    </div>
                    <a href="{{ route('member.bible.plans.catalog') }}" class="block w-full py-3 bg-white text-amber-900 rounded-xl font-bold text-center hover:bg-amber-50 transition-colors shadow-lg">
                        Explorar Desafios
                    </a>
                </div>
            </div>
        ...
        <div>
            <div class="flex items-center justify-between mb-8">
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                    <span class="w-1.5 h-8 bg-amber-600 rounded-full"></span>
                    Desafios em Andamento
                </h2>
            </div>

            @if($subscriptions->isEmpty())
                <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border-2 border-dashed border-gray-200 dark:border-gray-700 p-12 text-center group hover:border-amber-500/50 transition-colors">
                    <div class="w-20 h-20 bg-amber-50 dark:bg-amber-900/20 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300">
                        <x-icon name="book-open" class="w-10 h-10 text-amber-400" />
                    </div>
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Sua estante está vazia</h3>
                    <p class="text-gray-500 dark:text-gray-400 mb-8 max-w-md mx-auto">
                        Ainda não há desafios ativos. Escolha um desafio bíblico para começar a acompanhar seu progresso diário.
                    </p>
                    <a href="{{ route('member.bible.plans.catalog') }}" class="inline-flex items-center px-6 py-3 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 hover:-translate-y-1">
                        Ver Catálogo de Desafios
                    </a>
                </div>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($subscriptions as $sub)
                        <div class="group bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-700 flex flex-col h-full relative">
                            <!-- Card Header / Cover -->
                            <div class="h-32 bg-linear-to-r from-amber-500 to-orange-600 relative overflow-hidden">
                            ...
                            <!-- Progress Stripe (dynamic - Flowbite style) -->
                            <div class="w-full bg-gray-200 h-2.5 dark:bg-gray-700" role="progressbar" aria-valuenow="{{ $sub->percent }}" aria-valuemin="0" aria-valuemax="100">
                                <div class="bg-amber-500 h-2.5 rounded-r-full transition-all duration-500" style="width: {{ $sub->percent }}%"></div>
                            </div>
                            ...
                            <div class="p-6 flex-1 flex flex-col justify-between">
                                <div>
                                    ...
                                    <p class="text-xl font-black text-gray-900 dark:text-white mb-4">
                                        Dia <span class="text-amber-600 dark:text-amber-400">{{ $sub->current_day_number }}</span> de <span class="text-gray-600 dark:text-gray-300">{{ $sub->total_days ?? $sub->plan->duration_days }}</span>
                                    </p>
                                            ...
                                            <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" x-transition>
                                                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6 border border-gray-200 dark:border-gray-700" @click.outside="open = false">
                                                    ...
                                                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
                                                        A leitura restante será redistribuída até a data final original do desafio, evitando acúmulo de atrasos. Você continuará de onde parou, com os dias seguintes rebalanceados.
                                                    </p>
