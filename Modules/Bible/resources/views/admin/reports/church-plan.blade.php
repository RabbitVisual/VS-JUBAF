    ...
    <div class="p-6 space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="space-y-1">
                ...
                <p class="text-gray-600 dark:text-gray-400">Visão liderancaal dos membros inscritos no plano oficial e
                    engajamento na leitura</p>
            </div>
        ...
        @if ($churchPlans->isEmpty())
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
                ...
                <p class="text-gray-600 dark:text-gray-400">Nenhum plano marcado como &quot;Plano da Igreja&quot;. Defina um
                    plano como oficial em <a href="{{ route('admin.bible.plans.index') }}"
                        class="text-blue-600 dark:text-blue-400 hover:underline">Planos de Leitura</a>.</p>
            </div>
        @else
            ...
            <div
                ...
                <h2
                    class="text-lg font-semibold text-gray-900 dark:text-white px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex flex-col">
                                            <span
                                                class="text-sm font-medium text-gray-900 dark:text-white">{{ $row->user->name ?? '—' }}</span>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">
                                        {{ $row->plan->title ?? '—' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">
                                        {{ $row->subscription->start_date?->format('d/m/Y') ?? '—' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">
                                        {{ $row->days_read }} / {{ $row->expected_days }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $row->delay }}
                                        {{ $row->delay === 1 ? 'dia' : 'dias' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($row->status === 'em_dia')
                                            <span
                                                class="px-2.5 py-0.5 inline-flex text-xs font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">Em
                                                dia</span>
                                        @elseif($row->status === 'atraso')
                                            <span
                                                class="px-2.5 py-0.5 inline-flex text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300">Em
                                                atraso</span>
                                        @else
                                            <span
                                                class="px-2.5 py-0.5 inline-flex text-xs font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300">Crítico</span>
                                        @endif
                                    </td>
