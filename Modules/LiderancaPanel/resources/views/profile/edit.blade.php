    ...
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-linear-to-br from-gray-900 via-gray-800 to-gray-900 text-white shadow-xl border border-gray-700/50">
            ...
            <div class="relative p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="shrink-0">
                    <a href="{{ route('lideranca.profile.show') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 text-white font-bold hover:bg-white/20 transition-colors">
        ...
        <div class="rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-4 flex items-start gap-3">
            <x-icon name="information-circle" class="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
        ...
        <!-- Form -->
        <form action="{{ route('lideranca.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8" data-address-form onsubmit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Salvando perfil...' } }))">
            @csrf
            @method('PUT')
