...
```markdown
# PROJETO: Criação do Módulo liderancapanel (O Gabinete do Pastor) - VertexCBAV

# OBJETIVO: Separar a gestão técnica (Admin) da gestão ministerial (Pastor), criando um painel focado em pastoreio.

Atue como Arquiteto de Software e Especialista em Gestão Eclesiástica. Quero criar o `Modules\liderancapanel`, um ambiente exclusivo para o Pastor e sua equipe ministerial.
...
- Garanta que o Pastor tenha permissão de "Super Admin" no banco de dados, mas a interface do `liderancapanel` deve ocultar:
