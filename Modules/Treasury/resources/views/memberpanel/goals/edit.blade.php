...
@section('page-title', 'Tesouraria - Editar Meta')
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-200 pb-12">
        <div class="max-w-4xl mx-auto space-y-8 px-6 pt-8">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Editar meta</h1>
                    <p class="text-gray-500 dark:text-slate-400 mt-1 max-w-md">{{ $goal->name }}</p>
                </div>
        ...
        <form action="{{ route('memberpanel.treasury.goals.update', $goal) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-8 md:p-12 space-y-12">
                ...
                <div class="space-y-8">
                    <div class="flex items-center gap-4">
                        ...
                        <h3 class="text-lg font-black text-slate-900 dark:text-white uppercase tracking-widest">Identificação do Alvo</h3>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-8">
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-900 dark:text-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-medium placeholder:text-slate-400">{{ old('description', $goal->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm font-bold text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div class="grid grid-cols-5 gap-3">
                                @foreach(['bullseye-arrow', 'flag-checkered', 'sack-dollar', 'hand-holding-seedling', 'building-columns', 'church', 'heart', 'users-medical', 'microphone', 'book-bible'] as $icon)
                                    <label class="relative cursor-pointer group">
                                        <input type="radio" name="icon_select" value="{{ $icon }}" class="peer sr-only" {{ old('icon', $goal->icon ?? 'bullseye-arrow') === $icon ? 'checked' : '' }}>
                            ...
                            <div class="mt-4">
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="text" id="custom_icon"
                                        ...
                                        value="{{ !in_array($goal->icon, ['bullseye-arrow', 'flag-checkered', 'sack-dollar', 'hand-holding-seedling', 'building-columns', 'church', 'heart', 'users-medical', 'microphone', 'book-bible']) ? $goal->icon : '' }}"
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-4 gap-3">
                                @foreach(['indigo' => 'bg-indigo-500', 'purple' => 'bg-purple-500', 'emerald' => 'bg-emerald-500', 'rose' => 'bg-rose-500', 'amber' => 'bg-amber-500', 'blue' => 'bg-blue-500', 'slate' => 'bg-slate-600', 'pink' => 'bg-pink-500'] as $color => $class)
                                    <label class="relative cursor-pointer group">
                                        <input type="radio" name="color" value="{{ $color }}" class="peer sr-only" {{ old('color', $goal->color ?? 'indigo') === $color ? 'checked' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-2">
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="monthly" {{ old('type', $goal->type) === 'monthly' ? 'selected' : '' }}>Mensal (Recorrente)</option>
                                <option value="yearly" {{ old('type', $goal->type) === 'yearly' ? 'selected' : '' }}>Anual (Estratégica)</option>
                                <option value="campaign" {{ old('type', $goal->type) === 'campaign' ? 'selected' : '' }}>Campanha (Específica)</option>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="category" id="category"
                                ...
                                <option value="total_income" {{ old('category', $goal->category) === 'total_income' ? 'selected' : '' }}>Receita Bruta Total</option>
                                <option value="campaign" {{ old('category', $goal->category) === 'campaign' ? 'selected' : '' }}>Entradas de Campanhas</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="campaign_id" id="campaign_id"
                            ...
                            @foreach ($campaigns as $campaign)
                                <option value="{{ $campaign->id }}" {{ old('campaign_id', $goal->campaign_id) == $campaign->id ? 'selected' : '' }}>
                                    {{ $campaign->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-8 bg-slate-50 dark:bg-slate-800/30 rounded-[2.5rem] p-8 md:p-10 border border-slate-100 dark:border-slate-800/50">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="date" name="start_date" id="start_date" required value="{{ old('start_date', $goal->start_date->format('Y-m-d')) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="date" name="end_date" id="end_date" required value="{{ old('end_date', $goal->end_date->format('Y-m-d')) }}"
                                class="w-full px-5 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-900 dark:text-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold">
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 bg-slate-50 dark:bg-slate-800/40 rounded-3xl border border-slate-100 dark:border-slate-800">
                     <label class="relative inline-flex items-center cursor-pointer group">
                        ...
                        <span class="ml-4 text-sm font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest group-hover:text-emerald-500 transition-colors">Manter Meta Ativa</span>
                    </label>
                </div>
            </div>

            <!-- Hidden Icon Input -->
            <input type="hidden" name="icon" id="final_icon" value="{{ old('icon', $goal->icon) }}">
