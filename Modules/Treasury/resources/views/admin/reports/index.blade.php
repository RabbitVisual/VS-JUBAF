...
@section('content')
    <div class="space-y-8">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <span
                                class="px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-300 text-xs font-bold uppercase tracking-wider">Relatórios</span>
                            <span
                                class="px-3 py-1 rounded-full bg-green-500/20 border border-green-400/30 text-green-300 text-xs font-bold uppercase tracking-wider">{{ $daysDiff }}
                                {{ $daysDiff == 1 ? 'dia' : 'dias' }}</span>
                        </div>
        ...
        @if (isset($monthlyClosing) && $monthlyClosing)
            <div
                class="mt-4 bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-amber-300/70 dark:border-amber-500/60 px-5 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-700 dark:text-gray-300">
                        ...
                        @if ($monthlyClosing->ready_for_assembly)
                            ...
                            @if ($monthlyClosing->diretoria_approved_at)
                                em {{ $monthlyClosing->diretoria_approved_at->format('d/m/Y H:i') }}
                            @endif
                        @else
                ...
                @if ($candiretoriaApprove && !$monthlyClosing->ready_for_assembly)
                    <div class="flex items-center gap-2">
                        <button type="button" id="btn-approve-closing"
        ...
        <div
            ...
            <div
                class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-700/30 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-2">
                    <div class="w-2 h-2 rounded-full bg-blue-600 animate-pulse"></div>
                    <span class="text-xs font-black text-gray-900 dark:text-white uppercase tracking-widest">Parâmetros de
                        Análise</span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
                    <div class="md:col-span-3 space-y-2">
                        <label for="start_date"
                            class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Início</label>
                    ...
                    <div class="md:col-span-3 space-y-2">
                        <label for="end_date"
                            class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Fim</label>
                    ...
                    <div class="md:col-span-3 space-y-2">
                        <label for="category"
                            class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Categoria
                            Principal</label>
        ...
        @if (isset($planoCooperativo))
            ...
            <div class="bg-indigo-600 dark:bg-indigo-800 rounded-3xl shadow-xl border border-indigo-500/30 p-6 text-white">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <span class="text-xs font-black uppercase tracking-widest opacity-90">Plano Cooperativo (Contribuição
                        Denominacional)</span>
                </div>
                <p class="text-3xl font-black">
                    Sugestão de Repasse para Convenção ({{ number_format($planoCooperativo['percent'], 1) }}%): R$
                    {{ number_format($planoCooperativo['suggested_amount'], 2, ',', '.') }}
                </p>
                <p class="text-sm opacity-90 mt-1">Base do período: R$
                    {{ number_format($planoCooperativo['base_amount'], 2, ',', '.') }}</p>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <!-- Total Income -->
            <div
                class="relative group bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-300 hover:shadow-lg">
                ...
                <div class="relative z-10 flex flex-col h-full justify-between">
                    <div>
                        <div class="flex items-center gap-2 mb-4">
                            <div
                                class="p-2 rounded-lg bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400">
                        ...
                        <h3 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">
                            <span
                                class="text-sm font-medium text-gray-400 mr-1">R$</span>{{ number_format($totalIncome, 2, ',', '.') }}
                        </h3>
                    </div>
                    <div
                        class="mt-4 pt-4 border-t border-gray-50 dark:border-gray-700/50 flex items-center justify-between text-[10px] font-bold">
                        <span
                            class="text-green-600 dark:text-green-400 px-2 py-0.5 rounded bg-green-50 dark:bg-green-900/20">{{ $totalIncomeEntries }}
                            transações</span>
                        @if ($daysDiff > 0)
            ...
            <div
                ...
                <div class="relative z-10 flex flex-col h-full justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">
                            <span
                                class="text-sm font-medium text-gray-400 mr-1">R$</span>{{ number_format($totalExpense, 2, ',', '.') }}
                        </h3>
                    </div>
                    <div
                        class="mt-4 pt-4 border-t border-gray-50 dark:border-gray-700/50 flex items-center justify-between text-[10px] font-bold">
                        <span
                            class="text-red-600 dark:text-red-400 px-2 py-0.5 rounded bg-red-50 dark:bg-red-900/20">{{ $totalExpenseEntries }}
                            transações</span>
                        @if ($daysDiff > 0)
            ...
            <div
                ...
                <div class="relative z-10 flex flex-col h-full justify-between">
                    <div>
                        <div class="flex items-center gap-2 mb-4">
                            <div
                                class="p-2 rounded-lg {{ $balance >= 0 ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600' }} dark:{{ $balance >= 0 ? 'bg-blue-900/30 text-blue-400' : 'bg-orange-900/30 text-orange-400' }}">
                                <x-icon name="cash" class="w-5 h-5" />
                            </div>
                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Saldo do
                                Período</span>
                        </div>
                        <h3
                            class="text-3xl font-black {{ $balance >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-orange-600 dark:text-orange-400' }} tracking-tight">
                            <span
                                class="text-sm font-medium text-gray-400 mr-1">R$</span>{{ number_format($balance, 2, ',', '.') }}
                        </h3>
                    </div>
                    <div
                        class="mt-4 pt-4 border-t border-gray-50 dark:border-gray-700/50 flex items-center justify-between text-[10px] font-bold">
                        <span
                            class="px-2 py-0.5 rounded {{ $balance >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }} uppercase tracking-widest">{{ $balance >= 0 ? 'Superávit' : 'Déficit' }}</span>
                        @if ($totalIncome > 0)
                            <span class="text-gray-400">Margem:
                                {{ number_format(($balance / $totalIncome) * 100, 1) }}%</span>
                        @endif
                    </div>
                </div>
            </div>
            ...
            @if (isset($planoCooperativo))
                <div
                    class="relative group bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-300 hover:shadow-lg md:col-span-2 lg:col-span-4">
                    <div class="flex items-center gap-2 mb-4">
                        <div
                        ...
                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Plano Cooperativo
                            (Contribuição Denominacional)</span>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                        <div>
                            <span class="text-gray-500 dark:text-gray-400 block text-xs">Percentual configurado</span>
                            <span
                                class="font-bold text-gray-900 dark:text-white">{{ number_format($planoCooperativo['percent'], 1) }}%</span>
                        </div>
                        <div>
                            ...
                            <span class="font-bold text-gray-900 dark:text-white">R$
                                {{ number_format($planoCooperativo['base_amount'], 2, ',', '.') }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500 dark:text-gray-400 block text-xs">Valor sugerido a repassar</span>
                            <span class="font-bold text-indigo-600 dark:text-indigo-400">R$
                                {{ number_format($planoCooperativo['suggested_amount'], 2, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div
                ...
                <div class="relative z-10 flex flex-col h-full justify-between">
                    <div>
                        <div class="flex items-center gap-2 mb-4">
                            <div
                                class="p-2 rounded-lg bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400">
                                <x-icon name="collection" class="w-5 h-5" />
                            </div>
                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Volume de
                                Atividade</span>
                        </div>
                        <h3 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-50 dark:border-gray-700/50">
                        <p class="text-[10px] font-bold text-gray-400 leading-tight">Registros processados no período
                            selecionado.</p>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            @if ($largestIncome)
                <div
                    class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex items-center gap-6">
                    <div
                        class="w-16 h-16 rounded-3xl bg-green-50 dark:bg-green-900/30 flex items-center justify-center text-green-600 dark:text-green-400 border border-green-100 dark:border-green-800/50">
                        <x-icon name="trending-up" class="w-8 h-8" />
                    </div>
                    <div class="flex-1">
                        <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-1">Maior
                            Receita Registrada</span>
                        <h4 class="text-xl font-black text-gray-900 dark:text-white leading-tight mb-1 truncate">
                            {{ $largestIncome->title }}</h4>
                        <div class="flex items-center gap-2">
                            <span class="text-lg font-bold text-green-600 dark:text-green-400">R$
                                {{ number_format($largestIncome->amount, 2, ',', '.') }}</span>
                            <span class="text-xs text-gray-400">•</span>
                            <span
                                class="text-xs text-gray-500 dark:text-gray-400">{{ $largestIncome->entry_date->format('d/m/Y') }}</span>
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if ($largestExpense)
                <div
                    class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex items-center gap-6">
                    <div
                        class="w-16 h-16 rounded-3xl bg-red-50 dark:bg-red-900/30 flex items-center justify-center text-red-600 dark:text-red-400 border border-red-100 dark:border-red-800/50">
                        <x-icon name="trending-down" class="w-8 h-8" />
                    </div>
                    <div class="flex-1">
                        <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-1">Maior
                            Despesa Registrada</span>
                        <h4 class="text-xl font-black text-gray-900 dark:text-white leading-tight mb-1 truncate">
                            {{ $largestExpense->title }}</h4>
                        <div class="flex items-center gap-2">
                            <span class="text-lg font-bold text-red-600 dark:text-red-400">R$
                                {{ number_format($largestExpense->amount, 2, ',', '.') }}</span>
                            <span class="text-xs text-gray-400">•</span>
                            <span
                                class="text-xs text-gray-500 dark:text-gray-400">{{ $largestExpense->entry_date->format('d/m/Y') }}</span>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div
                ...
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs font-black text-gray-900 dark:text-white uppercase tracking-widest">Receitas
                            por Categoria</span>
                    </div>
                </div>
                    ...
                    @forelse ($incomeByCategory as $item)
                        <div class="group">
                            <div class="flex items-center justify-between text-xs mb-2">
                                <span
                                    class="font-bold text-gray-700 dark:text-gray-300 capitalize group-hover:text-blue-600 transition-colors">{{ str_replace('_', ' ', $item->category) }}</span>
                                <div class="flex gap-2">
                                    <span class="font-black text-gray-900 dark:text-white">R$
                                        {{ number_format($item->total, 2, ',', '.') }}</span>
                                    <span
                                        class="px-1.5 py-0.5 rounded bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 font-bold">{{ number_format($item->percentage, 1) }}%</span>
                                </div>
                            </div>
            ...
            <div
                ...
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs font-black text-gray-900 dark:text-white uppercase tracking-widest">Despesas
                            por Categoria</span>
                    </div>
                </div>
                <div class="p-6 space-y-5">
                            ...
                            <div class="flex items-center justify-between text-xs mb-2">
                                ...
                                <div class="flex gap-2">
                                    <span class="font-black text-gray-900 dark:text-white">R$
                                        {{ number_format($item->total, 2, ',', '.') }}</span>
                                    <span
                                        class="px-1.5 py-0.5 rounded bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 font-bold">{{ number_format($item->percentage, 1) }}%</span>
                                </div>
                            </div>
                            <div class="w-full bg-gray-100 dark:bg-gray-700 h-1.5 rounded-full overflow-hidden">
                                <div class="bg-red-500 h-full rounded-full transition-all duration-700 ease-out"
                                    style="width: {{ $item->percentage }}%"></div>
                            </div>
                        </div>
                    @empty
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div
                class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div
                    class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-700/30">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @forelse ($incomeByPaymentMethod as $item)
                            <div
                                class="p-3 rounded-xl border border-gray-100 dark:border-gray-700 flex items-center justify-between">
                                <span
                                    class="text-xs font-bold text-gray-700 dark:text-gray-300 capitalize">{{ str_replace('_', ' ', $item->payment_method) }}</span>
                                <div class="text-right">
                                    <p class="text-sm font-black text-gray-900 dark:text-white">R$
                        ...
                        @empty
                            <p
                                class="col-span-full text-center py-6 text-xs text-gray-400 font-bold uppercase tracking-widest">
                                Sem movimentações</p>
                        @endforelse
                    </div>
                </div>
        ...
        <div
            ...
            <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/30">
                <h3 class="text-xs font-black text-gray-900 dark:text-white uppercase tracking-[0.2em]">Fluxo de Caixa
                    Diário</h3>
            </div>

            @php
                $allDates = collect($incomeByDay->pluck('date'))
                    ->merge($expenseByDay->pluck('date'))
                    ->unique()
                    ->sortDesc();
            ...
            <div class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse ($allDates as $date)
                    ...
                    @endphp
                    <div
                        class="p-6 flex flex-col md:flex-row md:items-center gap-6 group hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                        <!-- Date Badge -->
                        <div class="flex items-center gap-4 w-48">
                            <div
                                class="w-12 h-12 rounded-xl bg-gray-100 dark:bg-gray-700 flex flex-col items-center justify-center border border-gray-200 dark:border-gray-600">
                                <span
                                    class="text-xs font-black text-blue-600 dark:text-blue-400 -mb-1">{{ $carbonDate->format('d') }}</span>
                                <span
                                    class="text-[8px] font-black text-gray-400 uppercase">{{ $carbonDate->locale('pt_BR')->shortMonthName }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span
                                    class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-tighter">{{ $carbonDate->locale('pt_BR')->dayName }}</span>
                                <span
                                    class="text-[9px] font-black {{ $dayBalance >= 0 ? 'text-green-600' : 'text-red-600' }} uppercase transform scale-90 -translate-x-1 origin-left">
                        ...
                        <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-1">
                                <div class="flex items-center justify-between">
                                    <span
                                        class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Receitas</span>
                                    <span class="text-[10px] font-bold text-green-600">R$
                                        {{ number_format($incomeAmount, 2, ',', '.') }}</span>
                                </div>
                                <div class="w-full bg-gray-100 dark:bg-gray-700 h-1.5 rounded-full overflow-hidden">
                            ...
                            <div class="space-y-1">
                                <div class="flex items-center justify-between">
                                    <span
                                        class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Despesas</span>
                                    <span class="text-[10px] font-bold text-red-600">R$
                                        {{ number_format($expenseAmount, 2, ',', '.') }}</span>
                                </div>
                                <div class="w-full bg-gray-100 dark:bg-gray-700 h-1.5 rounded-full overflow-hidden">
                                    <div class="bg-red-500 h-full rounded-full transition-all duration-700"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            btn.addEventListener('click', async function() {
                if (!confirm(
                        'Confirmar parecer fiscal do conselho e marcar este balancete como pronto para assembleia?'
                        )) {
                    return;
                }
                ...
                window.dispatchEvent(new CustomEvent('loading-overlay:show', {
                    detail: {
                        message: 'Registrando parecer fiscal...'
                    }
                }));
