    ...
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white shadow-xl border border-gray-700/50">
            ...
            <div class="relative p-8 md:p-10 flex flex-col gap-6">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <span class="px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-300 text-xs font-bold uppercase tracking-wider">Campanhas</span>
                        </div>
                        <h1 class="text-3xl md:text-4xl font-black tracking-tight mb-2">Nova campanha</h1>
                        <p class="text-gray-300 max-w-xl">Criação de novas frentes de arrecadação.</p>
                    </div>
                ...
                @include('treasury::admin.partials.nav', ['breadcrumb' => ['Campanhas' => route('treasury.campaigns.index'), 'Nova' => null]])
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden relative max-w-5xl">
            ...
            <form action="{{ route('treasury.campaigns.store') }}" method="POST" enctype="multipart/form-data" class="space-y-0" x-data x-on:submit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Lançando campanha...' } }))">
                @csrf
                ...
                <div class="p-10 md:p-14 space-y-12">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="text" name="name" id="name" required value="{{ old('name') }}"
                                class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-3xl text-sm font-bold text-slate-600 dark:text-slate-300 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600"
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="description" id="description" rows="5"
                            class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-4xl text-sm font-bold text-slate-600 dark:text-slate-300 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                        <div class="space-y-3">
                            ...
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none text-slate-400 group-focus-within:text-blue-500 transition-colors">
                                ...
                                <input type="number" name="target_amount" id="target_amount" step="0.01" min="0" value="{{ old('target_amount') }}"
                                    class="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-3xl text-sm font-black text-slate-900 dark:text-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all"
                                    placeholder="0,00">
                            </div>
                            @error('target_amount')
                                <p class="text-xs font-bold text-red-500 mt-2 ml-1">{{ $message }}</p>
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="date" name="start_date" id="start_date" value="{{ old('start_date') }}"
                                class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-3xl text-sm font-bold text-slate-600 dark:text-slate-300 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all">
                            @error('start_date')
                                <p class="text-xs font-bold text-red-500 mt-2 ml-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="group relative flex justify-center px-8 py-12 border-2 border-slate-200 dark:border-slate-700 border-dashed rounded-[2.5rem] bg-slate-50 dark:bg-slate-800/20 hover:bg-slate-100/50 dark:hover:bg-slate-800/40 hover:border-blue-500 transition-all duration-300">
                            <div class="space-y-4 text-center">
                                <div class="w-16 h-16 bg-white dark:bg-slate-800 rounded-3xl shadow-sm flex items-center justify-center mx-auto text-slate-400 group-hover:scale-110 group-hover:text-blue-500 transition-all duration-300">
                                    <x-icon name="image" style="duotone" class="h-8 w-8" />
                                </div>
                                <div class="flex flex-col text-sm text-slate-600 dark:text-slate-400">
                                    <label for="image-upload" class="relative cursor-pointer font-black text-blue-600 dark:text-blue-400 hover:text-blue-500">
                ...
                <div class="relative px-6 py-6 md:px-10 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-700 flex items-center justify-end">
                    <button type="submit"
                        class="inline-flex items-center px-8 py-3.5 bg-gray-900 dark:bg-blue-600 text-white rounded-xl font-bold hover:bg-gray-800 dark:hover:bg-blue-700 shadow-lg transition-all">
                        <x-icon name="rocket-launch" style="duotone" class="w-5 h-5 mr-2" />
                        Lançar campanha
                    </button>
                </div>
            </form>
