    ...
    <div class="space-y-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden relative">
            ...
            <form action="{{ route('treasury.entries.update', $entry) }}" method="POST" class="p-6 space-y-8" x-data x-on:submit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Atualizando...' } }))">
                ...
                <div x-data="{ type: '{{ old('type', $entry->type) }}' }" class="mb-6 space-y-3 p-5 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                ...
                <div class="space-y-2">
                    ...
                    <select name="igreja_id" id="igreja_id"
                        ...
                        @foreach ($igrejas as $igreja)
                            <option value="{{ $igreja->id }}" {{ old('igreja_id', $entry->igreja_id) == $igreja->id ? 'selected' : '' }}>{{ $igreja->nome }}</option>
                        @endforeach
                    </select>
                    @error('igreja_id')
                        <p class="text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
