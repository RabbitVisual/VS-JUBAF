...
@section('content')
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white shadow-xl border border-gray-700/50">
            ...
            <div class="relative p-8 md:p-10 flex flex-col gap-6">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <span class="px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-300 text-xs font-bold uppercase tracking-wider">Metas</span>
                        </div>
                        <h1 class="text-3xl md:text-4xl font-black tracking-tight mb-2">Nova meta estratégica</h1>
                        <p class="text-gray-300 max-w-xl">Defina objetivos claros para o crescimento financeiro.</p>
                    </div>
                ...
                @include('treasury::admin.partials.nav', ['breadcrumb' => ['Metas' => route('treasury.goals.index'), 'Nova' => null]])
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden relative max-w-5xl">
            ...
            <form action="{{ route('treasury.goals.store') }}" method="POST" class="divide-y divide-slate-100 dark:divide-slate-800" x-data x-on:submit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Criando meta...' } }))">
                @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                            ...
                            <div class="md:col-span-2 space-y-4">
                                ...
                                <textarea name="description" id="description" rows="4"
                                    ...
                                    placeholder="Descreva os objetivos desta meta e o impacto esperado...">{{ old('description') }}</textarea>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-4 gap-3">
                                    @foreach(['indigo' => 'bg-indigo-500', 'purple' => 'bg-purple-500', 'emerald' => 'bg-emerald-500', 'rose' => 'bg-rose-500', 'amber' => 'bg-amber-500', 'blue' => 'bg-blue-500', 'slate' => 'bg-slate-600', 'pink' => 'bg-pink-500'] as $color => $class)
                                        <label class="relative cursor-pointer group">
                                            <input type="radio" name="color" value="{{ $color }}" class="peer sr-only" {{ old('color', 'blue') === $color ? 'checked' : '' }}>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="relative group">
                                    ...
                                    <select name="category" id="category"
                                        ...
                                        <option value="total_income" {{ old('category') === 'total_income' ? 'selected' : '' }}>Receita Total</option>
                        ...
                        <div class="p-10 bg-slate-50 dark:bg-slate-800/30 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 space-y-10">
                            ...
                            <div class="flex items-center gap-6 p-4">
                                ...
                                <div class="space-y-0.5">
                                    <p class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">Ativar Imediatamente</p>
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">A meta começará a monitorar os aportes conforme as regras definidas.</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize final_icon based on old('icon') or default
        if (!finalIcon.value) {
            const checkedRadio = document.querySelector('input[name="icon_select"]:checked');
            if (checkedRadio) {
                finalIcon.value = checkedRadio.value;
            } else if (customIcon.value) {
                finalIcon.value = customIcon.value;
            } else {
                finalIcon.value = 'bullseye-arrow'; // Default if nothing else is set
            }
        }
        ...
        iconRadios.forEach(radio => {
            radio.addEventListener('change', function() {
                if (this.checked) {
                    ...
                    customIcon.value = ''; // Clear custom icon input if a radio is selected
                }
            });
        });
        ...
        customIcon.addEventListener('input', function() {
            if (this.value) {
                finalIcon.value = this.value;
                iconRadios.forEach(r => r.checked = false); // Uncheck all radios if custom icon is typed
            } else {
                // If custom icon is cleared, revert to the default or previously selected radio
                const checkedRadio = document.querySelector('input[name="icon_select"]:checked');
                if (checkedRadio) {
                    finalIcon.value = checkedRadio.value;
                } else {
                    finalIcon.value = 'bullseye-arrow'; // Default if custom is cleared and no radio is checked
                }
            }
        });

        // Handle initial state if custom_icon had a value from old()
        if (customIcon.value && !document.querySelector('input[name="icon_select"]:checked')) {
