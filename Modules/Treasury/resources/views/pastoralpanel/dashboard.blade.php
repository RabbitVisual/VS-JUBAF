...
@section('content')
    <div class="space-y-6">
            ...
            <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-2">
                        <a href="{{ route('pastor.transparencia.index') }}" class="hover:text-white transition-colors">Transparência</a>
                        <x-icon name="chevron-right" class="w-3.5 h-3.5" />
                        <span class="text-white font-bold">Tesouraria</span>
                    </nav>
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 text-xs font-bold uppercase tracking-wider mb-2">Gestão Financeira</span>
                    <h1 class="text-2xl md:text-3xl font-bold tracking-tight mb-1">Tesouraria</h1>
                    <p class="text-slate-300 text-sm max-w-xl">Controle de entradas e saídas, campanhas, metas e relatórios.</p>
                </div>
                <div class="flex flex-shrink-0 flex-wrap items-center gap-3">
                    <a href="{{ route('pastor.tesouraria.reports.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white font-medium hover:bg-white/20 transition-colors">
                    ...
                    @endif
                    <a href="{{ route('pastor.tesouraria.entries.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white font-medium hover:bg-white/20 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Receita do Mês</p>
                    <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white">R$ {{ number_format($monthlyIncome, 2, ',', '.') }}</p>
                </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white">R$ {{ number_format($monthlyExpense, 2, ',', '.') }}</p>
                </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-xl font-bold {{ $monthlyBalance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-xl font-bold {{ $yearlyBalance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
        ...
        <div class="bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-200 dark:border-amber-800 p-5">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h3 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Plano Cooperativo (Convenção)</h3>
            </div>
                        ...
                        <div class="space-y-3">
                            @foreach ($incomeByCategory as $item)
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300 capitalize">{{ str_replace('_', ' ', $item->category) }}</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">R$ {{ number_format($item->total, 2, ',', '.') }}</span>
                                </div>
                            @endforeach
                        </div>
                    @else
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50 flex items-center justify-between">
                <h3 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Últimas Entradas</h3>
                <a href="{{ route('pastor.tesouraria.entries.index') }}" class="text-xs font-bold text-amber-600 dark:text-amber-400 hover:underline">Ver todas</a>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                            @foreach ($recentEntries as $entry)
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                    <td class="px-5 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $entry->entry_date->format('d/m/Y') }}</td>
                                    <td class="px-5 py-3">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $entry->type === 'income' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' }}">
                                            {{ $entry->type === 'income' ? 'Entrada' : 'Saída' }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-3 text-sm font-medium text-gray-900 dark:text-white">{{ $entry->title }}</td>
                                    <td class="px-5 py-3 text-right text-sm font-bold {{ $entry->type === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
        ...
        @if ($activeCampaigns->count() > 0)
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <h3 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Campanhas Ativas</h3>
                    <a href="{{ route('pastor.tesouraria.campaigns.index') }}" class="text-xs font-bold text-amber-600 dark:text-amber-400 hover:underline">Ver todas</a>
                </div>
                    ...
                    @foreach ($activeCampaigns as $campaign)
                        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-5">
                            <div class="flex items-start justify-between mb-3">
                                ...
                                <span class="text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded">{{ number_format($campaign->progress_percentage ?? 0, 1) }}%</span>
                            </div>
                            ...
                            @if ($campaign->target_amount)
                                <div class="w-full bg-gray-100 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                                    <div class="bg-amber-500 h-full rounded-full transition-all" style="width: {{ min(100, $campaign->progress_percentage ?? 0) }}%"></div>
                                </div>
                            @endif
                        </div>
                    @endforeach
