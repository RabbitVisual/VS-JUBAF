...
@section('content')
    <div class="space-y-6">
            ...
            <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-2">
                        ...
                        <a href="{{ route('pastor.tesouraria.entries.index') }}" class="hover:text-white transition-colors">Lançamentos</a>
                        <x-icon name="chevron-right" class="w-3.5 h-3.5" />
                    ...
                    <p class="text-slate-300 text-sm max-w-xl">Registre receita ou despesa. Dados detalhados para prestação de contas.</p>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
            ...
            <div class="px-5 py-3 bg-amber-50 dark:bg-amber-900/20 border-b border-amber-100 dark:border-amber-800/50 flex items-start gap-3">
                <x-icon name="circle-info" class="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                <p class="text-xs text-amber-800 dark:text-amber-200">Doações via PIX/cartão podem ser vinculadas pela transação bancária (valor líquido).</p>
            </div>
            @include('treasury::pastoralpanel.entries._form')
        </div>
    </div>
@endsection
