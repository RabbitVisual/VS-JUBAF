...
@section('content')
    <div class="space-y-6">
            ...
            <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-2">
                        <a href="{{ route('pastor.tesouraria.dashboard') }}" class="hover:text-white transition-colors">Tesouraria</a>
                    ...
                    <p class="text-slate-300 text-sm max-w-xl">Controle detalhado de receitas e despesas. Filtre por tipo, categoria e período.</p>
                </div>
                @if ($permission->canCreateEntries())
                    <a href="{{ route('pastor.tesouraria.entries.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-medium transition-colors">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50 flex items-center gap-2">
            ...
            <div class="p-5">
                <form method="GET" action="{{ route('pastor.tesouraria.entries.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tipo</label>
                        <select name="type" class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 transition-all">
                    ...
                    <div>
                        ...
                        <select name="category" class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 transition-all">
                            ...
                            <option value="utilities" {{ request('category') === 'utilities' ? 'selected' : '' }}>Contas</option>
                    ...
                    <div>
                        ...
                        <select name="fund_id" class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 transition-all">
                            <option value="">Todos</option>
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ request('start_date') }}" class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Até</label>
                        <input type="date" name="end_date" value="{{ request('end_date') }}" class="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 transition-all">
                    </div>
                    <div class="flex items-end">
                        <button type="submit" class="w-full px-4 py-2.5 bg-slate-800 dark:bg-amber-600 text-white text-sm font-bold rounded-xl hover:bg-slate-700 dark:hover:bg-amber-700 transition-all inline-flex items-center justify-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    <thead class="bg-gray-50 dark:bg-slate-700/50">
                        <tr>
                            <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Data</th>
                            <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tipo / Categoria</th>
                            <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Descrição / Registro</th>
                            <th class="px-5 py-4 text-right text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Valor</th>
                            <th class="px-5 py-4 text-right text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Ações</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                        @forelse($entries as $entry)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group">
                                <td class="px-5 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 font-medium">{{ $entry->entry_date->format('d/m/Y') }}</td>
                                <td class="px-5 py-4 whitespace-nowrap">
                                    <div class="flex flex-col gap-1">
                                        <span class="text-xs font-bold {{ $entry->type === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                                            {{ $entry->type === 'income' ? 'Entrada' : 'Saída' }}
                                        </span>
                                        <span class="text-sm font-medium text-gray-900 dark:text-white capitalize">{{ $entry->financialCategory?->name ?? str_replace('_', ' ', $entry->category) }}</span>
                                        @if($entry->type === 'expense' && $entry->expense_status)
                                            @php
                                                ...
                                                $sc = $statusConfig[$entry->expense_status] ?? ['label' => $entry->expense_status, 'class' => 'bg-gray-100 text-gray-800'];
                                            @endphp
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        @if($permission->canCreateEntries() && !$entry->reversal_of_id && !isset($reversedEntryIds[$entry->id]))
                                            <form action="{{ route('pastor.tesouraria.entries.reverse', $entry) }}" method="POST" class="inline" onsubmit="if(confirm('Confirmar estorno? Será criada uma entrada inversa vinculada.')) { window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Estornando...' } })); return true; } return false;">
                                                @csrf
                                        ...
                                        @if($permission->canEditEntries())
                                            <a href="{{ route('pastor.tesouraria.entries.edit', $entry) }}" class="p-2 text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/30 rounded-xl transition-colors" title="Editar">
                                                <x-icon name="pencil" class="w-4 h-4" />
                                            </a>
                                        @endif
                                        @if($permission->canDeleteEntries())
                                            <form action="{{ route('pastor.tesouraria.entries.destroy', $entry) }}" method="POST" class="inline" onsubmit="if(confirm('Excluir esta entrada?')) { window.dispatchEvent(new CustomEvent('loading-overlay:show')); return true; } return false;">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr>
                                <td colspan="5" class="px-5 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        @if ($permission->canCreateEntries())
                                            <a href="{{ route('pastor.tesouraria.entries.create') }}" class="mt-4 inline-flex items-center gap-2 px-5 py-2.5 bg-amber-500 text-white font-bold rounded-xl hover:bg-amber-600 transition-all text-sm">
