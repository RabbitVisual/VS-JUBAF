...
@section('content')
    <div class="space-y-6">
        ...
        <form method="GET" action="{{ route('pastor.tesouraria.reports.index') }}" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-5">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div class="space-y-2">
                    <label for="start_date" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Data inicial</label>
                ...
                <div class="space-y-2">
                    <label for="end_date" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Data final</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
                <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total receitas</p>
                <p class="mt-1 text-2xl font-bold text-green-600 dark:text-green-400 tabular-nums">R$ {{ number_format($totalIncome, 2, ',', '.') }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $totalIncomeEntries }} lançamento(s)</p>
            </div>
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
                <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total despesas</p>
                <p class="mt-1 text-2xl font-bold text-red-600 dark:text-red-400 tabular-nums">R$ {{ number_format($totalExpense, 2, ',', '.') }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $totalExpenseEntries }} lançamento(s)</p>
            </div>
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
                <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Saldo do período</p>
                <p class="mt-1 text-2xl font-bold {{ $balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }} tabular-nums">R$ {{ number_format($balance, 2, ',', '.') }}</p>
