...
@section('content')
    <div class="space-y-6">
            ...
            <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <nav class="flex items-center gap-2 text-sm text-slate-400 font-medium mb-2">
                        ...
                        <a href="{{ route('pastor.tesouraria.campaigns.index') }}" class="hover:text-white transition-colors">Campanhas</a>
                        <x-icon name="chevron-right" class="w-3.5 h-3.5" />
                ...
                <a href="{{ route('pastor.tesouraria.campaigns.show', $campaign) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white font-medium hover:bg-white/20 transition-colors">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
            <form action="{{ route('pastor.tesouraria.campaigns.update', $campaign) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6" x-data x-on:submit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Atualizando...' } }))">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label for="name" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nome <span class="text-red-500">*</span></label>
                    ...
                    <div class="space-y-2">
                        <label for="slug" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Slug (opcional)</label>
                ...
                <div class="space-y-2">
                    <label for="description" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Descrição</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        <label for="target_amount" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Meta (R$)</label>
                        <div class="relative">
                    ...
                    <div class="space-y-2">
                        <label for="start_date" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Início</label>
                    ...
                    <div class="space-y-2">
                        <label for="end_date" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Término</label>
                ...
                <div class="space-y-2">
                    <label for="image" class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Capa (substituir)</label>
                    @if ($campaign->image)
                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Atual: <a href="{{ Storage::url($campaign->image) }}" target="_blank" class="text-amber-600 dark:text-amber-400 hover:underline">ver imagem</a></p>
                    @endif
                ...
                <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-slate-700/30 rounded-xl">
                    ...
                    <label for="is_active" class="text-sm font-medium text-gray-900 dark:text-white">Campanha ativa</label>
                </div>
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-slate-700">
                    <a href="{{ route('pastor.tesouraria.campaigns.show', $campaign) }}" class="px-6 py-2.5 text-sm font-bold text-gray-700 dark:text-gray-200 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 transition-all">Cancelar</a>
