        ...
        <div class="rounded-3xl bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 text-white shadow-xl border border-amber-900/30 overflow-hidden">
            <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full {{ $campaign->isActive() ? 'bg-green-500/20 text-green-300' : 'bg-gray-500/20 text-gray-300' }} text-xs font-bold uppercase tracking-wider mb-2">
                        {{ $campaign->isActive() ? 'Ativa' : 'Encerrada' }}
                    </span>
                    <h1 class="text-2xl md:text-3xl font-bold tracking-tight mb-1">{{ $campaign->name }}</h1>
                    <p class="text-slate-300 text-sm max-w-xl">{{ $campaign->description ? \Str::limit($campaign->description, 120) : 'Campanha ministerial.' }}</p>
                </div>
                <div class="flex flex-shrink-0 flex-wrap items-center gap-3">
                    ...
                    @if (isset($permission) && $permission->canManageCampaigns())
                        <a href="{{ route('pastor.tesouraria.campaigns.edit', $campaign) }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-medium transition-colors">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
                        <div>
                            ...
                            <p class="mt-1 text-xl font-bold text-gray-900 dark:text-white tabular-nums">R$ {{ number_format($campaign->current_amount, 2, ',', '.') }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Meta</p>
                            <p class="mt-1 text-xl font-bold text-amber-600 dark:text-amber-400 tabular-nums">R$ {{ number_format($campaign->target_amount, 2, ',', '.') }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Progresso</p>
                            <p class="mt-1 text-xl font-bold text-green-600 dark:text-green-400 tabular-nums">{{ number_format($campaign->progress_percentage, 1) }}%</p>
                        </div>
                    </div>
                    <div class="w-full bg-gray-100 dark:bg-slate-700 rounded-full h-2.5 overflow-hidden">
                        <div class="bg-amber-500 h-full rounded-full transition-all" style="width: {{ min(100, $campaign->progress_percentage) }}%"></div>
                    </div>
                </div>
                @endif
                                    ...
                                    @foreach ($campaign->financialEntries as $entry)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                                            ...
                                            <td class="px-5 py-3 text-right">
                                                <span class="text-sm font-bold text-green-600 dark:text-green-400">+ R$ {{ number_format($entry->amount, 2, ',', '.') }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
            ...
            <div class="space-y-6">
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-5">
                    <h4 class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4 pb-2 border-b border-gray-200 dark:border-slate-700">Cronograma</h4>
                    <div class="space-y-4">
                        @if ($campaign->start_date)
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-amber-500">
                ...
                @if ($campaign->image)
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden p-4">
                    <img src="{{ Storage::url($campaign->image) }}" alt="{{ $campaign->name }}" class="w-full rounded-xl object-cover aspect-video">
                </div>
                @endif
            </div>
        </div>
