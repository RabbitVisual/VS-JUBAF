...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
            <form action="{{ route('lideranca.tesouraria.goals.store') }}" method="POST" class="p-6 space-y-6" x-data
                x-on:submit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Salvando meta...' } }))">
                @csrf
                <div class="space-y-2">
                    <label for="name"
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name') }}"
                        placeholder="Ex: Meta mensal de dízimos"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="category" id="category"
                            ...
                            <option value="offering" {{ old('category') === 'offering' ? 'selected' : '' }}>Ofertas
                            </option>
                            <option value="donation" {{ old('category') === 'donation' ? 'selected' : '' }}>Doações
                            </option>
                            <option value="campaign" {{ old('category') === 'campaign' ? 'selected' : '' }}>Campanhas
                            </option>
                        </select>
                    </div>
                </div>
                    ...
                    <select name="campaign_id" id="campaign_id"
                        ...
                        @foreach ($campaigns as $campaign)
                            <option value="{{ $campaign->id }}"
                                {{ old('campaign_id') == $campaign->id ? 'selected' : '' }}>{{ $campaign->name }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="target_amount" id="target_amount" step="0.01" min="0.01"
                                required value="{{ old('target_amount') }}"
