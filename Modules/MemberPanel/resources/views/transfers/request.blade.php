...
@section('content')
    <div class="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-200">
        <div class="max-w-3xl mx-auto p-6 space-y-6">
            ...
            <div
                class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 p-6 relative overflow-hidden">
                ...
                <form id="requestTransferForm" class="space-y-6 relative z-10">
                    ...
                    <div class="space-y-4">
                        <p class="text-sm text-gray-600 dark:text-slate-300">
                            <span class="font-semibold">Importante:</span> a carta de transferência é um documento oficial
                            que
                            expressa comunhão entre igrejas batistas. Ela é emitida pela igreja atual e encaminhada à igreja
                            de destino, após avaliação liderancaal e do conselho.
                        </p>
                    ...
                    <div
                        class="flex flex-col md:flex-row justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-800 mt-4">
                        <button type="submit"
                        ...
                        const response = await fetch('{{ route('memberpanel.transfers.store') }}', {
                            ...
                            if (result.success) {
                                window.location.href = result.redirect ||
                                    '{{ route('memberpanel.transfers.index') }}';
                            } else {
