...
<!-- Top Navigation -->
<nav
    class="sticky top-0 z-40 backdrop-blur-xl bg-white/80 dark:bg-gray-900/90 border-b border-gray-200/50 dark:border-gray-700/50 px-6 py-4 shadow-sm transition-all duration-300">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
            @if (($marketplace_store_available ?? false) && Route::has('marketplace.storefront.cart'))
                <a href="{{ route('marketplace.storefront.cart') }}"
                    class="relative p-2.5 rounded-xl text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                    aria-label="Ver carrinho">
                    <x-icon name="cart-shopping" style="duotone" class="w-6 h-6" />
                    @if (($marketplace_cart_count ?? 0) > 0)
                        <span
                            class="absolute -top-0.5 -right-0.5 min-w-[1.25rem] h-5 px-1 flex items-center justify-center text-[10px] font-bold text-white bg-red-500 rounded-full">{{ ($marketplace_cart_count ?? 0) > 99 ? '99+' : $marketplace_cart_count }}</span>
                    @endif
                </a>
            @endif
            ...
            <div class="relative" x-data="{ open: false }" id="notifications-dropdown">
                <button @click="open = !open" type="button" id="notifications-toggle" data-tour="notifications"
                    ...
                    @if ($unreadCount > 0)
                        <span
                            class="absolute top-2 right-2 block h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white dark:ring-gray-900 animate-pulse"></span>
                    @endif
                ...
                <div x-show="open" @click.away="open = false" x-cloak
                    ...
                    <div class="p-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 rounded-t-2xl"
                        data-notifications-all-url="{{ route('memberpanel.notifications.index') }}">
                    ...
                    <div id="notification-list-container" class="max-h-80 overflow-y-auto">
                        ...
                        @empty
                            <div class="px-6 py-12 text-center">
                                <div
                                    class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gray-100 dark:bg-gray-800 mb-3 text-gray-400">
                                    <x-icon name="bell-slash" class="w-6 h-6" />
                                </div>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">Nada por aqui</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Você está em dia com as
                                    notificações!</p>
                            </div>
                        @endforelse
                    </div>
                    <div
                        class="p-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 rounded-b-2xl space-y-2">
                        @if ($unreadCount > 0)
                            <button type="button"
                                onclick="window.NotificationSystem && window.NotificationSystem.markAllAsRead()"
                                class="w-full px-4 py-2 text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-colors">
            ...
            <div class="relative" x-data="{ open: false }" id="user-menu-dropdown">
                <button @click="open = !open" type="button" id="user-menu-toggle"
                    ...
                    <div class="relative">
                        ...
                        <div
                            class="w-9 h-9 rounded-full bg-linear-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs shadow-md border-2 border-white dark:border-gray-800 hidden">
                            {{ $userInitial }}
                        </div>
                    </div>
                    <div class="hidden md:flex flex-col items-start px-1">
                        <span
                            class="text-xs font-bold text-gray-700 dark:text-gray-200 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ Str::limit($user->first_name ?? $user->name, 15) }}</span>
                    </div>
                    <x-icon name="chevron-down"
                        class="w-4 h-4 text-gray-400 dark:text-gray-500 group-hover:text-blue-500 transition-colors" />
                </button>
