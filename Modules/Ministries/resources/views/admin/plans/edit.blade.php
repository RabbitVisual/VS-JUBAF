        ...
        <div class="relative overflow-hidden rounded-3xl bg-linear-to-br from-gray-900 to-gray-800 text-white shadow-xl border border-gray-700/50">
            ...
            <div class="relative p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 class="text-3xl md:text-4xl font-black tracking-tight mb-2">Editar Plano</h1>
                    <p class="text-gray-300 max-w-xl">{{ $ministry->name }} · {{ $plan->title }}</p>
                </div>
                <a href="{{ route('admin.ministries.plans.show', [$ministry, $plan]) }}" class="flex-shrink-0 px-6 py-3 rounded-xl bg-white text-gray-900 font-bold hover:bg-gray-100 transition-all shadow-lg shadow-white/10 inline-flex items-center gap-2">
                    <x-icon name="arrow-left" class="w-5 h-5 text-blue-600" /> Voltar
                </a>
            </div>
        </div>

        <form action="{{ route('admin.ministries.plans.update', [$ministry, $plan]) }}" method="POST" x-data x-on:submit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Salvando...' } }))">
            @csrf
            @method('PUT')
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 relative overflow-hidden group">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $plan->title) }}" required class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white">
                    ...
                    <div>
                        ...
                        <input type="number" name="period_year" id="period_year" value="{{ old('period_year', $plan->period_year) }}" min="2020" max="2030" required class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white">
                    ...
                    <div>
                        ...
                        <select name="period_type" id="period_type" required class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white">
                            <option value="annual" {{ old('period_type', $plan->period_type) === 'annual' ? 'selected' : '' }}>Anual</option>
                            <option value="semiannual" {{ old('period_type', $plan->period_type) === 'semiannual' ? 'selected' : '' }}>Semestral</option>
                            <option value="quarterly" {{ old('period_type', $plan->period_type) === 'quarterly' ? 'selected' : '' }}>Trimestral</option>
                            <option value="monthly" {{ old('period_type', $plan->period_type) === 'monthly' ? 'selected' : '' }}>Mensal</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="period_start" id="period_start" value="{{ old('period_start', $plan->period_start?->format('Y-m-d')) }}" required class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white">
                    ...
                    <div>
                        ...
                        <input type="date" name="period_end" id="period_end" value="{{ old('period_end', $plan->period_end?->format('Y-m-d')) }}" required class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="objectives" id="objectives" rows="4" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white">{{ old('objectives', $plan->objectives) }}</textarea>
                    </div>
                    <div>
                        ...
                        <input type="number" name="budget_requested" id="budget_requested" value="{{ old('budget_requested', $plan->budget_requested) }}" step="0.01" min="0" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="budget_notes" id="budget_notes" rows="2" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-900 dark:text-white">{{ old('budget_notes', $plan->budget_notes) }}</textarea>
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-4">
                    <a href="{{ route('admin.ministries.plans.show', [$ministry, $plan]) }}" class="px-5 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 font-bold">Cancelar</a>
