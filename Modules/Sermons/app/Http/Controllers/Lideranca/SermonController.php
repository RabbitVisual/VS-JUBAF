...
namespace Modules\Sermons\App\Http\Controllers\Lideranca;
...
class SermonController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if ($request->filled('tag_id')) {
            $query->whereHas('tags', fn ($q) => $q->where('sermon_tags.id', $request->tag_id));
        }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($request->hasFile('cover_image_file')) {
            $validated['cover_image'] = $request->file('cover_image_file')->store('sermons/covers', 'public');
        }
        if ($request->hasFile('attachments')) {
            $attachments = [];
            foreach ($request->file('attachments') as $file) {
                $path = $file->store('sermons/attachments', 'public');
                $attachments[] = ['name' => $file->getClientOriginalName(), 'path' => $path, 'size' => $file->getSize(), 'mime' => $file->getMimeType()];
            }
        ...
        if ($sermon->status === 'published' && $sermon->visibility === 'members' && class_exists(NotifyMembersOfNewContent::class)) {
        ...
        if ($request->has('bible_references') && is_array($request->bible_references)) {
            ...
            foreach ($request->bible_references as $index => $ref) {
                if (!empty($ref['book_id']) && $booksMap->has($ref['book_id'])) $ref['book'] = $booksMap[$ref['book_id']];
                if (!empty($ref['chapter_id']) && $chaptersMap->has($ref['chapter_id'])) $ref['chapter'] = $chaptersMap[$ref['chapter_id']];
                if (!empty($ref['book'])) {
    ...
    public function update(Request $request, Sermon $sermon): RedirectResponse
    {
        ...
        if ($request->hasFile('attachments')) {
            ...
            foreach ($request->file('attachments') as $file) {
                $path = $file->store('sermons/attachments', 'public');
                $currentAttachments[] = ['name' => $file->getClientOriginalName(), 'path' => $path, 'size' => $file->getSize(), 'mime' => $file->getMimeType()];
            }
        ...
        if ($request->has('bible_references') && is_array($request->bible_references)) {
            ...
            foreach ($request->bible_references as $index => $ref) {
                ...
                if (!empty($ref['book'])) {
                    $sermon->bibleReferences()->create([
                        'book' => $ref['book'], 'chapter' => $ref['chapter'] ?? null, 'verses' => $ref['verses'] ?? null,
                        'reference_text' => $ref['reference_text'] ?? null, 'bible_version_id' => $ref['bible_version_id'] ?? null,
                        'book_id' => $ref['book_id'] ?? null, 'chapter_id' => $ref['chapter_id'] ?? null,
                        'type' => $ref['type'] ?? 'main', 'context' => $ref['context'] ?? null, 'order' => $index,
                    ]);
                }
            }
        }
    ...
    public function inviteCollaborator(Request $request, Sermon $sermon): RedirectResponse
    {
        ...
        if ($sermon->collaborators()->where('user_id', $user->id)->first()) {
        ...
        if (class_exists(\Modules\Notifications\App\Services\InAppNotificationService::class)) {
            app(\Modules\Notifications\App\Services\InAppNotificationService::class)->sendToUser(
                ...
                ['action_url' => route('memberpanel.sermons.collaborator.invite', $collab), 'action_text' => 'Ver convite', 'notification_type' => 'sermon_collaboration']
            );
        }
    ...
    private function extractTopicsFromHtml(string $html): string
    {
        ...
        if (preg_match_all('/<h[1-6][^>]*>(.*?)<\/h[1-6]>/si', $html, $m)) {
            foreach ($m[1] as $heading) {
                ...
                if ($text !== '') $out[] = '<p style="margin:0.3em 0;"><strong>' . htmlspecialchars($text) . '</strong></p>';
            }
        }
        if (empty($out)) {
