...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('lideranca.sermoes.commentaries.create') }}"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <form method="GET" action="{{ route('lideranca.sermoes.commentaries.index') }}"
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 flex-shrink-0">
                                            @if ($comment->cover_image)
                                                <img class="h-10 w-10 rounded-lg object-cover"
                                                    src="{{ asset('storage/' . $comment->cover_image) }}" alt="">
                                            @else
                                                <div
                                                    class="h-10 w-10 rounded-lg bg-gray-100 dark:bg-slate-700 flex items-center justify-center">
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-bold text-gray-900 dark:text-white">
                                                {{ $comment->reference }}</div>
                                            @if ($comment->is_official)
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100">
                                                    Oficial
                                                </span>
                                            @endif
                                        </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            class="text-sm text-gray-900 dark:text-white font-medium">{{ $comment->user->name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('lideranca.sermoes.commentaries.edit', $comment) }}"
                                            class="text-amber-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Editar</a>
                                        <form action="{{ route('lideranca.sermoes.commentaries.destroy', $comment) }}"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12">
                                    <div class="flex flex-col items-center justify-center text-center max-w-sm mx-auto">
                                        <div
                                            class="w-20 h-20 rounded-full bg-amber-50 dark:bg-amber-900/20 flex items-center justify-center mb-4">
                                            <x-icon name="comments" class="w-10 h-10 text-amber-500" />
                                        </div>
                                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">Nenhum comentário
                                            ainda</h3>
                                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Cadastre comentários
                                            bíblicos para consulta. Crie o primeiro registro.</p>
                                        <a href="{{ route('lideranca.sermoes.commentaries.create') }}"
