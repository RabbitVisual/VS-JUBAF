...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('lideranca.sermoes.commentaries.index') }}"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <form action="{{ route('lideranca.sermoes.commentaries.update', $commentary) }}" method="POST"
                ...
                <div class="bible-reference-item grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="col-span-1">
                        <label for="bible_version_id"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Versão</label>
                        <select name="bible_version_id" id="bible_version_id" required
                            ...
                            @foreach ($bibleVersions as $version)
                                <option value="{{ $version->id }}"
                                    {{ $selectedVersionId == $version->id ? 'selected' : '' }}>{{ $version->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="book_id"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Livro</label>
                    ...
                    <div>
                        <label for="chapter_id"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Capítulo</label>
                    ...
                    <div>
                        ...
                        <input type="text" name="verses" readonly data-selected-verses="{{ $versesString }}"
                            value="{{ $versesString }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="title" class="block text-sm font-bold text-gray-700 dark:text-gray-300">Título
                            (Opcional)</label>
                    ...
                    <div>
                        <label for="cover_image_file"
                            class="block text-sm font-bold text-gray-700 dark:text-gray-300">Imagem de Capa
                            (Opcional)</label>
                        <div class="mt-1 flex items-center space-x-4">
                            <div id="cover-preview"
                                class="w-12 h-12 rounded-lg bg-gray-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden border border-gray-200 dark:border-gray-600 flex-shrink-0">
                                @if ($commentary->cover_image)
                                    <img src="{{ asset('storage/' . $commentary->cover_image) }}"
                            ...
                            <div class="flex-1 space-y-1">
                                ...
                                @if ($commentary->cover_image)
                                    <label
                                        class="flex items-center text-[10px] text-red-600 dark:text-red-400 cursor-pointer">
                                        <input type="checkbox" name="remove_cover" value="1"
                                            class="mr-1 h-2.5 w-2.5 rounded text-red-600">
                                        Remover imagem
                                    </label>
                                @endif
                            </div>
                ...
                <div>
                    <label for="content"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Comentário</label>
                    <textarea name="content" id="content" rows="8" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-700 dark:border-gray-600 dark:text-white sm:text-sm @error('content') border-red-300 @enderror">{{ old('content', $commentary->content) }}</textarea>
                    @error('content')
                ...
                <div class="bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    @if ($commentary->audio_path || $commentary->audio_url)
                        <div id="audio-current-container"
                            ...
                            <div class="flex items-start justify-between mb-2">
                                ...
                                <button type="button"
                                    onclick="if(confirm('Tem certeza que deseja remover o áudio?')) { document.getElementById('remove_audio').value = '1'; document.getElementById('audio-current-container').remove(); }"
                    ...
                    <div x-data="{ audioType: '{{ old('audio_url') ? 'link' : 'upload' }}' }" class="space-y-4">
                        ...
                        <div class="flex gap-2 border-b border-gray-200 dark:border-gray-700">
                            <button type="button" @click="audioType = 'upload'"
                                :class="audioType === 'upload' ? 'border-blue-500 text-blue-600 dark:text-blue-400' :
                            ...
                            <button type="button" @click="audioType = 'link'"
                                :class="audioType === 'link' ? 'border-blue-500 text-blue-600 dark:text-blue-400' :
                                    'border-transparent text-gray-500 dark:text-gray-400'"
                                class="px-4 py-2 border-b-2 font-medium text-sm transition-colors">
                        ...
                        <div x-show="audioType === 'upload'" x-transition>
                            <label for="audio" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Novo Arquivo de Áudio @if ($commentary->audio_path || $commentary->audio_url)
                                    <span class="text-xs text-gray-500">(substituirá o atual)</span>
                                @endif
                            </label>
                        ...
                        <div x-show="audioType === 'link'" x-transition>
                            <label for="audio_url"
                                ...
                                URL do Áudio @if ($commentary->audio_path || $commentary->audio_url)
                                    <span class="text-xs text-gray-500">(substituirá o atual)</span>
                                @endif
                            </label>
                            <input type="url" name="audio_url" id="audio_url"
                                value="{{ old('audio_url', $commentary->audio_url) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-700 dark:border-gray-600 dark:text-white sm:text-sm"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="status" id="status" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-700 dark:border-gray-600 dark:text-white sm:text-sm">
                            <option value="draft" {{ old('status', $commentary->status) == 'draft' ? 'selected' : '' }}>
                                Rascunho</option>
                            <option value="published"
                                {{ old('status', $commentary->status) == 'published' ? 'selected' : '' }}>Publicado
                            </option>
                        </select>
                    </div>
                    <div class="flex items-center mt-6">
                        ...
                        <input type="checkbox" name="is_official" id="is_official" value="1"
                            {{ old('is_official', $commentary->is_official) ? 'checked' : '' }}
                            class="h-4 w-4 text-blue-600 border-gray-300 rounded">
                        <label for="is_official" class="ml-2 block text-sm text-gray-900 dark:text-gray-300">Comentário
                            Oficial</label>
                    </div>
                </div>
