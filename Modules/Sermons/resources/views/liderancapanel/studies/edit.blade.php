...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <form action="{{ route('lideranca.sermoes.studies.update', $study) }}" method="POST" enctype="multipart/form-data"
                class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label for="title"
                            class="block text-sm font-bold text-gray-700 dark:text-gray-300">Título</label>
                        <input type="text" name="title" id="title" value="{{ old('title', $study->title) }}"
                            required
                    ...
                    <div class="md:col-span-2">
                        <label for="subtitle"
                            class="block text-sm font-bold text-gray-700 dark:text-gray-300">Subtítulo</label>
                    ...
                    <div>
                        <label for="series_id"
                            class="block text-sm font-bold text-gray-700 dark:text-gray-300">Série</label>
                        <select name="series_id" id="series_id"
                            class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-700 dark:border-gray-600 dark:text-white sm:text-sm transition-all">
                            <option value="">Selecione uma série (opcional)</option>
                            @foreach ($series as $s)
                                <option value="{{ $s->id }}"
                                    {{ old('series_id', $study->series_id) == $s->id ? 'selected' : '' }}>
                                    {{ $s->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        <label for="category_id"
                            class="block text-sm font-bold text-gray-700 dark:text-gray-300">Categoria</label>
                        <select name="category_id" id="category_id"
                            class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-700 dark:border-gray-600 dark:text-white sm:text-sm transition-all">
                            <option value="">Selecione uma categoria</option>
                            @foreach ($categories as $c)
                                <option value="{{ $c->id }}"
                                    {{ old('category_id', $study->category_id) == $c->id ? 'selected' : '' }}>
                                    {{ $c->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <label for="cover_image_file"
                            class="block text-sm font-bold text-gray-700 dark:text-gray-300">Imagem de Capa</label>
                        <div class="mt-1 flex items-center space-x-4">
                            <div id="cover-preview"
                                class="w-24 h-24 rounded-xl bg-gray-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden border border-gray-200 dark:border-gray-600 flex-shrink-0">
                                @if ($study->cover_image)
                                    <img src="{{ asset('storage/' . $study->cover_image) }}"
                                ...
                                @if ($study->cover_image)
                                    <label class="flex items-center text-xs text-red-600 dark:text-red-400 cursor-pointer">
                                        <input type="checkbox" name="remove_cover" value="1"
                                            class="mr-1 h-3 w-3 rounded text-red-600">
                ...
                <!-- Content -->
                <div>
                    <label for="content" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Conteúdo
                        Completo</label>
                    ...
                    <div x-data="{ audioType: '{{ old('audio_url') ? 'link' : 'upload' }}' }" class="space-y-4">
                        ...
                        <div x-show="audioType === 'upload'" x-transition>
                            <label for="audio_file"
                                ...
                                Novo Arquivo de Áudio @if ($study->audio_file || $study->audio_url)
                        ...
                        <div x-show="audioType === 'link'" x-transition>
                            <label for="audio_url"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                URL do Áudio @if ($study->audio_file || $study->audio_url)
                                    <span class="text-xs text-gray-500">(substituirá o atual)</span>
                                @endif
                            </label>
                            <input type="url" name="audio_url" id="audio_url"
                                value="{{ old('audio_url', $study->audio_url) }}"
                ...
                <!-- Video URL -->
                <div>
                    <label for="video_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300">URL do Vídeo
                        (Opcional)</label>
                    <input type="url" name="video_url" id="video_url"
                        value="{{ old('video_url', $study->video_url) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-700 dark:border-gray-600 dark:text-white sm:text-sm"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="status" id="status" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-700 dark:border-gray-600 dark:text-white sm:text-sm">
                            <option value="draft" {{ old('status', $study->status) == 'draft' ? 'selected' : '' }}>
                                Rascunho</option>
                            <option value="published"
                                {{ old('status', $study->status) == 'published' ? 'selected' : '' }}>Publicado</option>
                            <option value="archived" {{ old('status', $study->status) == 'archived' ? 'selected' : '' }}>
                                Arquivado</option>
                        </select>
                    </div>
                    <div>
                        <label for="visibility"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Visibilidade</label>
                        <select name="visibility" id="visibility" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-700 dark:border-gray-600 dark:text-white sm:text-sm">
                            <option value="public"
                                {{ old('visibility', $study->visibility) == 'public' ? 'selected' : '' }}>Público</option>
                            <option value="members"
                                {{ old('visibility', $study->visibility) == 'members' ? 'selected' : '' }}>Membros</option>
                            <option value="private"
                                {{ old('visibility', $study->visibility) == 'private' ? 'selected' : '' }}>Privado</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <input type="checkbox" name="is_featured" id="is_featured" value="1"
                        {{ old('is_featured', $study->is_featured) ? 'checked' : '' }}
