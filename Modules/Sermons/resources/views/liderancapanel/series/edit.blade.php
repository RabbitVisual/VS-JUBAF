...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <form action="{{ route('lideranca.sermoes.series.update', $series) }}" method="POST" enctype="multipart/form-data"
                class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="draft" {{ old('status', $series->status) == 'draft' ? 'selected' : '' }}>
                                Rascunho</option>
                            <option value="published" {{ old('status', $series->status) == 'published' ? 'selected' : '' }}>
                                Publicado</option>
                            <option value="archived" {{ old('status', $series->status) == 'archived' ? 'selected' : '' }}>
                                Arquivado</option>
                        </select>
                    </div>
                ...
                <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800/30">
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" name="is_featured" id="is_featured" value="1"
                            {{ old('is_featured', $series->is_featured) ? 'checked' : '' }}
