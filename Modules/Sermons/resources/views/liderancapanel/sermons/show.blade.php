        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3" x-data="{ exportModalOpen: false, format: 'full', size: 'a5' }">
                ...
                <div x-show="exportModalOpen" x-cloak
                    ...
                    <div @click.outside="exportModalOpen = false"
                        class="w-full max-w-md rounded-xl bg-white dark:bg-gray-800 shadow-xl border border-gray-200 dark:border-gray-700 p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="flex gap-3">
                                    <label
                                        class="flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border-2 cursor-pointer transition-colors"
                                        :class="format === 'full' ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' :
                                            'border-gray-200 dark:border-gray-600 hover:border-gray-300'">
                                        <input type="radio" name="export_format" value="full" x-model="format"
                                            class="sr-only">
                                        <span class="text-sm font-medium">Esboço completo</span>
                                    </label>
                                    <label
                                        class="flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border-2 cursor-pointer transition-colors"
                                        :class="format === 'topics' ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' :
                                            'border-gray-200 dark:border-gray-600 hover:border-gray-300'">
                                        <input type="radio" name="export_format" value="topics" x-model="format"
                                            class="sr-only">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Tamanho do
                                    papel</label>
                                <div class="flex gap-4">
                                    <label class="flex-1 cursor-pointer" @click="size = 'a4'">
                                        <div class="rounded-lg border-2 p-2 transition-colors text-center"
                                            :class="size === 'a4' ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' :
                                                'border-gray-200 dark:border-gray-600'">
                                            <div class="mx-auto rounded bg-gray-200 dark:bg-gray-600"
                                                style="width: 42px; height: 59px;"></div>
                                            <span
                                                class="block text-xs font-bold mt-1 text-gray-700 dark:text-gray-300">A4</span>
                                        </div>
                                    </label>
                                    <label class="flex-1 cursor-pointer" @click="size = 'a5'">
                                        <div class="rounded-lg border-2 p-2 transition-colors text-center"
                                            :class="size === 'a5' ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' :
                                                'border-gray-200 dark:border-gray-600'">
                                            <div class="mx-auto rounded bg-gray-200 dark:bg-gray-600"
                                                style="width: 30px; height: 42px;"></div>
                                            <span
                                                class="block text-xs font-bold mt-1 text-gray-700 dark:text-gray-300">A5</span>
                                        </div>
                                    </label>
                                </div>
