...
<div class="space-y-6">
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 text-white shadow-xl border border-amber-900/30">
        ...
        <div class="relative p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <a href="{{ route('pastor.sermoes.series.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-medium transition-colors shrink-0">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-4">
        <form method="GET" action="{{ route('pastor.sermoes.series.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex gap-2">
                ...
                @if (request()->hasAny(['search', 'status']))
                    <a href="{{ route('pastor.sermoes.series.index') }}" class="px-4 py-2 rounded-xl bg-slate-200 dark:bg-slate-700 text-gray-800 dark:text-white font-medium hover:bg-slate-300 dark:hover:bg-slate-600">Limpar</a>
                @endif
            </div>
        </form>
    </div>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-medium rounded-full
                                    ...
                                    {{ $item->status === 'archived' ? 'bg-gray-100 text-gray-800 dark:bg-slate-700 dark:text-gray-300' : '' }}">
                                    {{ ucfirst($item->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('pastor.sermoes.series.edit', $item) }}" class="text-amber-600 dark:text-amber-400 hover:underline font-medium">Editar</a>
                                    <form action="{{ route('pastor.sermoes.series.destroy', $item) }}" method="POST" class="inline"
                            ...
                            <td colspan="5" class="px-6 py-12">
                                <div class="flex flex-col items-center justify-center text-center max-w-sm mx-auto">
                                    <div class="w-20 h-20 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center mb-4">
                                        <x-icon name="collection" class="w-10 h-10 text-amber-500 dark:text-amber-400" />
                                    </div>
                                    ...
                                    <a href="{{ route('pastor.sermoes.series.create') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-medium transition-colors">
                                        <x-icon name="plus" class="w-5 h-5" />
                                        Nova série
                                    </a>
                                </div>
