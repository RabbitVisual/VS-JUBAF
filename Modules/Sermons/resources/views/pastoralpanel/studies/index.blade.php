...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        <form method="GET" action="{{ route('pastor.sermoes.studies.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <select name="series_id" class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white">
            ...
            <select name="status" class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white">
            ...
            <div class="flex gap-2">
                <button type="submit" class="px-4 py-2 bg-amber-500 dark:bg-blue-500 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-amber-500">
                    Filtrar
                </button>
                @if (request()->hasAny(['search', 'series_id', 'status']))
                    <a href="{{ route('pastor.sermoes.studies.index') }}" class="px-4 py-2 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600">
                        Limpar
                    </a>
                @endif
            </div>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-medium rounded-full
                                    ...
                                    {{ $study->status === 'archived' ? 'bg-gray-100 text-gray-800 dark:bg-slate-700 dark:text-gray-300' : '' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('pastor.sermoes.studies.destroy', $study) }}" method="POST" class="inline"
