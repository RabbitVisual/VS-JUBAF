...
<div class="checkin-scanner min-h-[calc(100vh-8rem)] flex flex-col gap-4 sm:gap-6" x-data="checkinScanner()" x-init="init()">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-3 sm:p-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 flex-1 min-h-0">
        <div class="lg:col-span-2 flex flex-col gap-4 min-h-0">
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden flex-1 flex flex-col min-h-[280px] sm:min-h-[320px]">
                    ...
                    <div class="absolute inset-0 z-30 flex items-center justify-center p-4 sm:p-6 bg-gray-900/95" x-show="feedback.visible" x-transition style="display: none;">
                        <div class="text-center w-full max-w-sm bg-gray-800 rounded-2xl p-6 shadow-xl border border-gray-700">
                            <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mx-auto mb-4 text-white"
                                ...
                                <x-icon name="circle-check" class="w-8 h-8 sm:w-10 sm:h-10" x-show="feedback.type === 'success'" />
                                <x-icon name="circle-xmark" class="w-8 h-8 sm:w-10 sm:h-10" x-show="feedback.type === 'error'" />
                                <x-icon name="exclamation-circle" class="w-8 h-8 sm:w-10 sm:h-10" x-show="feedback.type === 'warning'" />
                            </div>
        ...
        <div class="flex flex-col gap-4 sm:gap-6 min-h-0">
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-4 sm:p-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-4 sm:p-6 flex-1 flex flex-col min-h-0">
                ...
                <div class="space-y-2 overflow-y-auto flex-1 min-h-0 -mr-1 pr-1">
                    <template x-for="(entry, index) in history" :key="index">
                        <div class="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-slate-600 shrink-0">
...
document.addEventListener('alpine:init', () => {
    Alpine.data('checkinScanner', () => ({
        ...
        async validateTicket(hash) {
            try {
                const response = await fetch("{{ route('pastor.eventos.checkin.validate') }}", {
                    method: 'POST',
                    headers: {
