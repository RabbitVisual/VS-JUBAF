...
@section('title', 'Editar Lição | ' . $lesson->title)
...
<div class="space-y-8 animate-in fade-in duration-500 pb-24">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-gray-100 dark:border-gray-800">
        <div>
            <div class="flex items-center gap-2 mb-2">
                <span class="px-2 py-0.5 text-[10px] font-black uppercase tracking-widest bg-amber-600 text-white rounded">Ajuste Pedagógico</span>
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Escola Bíblica Dominical</span>
            </div>
            <h1 class="text-4xl font-black text-gray-900 dark:text-white tracking-tight">Editar <span class="text-amber-600">Planejamento</span></h1>
            <p class="text-gray-500 dark:text-gray-400 mt-2 max-w-2xl font-medium">Refinando a lição: <span class="text-gray-900 dark:text-white font-bold text-sm uppercase tracking-tight">{{ $lesson->title }}</span></p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.ebd.lessons.show', $lesson) }}"
                class="inline-flex items-center px-4 py-3 rounded-xl bg-gray-950 dark:bg-white text-white dark:text-gray-950 text-sm font-bold transition-all hover:scale-105 active:scale-95 shadow-xl">
                Ver Lição
            </a>
    ...
    <!-- Form -->
    <form action="{{ route('admin.ebd.lessons.update', $lesson) }}" method="POST" class="space-y-8" onsubmit="window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Salvando lição...' } }))">
        @csrf
        @method('PUT')
                    ...
                    <div class="p-8 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="title" value="{{ old('title', $lesson->title) }}" required placeholder="Ex: A Graça de Deus..."
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="date" name="lesson_date" value="{{ old('lesson_date', $lesson->lesson_date->format('Y-m-d')) }}" required
                                    class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-black focus:ring-2 focus:ring-amber-500/20 transition-all">
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Horário</label>
                                <input type="time" name="lesson_time" value="{{ old('lesson_time', \Carbon\Carbon::parse($lesson->lesson_time)->format('H:i')) }}" required
                                    class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-black focus:ring-2 focus:ring-amber-500/20 transition-all">
                            </div>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Breve Descrição</label>
                            <textarea name="description" rows="2" placeholder="Resumo rápido..."
                                class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-amber-500/20 transition-all resize-none">{{ old('description', $lesson->description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-xl overflow-hidden">
                    ...
                    <div class="p-8 space-y-8">
                        <div class="space-y-2">
                            ...
                            <textarea name="objective" rows="2" placeholder="Objetivo..."
                                class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-amber-500/20 transition-all resize-none">{{ old('objective', $lesson->objective) }}</textarea>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Introdução</label>
                            <textarea name="introduction" rows="3" placeholder="Introdução..."
                                class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-amber-500/20 transition-all resize-none">{{ old('introduction', $lesson->introduction) }}</textarea>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Desenvolvimento Teológico</label>
                            <textarea name="development" rows="10" placeholder="Corpo principal..."
                                class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-amber-500/20 transition-all resize-none">{{ old('development', $lesson->development) }}</textarea>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Conclusão</label>
                                <textarea name="conclusion" rows="4" placeholder="Conclusão..."
                                    class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-amber-500/20 transition-all resize-none">{{ old('conclusion', $lesson->conclusion) }}</textarea>
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Aplicação Prática</label>
                                <textarea name="application" rows="4" placeholder="Aplicação..."
                                    class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-amber-500/20 transition-all resize-none">{{ old('application', $lesson->application) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-xl overflow-hidden">
                    ...
                    <div class="p-6">
                        <select name="status" required class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-black focus:ring-2 focus:ring-amber-500/20 transition-all cursor-pointer">
                            <option value="scheduled" {{ old('status', $lesson->status) === 'scheduled' ? 'selected' : '' }}>Agendada</option>
                            <option value="in_progress" {{ old('status', $lesson->status) === 'in_progress' ? 'selected' : '' }}>Em Andamento</option>
                            <option value="completed" {{ old('status', $lesson->status) === 'completed' ? 'selected' : '' }}>Concluída</option>
                            <option value="cancelled" {{ old('status', $lesson->status) === 'cancelled' ? 'selected' : '' }}>Cancelada</option>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="p-6 space-y-5">
                            ...
                            <select name="bible_version" id="bible_version" required class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border-none rounded-xl text-xs font-black focus:ring-2 focus:ring-blue-500/20 transition-all">
                                ...
                                @foreach ($bibleVersions as $version)
                                    <option value="{{ strtolower($version->abbreviation) }}" data-version-id="{{ $version->id }}"
                                        {{ old('bible_version', $lesson->bible_version) === strtolower($version->abbreviation) ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="bible_verses" id="bible_verses" value="{{ old('bible_verses', $lesson->bible_verses) }}" placeholder="1-10"
                        ...
                        <div id="bible_reference_preview" class="hidden mt-4 space-y-3">
                            <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-800/50">
                                <span class="text-[9px] font-black text-blue-400 dark:text-blue-500 uppercase tracking-widest block mb-1" id="current_ref_label">Referência Ativa</span>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch(`{{ route('bible.api.books') }}?version_id=${versionId}`)
                ...
                .then(d => {
                    ...
                    list.forEach(b => {
                        ...
                        if (o.value === '{{ old('bible_book', $lesson->bible_book) }}') o.selected = true;
            ...
            fetch(`{{ route('bible.api.chapters') }}?book_id=${bookId}`)
                ...
                .then(d => {
                    ...
                    list.forEach(c => {
                        ...
                        if (o.value == '{{ old('bible_chapter', $lesson->bible_chapter) }}') o.selected = true;
        ...
        function loadVersesText() {
            ...
            document.getElementById('verses_text_preview').classList.remove('hidden');
