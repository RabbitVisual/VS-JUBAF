...
@section('title', 'Adicionar Questão | ' . $lesson->title)
...
<div class="space-y-8 animate-in fade-in duration-500">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-gray-100 dark:border-gray-800">
        <div>
            ...
            <h1 class="text-4xl font-black text-gray-900 dark:text-white tracking-tight">Nova <span class="text-amber-600">Questão</span></h1>
    ...
    <!-- Form -->
    <form action="{{ route('admin.ebd.lessons.questions.store', $lesson) }}" method="POST" class="space-y-8 max-w-4xl pb-20">
        @csrf
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-xl overflow-hidden">
            ...
            <div class="p-8 space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="question" rows="4" required placeholder="Elabore a pergunta de forma clara..."
                        class="w-full px-6 py-6 bg-gray-50 dark:bg-gray-800/50 border-none rounded-3xl text-sm font-medium focus:ring-2 focus:ring-amber-500/20 transition-all resize-none">{{ old('question') }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="type" id="type" required class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800/50 border-none rounded-2xl text-sm font-black focus:ring-2 focus:ring-amber-500/20 transition-all cursor-pointer">
                            <option value="short_answer" {{ old('type') === 'short_answer' ? 'selected' : '' }}>Resposta Curta (Texto)</option>
                            <option value="essay" {{ old('type') === 'essay' ? 'selected' : '' }}>Dissertativa (Parágrafo)</option>
                            <option value="multiple_choice" {{ old('type') === 'multiple_choice' ? 'selected' : '' }}>Múltipla Escolha</option>
                            <option value="true_false" {{ old('type') === 'true_false' ? 'selected' : '' }}>Verdadeiro ou Falso</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input type="number" name="points" min="1" max="100" value="{{ old('points', 10) }}" required
                    ...
                    <div id="options-list" class="space-y-3">
                        <div class="flex items-center gap-3">
                            <div class="flex-1 relative">
                                <input type="text" name="options[]" placeholder="Opção de resposta..."
