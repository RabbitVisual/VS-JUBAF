    ...
    <div class="space-y-6">
        <div class="rounded-3xl bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 text-white shadow-xl border border-amber-900/30 overflow-hidden">
            <div class="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold tracking-tight mb-1">Avaliações EBD</h1>
                    <p class="text-slate-300 text-sm md:text-base">Avaliações e notas dos alunos.</p>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-4">
            <form method="GET" action="{{ route('pastor.ebd.evaluations.index') }}" class="flex flex-wrap gap-3 items-end">
                <div>
                ...
                <div>
                    ...
                    <select name="status" class="rounded-xl border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white px-4 py-2.5 text-sm focus:ring-2 focus:ring-amber-500">
                        ...
                        <option value="graded" {{ request('status') === 'graded' ? 'selected' : '' }}>Corrigida</option>
                    </select>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    <thead class="bg-gray-50 dark:bg-slate-900/50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Lição / Turma</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Nota</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                        @forelse($evaluations as $e)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                                <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $e->student->user->name ?? '—' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                    {{ $e->lesson->title ?? '—' }}<br>
                                    <span class="text-xs">{{ $e->lesson->ebdClass->name ?? '—' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    @if($e->score !== null)
                                        <span class="font-bold text-amber-600 dark:text-amber-400">{{ $e->score }}</span>
                                    @else
                                        <span class="text-gray-400">—</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium {{ $e->status === 'graded' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300' }}">{{ $e->status === 'graded' ? 'Corrigida' : 'Pendente' }}</span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('pastor.ebd.evaluations.show', $e) }}" class="text-amber-600 dark:text-amber-400 hover:underline font-medium text-sm">Ver</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">Nenhuma avaliação encontrada.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if($evaluations->hasPages())
                <div class="px-6 py-4 border-t border-gray-200 dark:border-slate-700">{{ $evaluations->appends(request()->query())->links('pagination::tailwind') }}</div>
            @endif
        </div>
    </div>
@endsection
