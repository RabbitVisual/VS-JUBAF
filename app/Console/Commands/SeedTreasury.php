...
class SeedTreasury extends Command
{
    protected $signature = 'treasury:seed';

    protected $description = 'Seeds Treasury module with sample data.';

    public function handle()
    {
        $this->info('🌱 Iniciando seed do módulo Treasury...');
        ...
        $this->info('✅ Seed do módulo Treasury concluído!');
    }
    ...
    private function seedPermissions(): void
    {
        ...
        if ($users->isEmpty()) {
            $this->warn('    ⚠️  Nenhum usuário encontrado. Pulando criação de permissões.');
        ...
        $this->info("    ✅ {$users->count()} permissões criadas");
    }

    private function seedCampaigns(): array
    {
        $this->info('  🎯 Criando campanhas...');
        ...
        $this->info('    ✅ '.count($createdCampaigns).' campanhas criadas');
    ...
    {
        $this->info('  🎯 Criando metas financeiras...');
        ...
        $this->info('    ✅ '.count($goals).' metas criadas');
    }

    private function seedFinancialEntries(array $campaigns): void
    {
        $this->info('  💰 Criando entradas financeiras...');

        $users = User::limit(10)->get();
        if ($users->isEmpty()) {
            $this->warn('    ⚠️  Nenhum usuário encontrado. Pulando criação de entradas.');
        ...
        // Atualizar valores das metas
        FinancialGoal::all()->each(function ($goal) {
            $goal->updateCurrentAmount();
        });

        $this->info("    ✅ {$entriesCreated} entradas financeiras criadas");
    }
