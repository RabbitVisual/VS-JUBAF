...
function setMode(mode) {
    ...
    try {
        document.dispatchEvent(
            new CustomEvent("notification:mode-changed", { detail: { mode } }),
        );
    } catch (_) {}
}
...
function updateNotificationBadge(count) {
    ...
    if (label) {
        label.textContent =
            count > 0 ? `${count} não lida${count !== 1 ? "s" : ""}` : "";
...
function getTypeConfig(type, priority, notificationType) {
    ...
    const base = (notificationType && byCategory[notificationType]) || {
        icon: "bell",
...
function renderListContainer(items, emptyMessage, allUrl) {
    ...
    container.innerHTML = items
        .map((item) => renderNotificationItem(item, allUrl))
        .join("");
}
...
async function fetchUnreadCount() {
    ...
    try {
        const res = await fetch(UNREAD_COUNT_URL, {
            method: "GET",
            headers: getHeaders(),
...
async function fetchAndRenderList() {
    ...
    try {
        ...
        const allUrl =
            document
                .querySelector("[data-notifications-all-url]")
                ?.getAttribute("data-notifications-all-url") || "#";
        renderListContainer(
            items,
...
function onNotificationReceived() {
    ...
    fetchUnreadCount();
    if (!isTabVisible())
        showNotificationToast({ title: "Nova(s) notificação(ões)" });
}
        ...
        .then(({ default: Pusher }) => {
            return import("laravel-echo").then(({ default: Echo }) => {
                ...
                const echo = new Echo({
                    ...
                    forceTLS:
                        (window.Laravel?.pusherScheme || "https") === "https",
